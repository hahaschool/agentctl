...
| **C** | `agent/claude-1/feat/memory-ops-pr-c` | Frontend: Settings → Memory & Embeddings + ApiError.details | minor | In progress; rebase onto post-#802 main before PR | [pr-c.md](./pr-c.md) |
...
PRs must merge in order A → B → C → D → E → F → G. PR A landed in PR #783 and PR B landed in PR #797; continue with C next from latest `main` after the post-#802 roadmap sync. Each remaining PR has its own branch off `main`. Never branch from another PR's branch.
