...
| **D** | `codex/memory-ops-pr-d` | Backend: jobs CRUD + BullMQ + SSE + preview endpoint + capabilities | patch | Active implementation branch from post-#811 `main@d64ae2f2`; focused tests/build passing locally | [pr-d.md](./pr-d.md) |
...
PRs must merge in order A → B → C → D → E → F → G. PR A landed in PR #783, PR B landed in PR #797, and PR C landed in PR #806 with PR #807/#808 as saved-provider test persistence and hardening follow-ups. PR D is active in `codex/memory-ops-pr-d` from post-#811 `main@d64ae2f2`; after it merges, continue with PR E from latest `main`. Each remaining PR has its own branch off `main`. Never branch from another PR's branch.
