...
|----|--------|------|------|--------|------|
| **A** | `agent/claude-1/feat/memory-ops-pr-a` | Foundation: migration + shared types + EmbeddingClient additive + credential_kind filter | patch | Critical path | [pr-a.md](./pr-a.md) |
...
PRs must merge in order A → B → C → D → E → F → G. Each PR has its own branch off `main`. Never branch from another PR's branch.
