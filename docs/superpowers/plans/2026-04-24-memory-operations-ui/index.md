...
| **C** | `agent/claude-1/feat/memory-ops-pr-c` | Frontend: Settings → Memory & Embeddings + ApiError.details | minor | Landed in PR #806; saved-provider test persistence follow-up landed in PR #807 | [pr-c.md](./pr-c.md) |
| **D** | `agent/claude-1/feat/memory-ops-pr-d` | Backend: jobs CRUD + BullMQ + SSE + preview endpoint + capabilities | patch | Next critical path slice; branch from post-#807 `main` | [pr-d.md](./pr-d.md) |
...
PRs must merge in order A → B → C → D → E → F → G. PR A landed in PR #783, PR B landed in PR #797, and PR C landed in PR #806 with PR #807 as the saved-provider test persistence follow-up. Continue with PR D next from latest `main` after the post-#807 roadmap sync. Each remaining PR has its own branch off `main`. Never branch from another PR's branch.
