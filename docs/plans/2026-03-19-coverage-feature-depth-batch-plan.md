...
>
> **Status update (2026-04-25):** Roadmap §20.1-§20.8 are delivered on `main`: task batches 1-5 landed via PRs #259, #256, #261, #258, and #257; §20.2 and §20.3 landed via PRs #266 and #265; §20.4 Memory Dashboard activation landed later; and §20.5 gained additional browser-depth follow-ups with `/logs` Security Findings Playwright coverage in PR #454, `/memory/browser` facts-flow coverage in PR #455, `/memory/import` wizard completion/cancellation coverage in PR #457, backend-independent `/settings` notification preference add/remove coverage in PR #458 with CI gating in PR #546, `/discover` grouped/filter/import/new-session coverage in PR #459, `/memory` index redirect/sidebar coverage in PR #461, `/memory/consolidation` queue/action coverage in PR #462, broader `/memory/scopes` flow coverage in PR #465, broader `/memory/graph` flow coverage in PR #466, dynamic `/spaces/[id]` + `/tasks/[id]` detail coverage in PR #468 with CI gating in PR #762, agent detail CI gating in PR #765, emergency-stop CI gating in PR #766, broader tasks/spaces CI gating in PR #767, task auto-decompose CI gating in PR #768, machine-terminal CI gating in PR #769, and memory import plus memory scopes CI gating in PR #771. The focused Web E2E CI lane now runs 32 spec files / 118 executed tests.
