...
| 6 | Planted-needle recall regression bench pattern | Phase 0 | 🟡 |
...
- [ ] memory-search with empty DB returns { results: [], total: 0 }, not
      an error.
- [ ] memory-recall with empty DB returns { facts: [], edges: [] }.
- [ ] memory-stats with empty DB returns { fact_count: 0,
      drawer_count: 0, ... } — all zeros, not nulls.
- [ ] memory-traverse from a nonexistent entity returns empty graph, not
      404.
- [ ] Every MCP route accepts { arguments: null } without hanging
      (issue-#394 parallel — mirrors MemPalace's fix).
- [ ] memory-dedup-check on empty DB recommends 'store_new' with
      nearest_matches: [].

Test file: packages/agent-worker/src/api/routes/__tests__/
memory-cold-start.test.ts. Use an isolated test DB with zero rows
across all memory tables. Shared with control-plane route tests via a
helper createEmptyMemoryDb().
...
Phase 0 ships two eval layers:
  ...
  Layer 2 — Internal planted-needle regression (new):
    packages/control-plane/src/memory/eval/planted-needle.bench.ts

    1. Insert N = MEMORY_BENCH_NEEDLE_COUNT (default 100) facts with
       content prefix "NEEDLE_<uuid>: <synthetic content>".
    2. Insert M = MEMORY_BENCH_NOISE_COUNT (default 2000) noise facts
       generated by templated sentences.
    3. For each needle, run memory-search with the content minus the
       NEEDLE_ prefix as query.
    4. Assert recall@5 >= MEMORY_BENCH_MIN_RECALL (default 0.85).
    5. Report p50 / p95 / p99 search latency.

    Runs on every CP PR in under 30s. Unlike the external eval, this
    enforces a hard threshold — PR blocked if recall drops.

Unlike MemPalace's equivalent, we enforce the threshold (they just
observe). Also bench scales: rerun at N={100, 1000, 5000} on tagged
releases and publish the curve to catch sublinear-degradation bugs.
```
