...
`.github/workflows/memory-evals.yml`; the remaining Phase 0 work is private
fixture secret provisioning/changelog discipline and richer failure-mode-
targeted examples.
...
4. Add deterministic split discipline:
   ...
   - release-tag runs skip with an explicit summary until the private fixture, database, and embedding secrets are configured; manual dispatch stays strict so operators can validate the private setup deliberately.
...
- [x] Human-readable eval report now prints per-category/per-tag metrics plus capped failure examples without changing `--json` output. *(current `main`)*
