...
6. Partially delivered: `InjectionBudget.tierTokenCaps` landed in PR #720, and this branch adds `resultMode` plus bounded injector wrapper plumbing for PostgreSQL `fact-plus-snippet`; true `full-drawer` handling and deeper additive budget accounting remain open.
7. Keep default injection `fact-only` until eval and UI are ready.
8. Delivered in PRs #734/#735: add a Surface A generator dry-run that creates a bounded L0/L1 `MEMORY.md` proposal from reviewed facts without overwriting human-curated content, and point it at the real Claude project memory path. Remaining bridge work is DB/API-backed reviewed-fact selection plus the final approval/write flow.
...
- Context-budget enforcement respects per-tier token caps; this branch adds bounded snippet evidence caps, while true full-drawer treatment and deeper additive-budget accounting remain open.
...
- [ ] Injector supports `fact-only` plus a bounded PostgreSQL `fact-plus-snippet` slice, but still needs true `full-drawer` mode and tighter per-tier/additive budget accounting. *(The `InjectionBudget.tierTokenCaps` contract/enforcement landed in PR #720; bounded result-mode plumbing lands in this branch.)*
