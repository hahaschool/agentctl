...
- This follow-up branch adds a paged control-plane `/api/memory/facts` source for dry-run reviewed-fact selection, with JSON input preserved for fixtures.
- Next step is promoting that dry-run bridge into the final write-gated approval flow.
...
snapshot, and the current changelog-discipline follow-up requires a dated
...
5. **PR E: Provenance + Injection Budget**
   ...
   - Remaining: promotion of the current Surface A dry-run generator into the final write-gated approval flow.
...
- [x] Private eval workflow runs require a dated fixture changelog when the workflow gate is enabled, and upload the changelog with the eval artifact. *(current changelog-discipline follow-up.)*
