...
3. **PR C: Backfill**
   ...
   - PR #841: Memory Import API jobs get persisted job state, restart-visible interruption, a resume endpoint, and a web summary resume action on top of PR #836's job-id provenance plus rollback endpoint/UI. This follow-up closes the structured `facts[]` graph gap for `claude-mem` observations by creating child facts and child-to-parent `summarizes` edges. Broader `claude-mem` source-local idempotency and embedding-similarity dedup remain follow-ups.
