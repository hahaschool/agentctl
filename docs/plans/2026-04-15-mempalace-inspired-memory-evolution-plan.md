...
7. Current follow-up: persist Memory Import API jobs in `memory_import_jobs`, surface restart-visible interrupted jobs, resume durable JSONL/claude-mem imports from saved progress, and show the resume action in the web import summary. PR #836 already stamped imported facts with `import_job_id`, allowed finished jobs to roll back their own rows, and showed the rollback action in the web import summary.
...
1. **PR A: Eval Harness**
   ...
   - Current follow-up: private workflow evals now require a dated fixture changelog alongside the private fixture JSON, using `MEMORY_EVAL_REQUIRE_FIXTURE_CHANGELOG=true` and `--fixture-changelog`.
