...
**Status sync (2026-04-25):** `origin/main@0d507c5d` is current through PR #862 after PR #857/#858/#859/#861/#862. PR #857 adds gitignored private eval fixture/changelog scaffold tooling through `pnpm memory:eval:fixture-scaffold`, PR #858 adds the first bounded read-only `GET /api/memory/timeline` control-plane route without schema or mesh payload changes, PR #859 adds dry-run Memory Operations rollout preflight checks through `pnpm memory:ops:preflight`, PR #861 hardens equivalent `as_of`/`asOf` timeline timestamp aliases, and PR #862 adds the worker-side MCP timeline route. Phase 0 still has the deterministic eval foundation, planted-needle bench, facts-only baseline, cold-start MCP contracts, opt-in live `pnpm memory:eval --no-mock` wiring, richer human-readable reporting, workflow-owned weekly/release automation, private-fixture failure-mode coverage and changelog gates, fixture coverage surfacing, secret preflight tooling, the PR #851 private-fixture authoring guard, the PR #854 workflow gate, and the PR #857 fixture scaffold (PRs #655, #660, #667, #681, #685, #713, #722, #729, #796, #802, #815, #833, #835, #851, #854, #857). Phase 1/1.5 still has drawer schema/chunking/sanitization/audit, resumable JSONL and `claude-mem` backfill, import rollback/resume durability, stable cursors, structured `claude-mem` child facts, and source-keyed import ids (PRs #671, #679, #682, #686, #692, #699, #703, #708, #721, #836, #841, #842, #845, #847, #849). Phase 3 includes provenance schema/write/read paths, stable empty-provenance envelopes, legacy compatibility guards, tier token caps, Surface A dry-run/API sourcing, approval-gated writes, bounded `fact-plus-snippet`, true PostgreSQL `full-drawer`, additive evidence token accounting, and store/API merge-provenance preservation (PRs #684, #688, #720, #728, #730, #734, #735, #736, #737, #738, #748, #801, #828, #830, #832). Phase 4 drawer/fact fusion hardening remains complete (PRs #707, #712, #716, #717). Phase 6 now has pure entity canonicalization helpers and benchmark groundwork, the first local-only production canonical entity schema/store slice, dry-run proposal tooling, the first fact-validity/direct-edge timeline read slice, alias compatibility, and worker MCP wiring (PRs #840, #852, #855, #858, #861, #862), while human-approved apply/runtime integration, dedicated temporal edge fields, edge-level recursive/as_of behavior, and UI timeline wiring remain open. Phase 7 has the first web visibility slices, Memory Browser drawer/evidence coverage, mobile-sized session/agent fact-detail evidence rendering, and dashboard quick-action accessibility/contrast coverage (PRs #709, #719, #723, #726, #728, #803, #839). Adjacent Memory Operations work includes the DB-backed embedding provider backend, Settings -> Memory & Embeddings provider UI, saved-provider persistence/hardening, backend job-orchestration shell, embedding/drawer backfill workers, the `/memory/operations` page with alerts/egress flow, consolidation/synthesis workers with focused coverage and rollout docs, and the dry-run rollout preflight CLI (PRs #797, #806-#808, #812, #816, #820, #827, #859). Remaining plan work is operator provisioning of the validated private eval secrets, filling the scaffold with richer private held-out/full failure-mode examples under the coverage gate, embedding-similarity dedup, drawer-aware fusion default-on criteria, Memory Browser evidence filters, why-this-matched/raw-source filters, Diary/Timeline result treatment, temporal edge fields beyond PR #858's first read slice, UI timeline wiring, human-approved entity canonicalization apply/runtime integration beyond PR #855's dry-run proposals, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification after running the PR #859 preflight in an intentionally prepared environment.

**Post-PR #862 operational checkpoint:** GitHub reported no open PRs after PR #862 merged; post-merge main CI `24931385074`, Security Audit `24931385101`, and Build & Publish Docker Images `24931385068` passed at `0d507c5d`. Direct alert-list API calls reported 0 open code-scanning, Dependabot, and secret-scanning alerts.

**Post-PR #849 import durability and structure note:** PR #839 and PR #840 also landed before PR #841. PR #841 added persisted Memory Import API jobs and resume UI/API; PR #842 repairs direct resume of restart-orphaned `running` jobs by reconciling them to `interrupted` before reacquiring the singleton running slot. PR #845 adds `memory_import_jobs.cursor_json`, resumes JSONL imports from the last completed relative path/import source id instead of a positional index, advances `claude-mem` resume with observation-id cursors after the first checkpointed row, and counts same-job JSONL rows as imported when the fact write survived but the progress checkpoint did not. PR #847 then imports structured `claude-mem` `facts[]` as child `memory_facts` and links each child to its parent observation fact with a `summarizes` edge. PR #849 qualifies `claude-mem` observation and structured child-fact import ids with a stable source key, preserving same-source idempotency while avoiding row-id collisions across different local databases.
...
1. Follow [2026-04-25-memory-mesh-temporal-sync-compat-contract.md](2026-04-25-memory-mesh-temporal-sync-compat-contract.md) before opening any schema PR.
2. `memory_edges` currently syncs whole-row payloads because `sync_capture_change()` serializes `to_jsonb(NEW)`.
3. `memory_drawers` currently has no sync trigger and must remain unsynced unless a later PR defines an explicit drawer envelope.
4. If edge temporal fields are intentionally synced, route them through the existing schema-version/protocol compatibility flow introduced by `0025_sync_nodes_peer_version.sql`, `0027_sync_nodes_schema_ahead_rejection.sql`, and `docs/MESH_COMPAT.md`, with old-peer, legacy-envelope, and schema-ahead tests before enablement.
...
- PR #734 adds the first dry-run generator that reads reviewed-fact JSON input and proposes a bounded Surface A summary without writing.
- PR #735 corrects the Claude project memory target to `~/.claude/projects/<encoded>/memory/MEMORY.md`.
- PR #737 adds a paged control-plane `/api/memory/facts` source for dry-run reviewed-fact selection, with JSON input preserved for fixtures.
- PR #832 promotes that dry-run bridge into the final write-gated approval flow with deterministic proposal tokens, durable approval gates, and approved-token verification before writes.
- Keep the no-auto-edit invariant: human-curated lines still require a dry-run diff plus the approval-token write flow before any generated section is written.
...
endpoint guards while keeping mock ranking as the default. Current `main` adds
human-readable By Tag tables plus capped failure examples while leaving JSON
output unchanged. PR #729 adds the release/weekly held-out automation via
`.github/workflows/memory-evals.yml`, PR #796 adds the workflow/CLI
failure-mode coverage gate, and PR #802 surfaces required-tag coverage counts
in CLI/report output while keeping workflow summaries/artifacts available on
failed eval runs. PR #815 gives the public sample fixture one sanitized,
focused row per default failure-mode tag plus a refreshed facts-only baseline
snapshot, and PR #833 requires a dated
private fixture changelog in workflow-owned eval runs. PR #857 adds
`pnpm memory:eval:fixture-scaffold` so operators can create the gitignored
private fixture plus changelog scaffold with excluded placeholder rows; the
remaining Phase 0 work is private fixture secret provisioning/rotation and
filling that scaffold with richer reviewed private held-out/full
failure-mode examples under the gate.
...
- A test asserts fixture failure-mode coverage. *(PR #796 adds the default-on private-fixture gate.)*
...
**Status:** State persistence landed in PR #682. PR #686 added the JSONL drawer backfill CLI foundation, PR #692 added the `--source-type claude-mem` drawer-write path, PR #699 added observation atomic-fact writes with offset source spans, PR #703 added session-summary atomic-fact writes, and PR #708 repaired source-key compatibility plus missing-source-span retry edge cases. PR #721 added dry-run estimates for drawer chunks, embedding tokens, embedding cost, and storage bytes. Current `main` finishes embedding generation/backfill for CLI-written facts and adds bounded batch/retry behavior to the backfill embedding path. PR #836 added job-id provenance to imported facts plus a rollback endpoint/UI action for completed import jobs. PR #841 added `memory_import_jobs` persistence so Memory Import API jobs survive process restart as interrupted/resumable work instead of only living in the singleton active job, and PR #842 makes direct resume reconcile orphaned `running` jobs before reacquiring the running slot. PR #845 stores source-specific import cursors in `cursor_json`, prevents JSONL file-list churn from shifting resume work, persists `claude-mem` observation-id cursors after per-row progress updates, and treats same-job JSONL rows that survived a crash-before-checkpoint as imported instead of skipped. PR #847 adds Memory Import API child-fact creation from `claude-mem` `facts[]` plus child-to-parent `summarizes` edges. PR #849 adds source-keyed `claude-mem` observation/child-fact import ids so source-local idempotency does not collapse different databases with overlapping row ids. Remaining scope is embedding-similarity dedup.
...
2. ✅ PR #692 added the `--source-type claude-mem` drawer-write path; PR #699 added observation atomic-fact writes; PR #703 added session-summary atomic-fact writes; current `main` adds embedding generation/backfill for CLI-written facts plus bounded batching/backoff.
3. ✅ Add resumable state in `memory_drawer_backfill_state`.
4. Batch embedding calls and rate-limit with exponential backoff.
5. ✅ JSONL CLI writes through `(source_type, source_id, chunk_index)` and resumes by cursor; PR #849 extends source-local idempotency coverage to Memory Import API `claude-mem` observation and structured child-fact import ids.
...
7. PR #841 persists Memory Import API jobs in `memory_import_jobs`, surfaces restart-visible interrupted jobs, resumes JSONL/claude-mem imports from saved progress, and shows the resume action in the web import summary. PR #842 reconciles restart-orphaned `running` jobs before direct resume. PR #836 already stamped imported facts with `import_job_id`, allowed finished jobs to roll back their own rows, and showed the rollback action in the web import summary. PR #845 adds `cursor_json` resume state, non-positional JSONL cursor resume, same-job JSONL crash-before-checkpoint accounting, and per-row `claude-mem` progress cursor updates. PR #847 adds `claude-mem` `facts[]` child facts plus `summarizes` edges.
...
6. Delivered: `InjectionBudget.tierTokenCaps` landed in PR #720, PR #736 adds `resultMode` plus bounded injector wrapper plumbing for PostgreSQL `fact-plus-snippet`, PR #828 adds true PostgreSQL `full-drawer` handling for on-demand facts with source-drawer hydration and global/on-demand tier caps, and PR #830 debits additive evidence tokens against the global and per-tier budgets for `fact-plus-snippet` and `full-drawer`.
7. Keep default injection `fact-only` until eval and UI are ready.
8. Delivered in PRs #734/#735/#737/#832: add a Surface A generator dry-run that creates a bounded L0/L1 `MEMORY.md` proposal from reviewed facts without overwriting human-curated content, point it at the real Claude project memory path, and allow reviewed facts to come from the control-plane API as well as JSON fixtures. Remaining bridge work is follow-on operator UX, rollout, and e2e hardening around the delivered write gate.
...
- Context-budget enforcement respects per-tier token caps; PR #736 adds bounded snippet evidence caps, PR #828 adds first-slice full-drawer caps for on-demand facts, and PR #830 accounts additive evidence tokens against global/per-tier budgets.
...
**Status:** PR #685 delivered the worker-side `memory_traverse` contract and shared types: argument validation, default/hard max-hop caps, relation/min-confidence filters, null-argument 400s, and missing/no-data empty graph responses through the planned control-plane proxy. PR #689 delivered the control-plane `POST /api/memory/traverse` route with iterative per-hop BFS, hop/node cap enforcement and `partial` flag, `relation_types` / `min_confidence` / `as_of` filters against endpoint facts, and 404 on missing start entities (15 Vitest cases). PR #840 adds pure entity canonicalization helpers and a read-only extraction benchmark, PR #852 adds the first local-only production canonical entity schema/store slice without sync triggers or fact/edge rewrites, PR #855 adds dry-run canonicalization proposals for JSON/JSONL candidate rows plus alias JSON/JSONL or local DB alias reads, PR #858 adds the first read-only `GET /api/memory/timeline` route with fact-validity windows, direct edges, opaque cursors, and explicit limitations around fact-id fallback and missing edge temporal fields, PR #861 hardens equivalent `as_of`/`asOf` timestamp aliases, and PR #862 adds the worker-side MCP timeline route. The mesh precondition for any future temporal edge payload change now lives in [2026-04-25-memory-mesh-temporal-sync-compat-contract.md](2026-04-25-memory-mesh-temporal-sync-compat-contract.md). Remaining scope is dedicated temporal edge fields, UI timeline wiring, human-approved production entity canonicalization apply/runtime integration beyond PR #855's dry-run proposal tooling, and recursive-CTE-based `as_of` validity-window behavior at the edge level.
...
- Create/extend: `packages/control-plane/src/memory/entity-timeline.ts` *(first control-plane fact-validity/direct-edge slice delivered in PR #858)*
- Create: `packages/control-plane/src/memory/entity-extraction-benchmark.ts`
- Create/extend: `packages/control-plane/src/api/routes/memory-timeline.ts` *(first read-only GET route delivered in PR #858)*
- Create/extend: `packages/agent-worker/src/api/routes/memory-timeline.ts` *(worker-side MCP route delivered in PR #862)*
...
3. Ship entity canonicalization with Phase 6. It is not polish because timeline joins depend on stable identity:
   - PR #840 covers pure helper/benchmark groundwork, PR #852 covers the first local-only production `memory_entities` / `memory_entity_aliases` schema plus store helpers, and PR #855 covers dry-run proposal output for review without mutating facts, edges, aliases, or mesh sync state.
   - Add nullable canonical IDs to facts and temporal edge subjects/objects.
   - Extend the PR #852 alias model only through explicit migrations; today it stores `canonical_id text`, `alias`, `normalized_alias`, and `source_json`.
...
5. Add timeline API:
   ...
   - PR #861 hardened conflicting alias rejection and invalid `asOf` errors on that control-plane route.
   - Extend beyond PR #858/#861/#862 with canonical-ID matching, edge-level temporal fields, recursive edge traversal, web wiring, and the mutation route below.
   - `POST /api/memory/edges/:id/invalidate`
6. Define MCP `memory_timeline` parameters *(worker route delivered in PR #862; fuller temporal semantics remain pending)*:
...
PR #728 added fact-detail evidence previews plus archived/unavailable
provenance markers in the Memory Browser detail panel. Remaining Phase 7 work
is raw-source filters, the context-picker include-raw toggle, broader
why-this-matched explanations, Diary/Timeline result treatment, and
...
2. Delivered in PR #728: the first evidence/detail panel lands with Phase 3 read paths; Phase 7 now covers follow-on polish and richer explanation/filtering.
...
- Backend-independent Memory Browser drawer result section render. *(PR #723 unit coverage, PR #726 browser/E2E coverage.)*
- Evidence/detail panel render. *(PR #728 shared route + web unit coverage; browser/E2E follow-up can land when the lane widens again.)*
...
| `MEMORY_EVAL_REQUIRE_FIXTURE_CHANGELOG` | boolean | `false` | scripts/workflow |
| `EVAL_SPLIT_SEED` | number | `42` | control-plane/scripts |

## Suggested PR Slices

1. **PR A: Eval Harness**
   - Delivered across PR #655/#660/#667/#681/#685/#713/#722 plus current `main` follow-up for the current scope: fixture schema/sanitization, seed-42 split helpers, deterministic mock scoring, sanitized sample fixture, `pnpm memory:eval`, planted-needle recall bench, current memory MCP cold-start/null-arguments contracts, first `memory_dedup_check` empty-DB/null-arguments route coverage, first `memory_traverse` worker cold-start/null-arguments coverage, facts-only baseline snapshot, opt-in live `--no-mock` eval wiring, and human-readable By Tag/failure-example reporting.
   - PR #833: private workflow evals now require a dated fixture changelog alongside the private fixture JSON, using `MEMORY_EVAL_REQUIRE_FIXTURE_CHANGELOG=true` and `--fixture-changelog`.
   - PR #857: `pnpm memory:eval:fixture-scaffold` creates the gitignored private fixture/changelog scaffold with excluded placeholder rows for authoring.
   - Remaining: private fixture secret provisioning/rotation and richer private held-out/full failure-mode examples filled into the scaffold.
...
3. **PR C: Backfill**
   ...
   - Completed on current `main`: execute-mode embedding generation/backfill for CLI-written facts plus bounded batching/backoff.
   - PR #841: Memory Import API jobs get persisted job state, restart-visible interruption, a resume endpoint, and a web summary resume action on top of PR #836's job-id provenance plus rollback endpoint/UI. PR #847 closes the structured `facts[]` graph gap for `claude-mem` observations by creating child facts and child-to-parent `summarizes` edges. PR #849 adds source-keyed `claude-mem` import ids so source-local idempotency survives overlapping observation ids across different local databases; embedding-similarity dedup remains a follow-up.
...
5. **PR E: Provenance + Injection Budget**
   ...
   - Delivered: PR #832 promoted the current Surface A dry-run generator into the final write-gated approval flow. Remaining: rollout and e2e hardening around that gate.
...
6. **PR F: Drawer-Aware Search**
   ...
   - Delivered: CP drawer search + get index integration landed in PR #704, production embedding-client wiring landed in PR #707, the first web drawer-search page landed in PR #709, gated facts-side drawer fusion landed in PR #712, drawer route/helper hardening landed in PR #716, and facts-side fusion sanitization plus strict `drawerLimit` validation landed in PR #717. Remaining: eval comparison and no default enablement until metrics pass.
...
8. **PR H: Temporal Timeline**
   ...
   - PR #858 added the first read-only control-plane timeline route over fact-validity windows and direct edges.
   - PR #861 hardened equivalent timeline timestamp aliases, and PR #862 added the worker MCP `memory_timeline` route.
   - Remaining: edge temporal fields, control-plane recursive traversal, human-approved production canonicalization apply/runtime integration beyond PR #855's dry-run proposals, UI timeline wiring, and edge-level `as_of` validity-window behavior only after mesh sync gate.
...
- [x] Extracted facts link to supporting drawer offsets without copying quoted content. *(PR #688 foundation, PR #699 observations, PR #703 session summaries, and PR #708 retry/source-key edge-case repair with fallback provenance over every returned drawer chunk.)*
- [x] Legacy facts without drawer provenance still search, render, inject, and sync. *(Stable empty-preview detail/search compatibility landed across PR #730 and PR #801; mesh sync behavior is unchanged because these are read-path guards.)*
...
- [x] Eval harness uses deterministic 10% dev / 90% held-out split with seed 42 and gates `held-out/full` runs behind workflow-owned env flags rather than a local bypass. *(PR #655 foundation; current `main` gate hardening.)*
...
- [x] Human-readable eval report now prints per-category/per-tag metrics plus capped failure examples without changing `--json` output. *(current `main`; PR #802 also surfaces required-tag coverage counts.)*
- [x] Public sample fixture has one sanitized, focused failure-mode-targeted row for vocabulary gap, temporal ambiguity, assistant-reference, person-name, and noisy-distractor failure modes. *(PR #815)*
- [x] Private eval workflow runs require a dated fixture changelog when the workflow gate is enabled, and upload the changelog with the eval artifact. *(PR #833)*
- [x] Private fixture authoring has a gitignored scaffold command for the private JSON plus fixture changelog, with excluded placeholder rows for the default failure-mode tags. *(PR #857)*
- [ ] Private held-out/full fixtures still need richer failure-mode-targeted examples under the workflow-owned coverage gate.
...
- [x] Phase 0 records a facts-only baseline number before drawer-aware search changes ranking. *(PR #713 committed `docs/memory-evals/phase-0-facts-only-baseline.json` with deterministic `--baseline facts-only` output from `scripts/memory-bench.ts`, stamped with fixture SHA256, git SHA, and ISO timestamp.)*
...
- [x] Injector supports `fact-only`, bounded PostgreSQL `fact-plus-snippet`, and first-slice true `full-drawer` mode for on-demand facts with source-drawer hydration plus global/on-demand tier caps. *(The `InjectionBudget.tierTokenCaps` contract/enforcement landed in PR #720; bounded result-mode plumbing landed in PR #736; full-drawer landed in PR #828; additive evidence token accounting for `fact-plus-snippet` and `full-drawer` landed in PR #830.)*
- [x] Checkpoint capture cannot block an agent run, proven by scheduler-injected route coverage that returns `202` before capture starts. *(PR #800)*
- [x] PreCompact checkpoint hook returns within 2.5 seconds under a stalled queue fixture. *(PRs #776, #784; PR #800 also defers route-triggered capture onto the next tick.)*
...
- [x] Mesh sync behavior for drawers and temporal edge fields is explicit before any sync payload changes. *(See [2026-04-25-memory-mesh-temporal-sync-compat-contract.md](2026-04-25-memory-mesh-temporal-sync-compat-contract.md): `memory_edges` still syncs whole-row payloads via `sync_capture_change()`, `memory_drawers` remains unsynced, and any later payload change now requires an explicit projection-or-version-gated rollout contract.)*
- [ ] Phase 6 includes production entity canonicalization apply/runtime integration beyond PR #855's dry-run proposal tooling, or ships with an explicit UI warning and follow-up PR.
- [x] Phase 6 has a first read-only control-plane timeline route for bounded direct-edge reads over fact validity windows without changing schema or mesh payloads. *(PR #858)*
