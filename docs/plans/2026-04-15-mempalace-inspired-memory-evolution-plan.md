...
6. ✅ Dry-run mode reports discovered files/lines/chunks, parse errors, estimated tokens, estimated embedding cost, and estimated storage across JSONL, secret-sanitized, and claude-mem paths. *(PR #721)*
...
6. Extend `InjectionBudget` with `tierTokenCaps` and result modes. *(tierTokenCaps half delivered in PR #720 as an additive optional `Partial<Record<InjectionTier, number>>` enforced after the global cap in `context-budget.ts`; `resultMode` plumbing remains open.)*
...
1. Add result type badges: Fact, Drawer, Diary, Timeline. *(Fact side delivered in PR #719 via `MatchSourceBadge` + `sourcePathByFactId` threading through `FactsList`/`FactCard` with a partial "why this matched" row; drawer side delivered in PR #723 via extracted `drawerMatchType.ts` helper and the new `DrawerResultsSection` rendered in `MemoryBrowserView`. Diary and timeline badge surfaces remain open along with the Step 4 context-picker "include raw drawers" toggle and per-row drawer-detail navigation.)*
...
1. **PR A: Eval Harness**
   - Delivered across PR #655/#660/#667/#681/#685/#722 for the current scope: fixture schema/sanitization, seed-42 split helpers, deterministic mock scoring, sanitized sample fixture, `pnpm memory:eval`, planted-needle recall bench, current memory MCP cold-start/null-arguments contracts, first `memory_dedup_check` empty-DB/null-arguments route coverage, first `memory_traverse` worker cold-start/null-arguments coverage, and the opt-in `--no-mock` live-search adapter wiring `scripts/memory-eval.ts` through control-plane `MemorySearch` with mock ranking still the default.
...
3. **PR C: Backfill**
   ...
   - PR #721 added dry-run chunk-count, embedding-token, embedding-spend, and storage-byte estimators across JSONL, secret-sanitized, and claude-mem paths with a configurable default embedding price, closing the cost/storage estimate gap.
   - Remaining: PR #708 source-key/source-span repair, embedding generation/backfill for CLI-written facts, and batching/backoff.
...
5. **PR E: Provenance + Injection Budget**
   ...
   - PR #720 extended `InjectionBudget` in `@agentctl/shared` with an additive optional `tierTokenCaps?: Partial<Record<InjectionTier, number>>` field that `context-budget.ts` enforces as a per-tier ceiling after the global cap (`DEFAULT_INJECTION_BUDGET` unchanged, tests 32 → 37), delivering the `tierTokenCaps` half of Phase 3 Step 6.
   - Remaining: fact-source write-path read-time quote previews, injector `resultMode` plumbing (fact-only / fact-plus-snippet / full-drawer), and Surface A dry-run generator.
