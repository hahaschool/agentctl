...
**Status sync (2026-04-25):** `origin/main@bf0bf676` is current through PR #852 after PR #839/#840/#841/#842/#845/#846/#847/#848/#849/#850/#851/#852. PR #850 was docs-only, PR #851 added aggregate-only `pnpm memory:eval:fixture-check` validation for private coverage/changelog/split sanity without printing fixture contents, and PR #852 added local-only `memory_entities` / `memory_entity_aliases` tables plus store helpers without mesh sync triggers or fact/edge rewrites. Phase 0 still has the deterministic eval foundation, planted-needle bench, facts-only baseline, cold-start MCP contracts, opt-in live `pnpm memory:eval --no-mock` wiring, richer human-readable reporting, workflow-owned weekly/release automation, the private-fixture failure-mode coverage gate, fixture coverage surfacing in CLI/report/workflow output, and the PR #851 private-fixture authoring guard (PRs #655, #660, #667, #681, #685, #713, #722, #729, #796, #802, #851). PR #815 gives the public sample fixture one sanitized, focused row per default failure-mode tag and refreshes the facts-only baseline snapshot, PR #833 makes private eval runs require a dated fixture changelog through `MEMORY_EVAL_REQUIRE_FIXTURE_CHANGELOG=true` and `--fixture-changelog`, PR #835 adds `pnpm memory:eval:secrets` so private fixture/changelog secret rotation has a local validation and stdin-based `gh` apply path that does not print secret bodies, and PR #837 synced the roadmap/plan state after those landings. Phase 1/1.5 still has drawer schema/chunking/sanitization/audit, resumable JSONL and `claude-mem` backfill, fact-source provenance repair, dry-run chunk/token/cost/storage estimates, import job rollback, durable import resume state, orphaned running-job direct-resume reconciliation, PR #845's persisted import cursor plus same-job JSONL crash-before-checkpoint accounting, PR #847's structured `claude-mem` child facts plus child-to-parent edges, and PR #849's source-keyed `claude-mem` observation/child-fact import ids (PRs #671, #679, #682, #686, #692, #699, #703, #708, #721, #836, #841, #842, #845, #847, #849). Phase 3 now includes provenance schema/write/read paths, stable empty-provenance envelopes, legacy no-provenance compatibility guards, `InjectionBudget.tierTokenCaps`, Surface A dry-run generation/API sourcing, approval-gated writes (PR #832), bounded `fact-plus-snippet` result mode, true PostgreSQL `full-drawer` injection for on-demand facts with source-drawer hydration and global/on-demand caps, additive global/per-tier evidence token accounting for `fact-plus-snippet` and `full-drawer`, and store-level/API-level near-duplicate merge provenance preservation (PRs #684, #688, #720, #728, #730, #734, #735, #736, #737, #738, #748, #801, #828, #830, #832), plus nonblocking PreCompact checkpoint capture with timeout/stdout/ack-before-capture hardening (PRs #776, #784, #800). Phase 4 drawer/fact fusion hardening remains complete (PRs #707, #712, #716, #717). Phase 6 now has pure entity canonicalization helpers and a read-only extraction benchmark from PR #840 plus PR #852's first local-only production canonical entity schema/store slice, while canonicalization backfill/apply/runtime integration and timeline fields/routes remain open. Phase 7 has the first web visibility slices, Memory Browser drawer/evidence coverage, mobile-sized session/agent fact-detail evidence rendering, and dashboard quick-action accessibility/contrast coverage (PRs #709, #719, #723, #726, #728, #803, #839). Adjacent Memory Operations work now includes the DB-backed embedding provider backend, Settings -> Memory & Embeddings provider UI, saved-provider persistence/hardening, backend job-orchestration shell, embedding/drawer backfill workers, the `/memory/operations` page with alerts/egress flow, and consolidation/synthesis workers with focused unit/Playwright coverage and rollout docs (PRs #797, #806-#808, #812, #816, #820, #827). Adjacent hardening includes memory identifier/path validation, cleared dependency/security alert waves, exact-hit TypeScript build-cache restores after PR #818 fixed stale shared declaration reuse in main CI, and PR #822/#823/#825's Docker scanner/runtime refresh plus fixed-CVE reporting policy after post-#821/post-#822 container annotations (PRs #732, #733, #777, #818, #822, #823, #825). Basic JSONL history import is wired through the Memory Import UI/API (PR #788), PR #794 adds duplicate re-import prevention, aligned preview/import counts, bounded fd-based reads, file-count overflow errors, and visible import-start failures, PR #836 stamps imported facts with an import job id, adds a rollback endpoint for finished jobs, PR #841 adds persisted `memory_import_jobs`, restart-visible interrupted job state, a resume endpoint, and a Web resume action, PR #842 reconciles restart-orphaned `running` jobs before direct resume, PR #845 adds stable cursor-based resume for JSONL and `claude-mem` imports, PR #846 synced roadmap/plan state after PR #845, PR #847 splits `claude-mem` `facts[]` entries into child facts with child-to-parent `summarizes` edges, and PR #849 qualifies `claude-mem` import ids by source key so overlapping observation ids from different local databases no longer collide. The explicit pre-schema mesh contract for future drawer sync and temporal edge fields now lives in [2026-04-25-memory-mesh-temporal-sync-compat-contract.md](2026-04-25-memory-mesh-temporal-sync-compat-contract.md). Remaining plan work is operator provisioning of the validated private eval secrets, richer private held-out/full failure-mode examples under the coverage gate, embedding-similarity dedup, drawer-aware fusion default-on criteria, Memory Browser evidence filters, why-this-matched/raw-source filters, Diary/Timeline result treatment, temporal edge fields, entity canonicalization backfill/apply/runtime integration beyond PR #852's local schema/store, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification.

**Post-PR #852 operational checkpoint:** GitHub reported no open PRs after PR #852 merged; post-merge main CI `24928487675`, Security Audit `24928487676`, and Build & Publish Docker Images `24928487694` passed at `bf0bf676`. Direct alert-list API calls still reported 0 open code-scanning, Dependabot, and secret-scanning alerts.
...
private fixture changelog in workflow-owned eval runs; the remaining Phase 0
work is private fixture secret provisioning/rotation and richer private
held-out/full failure-mode examples under that gate.
...
3. Ship entity canonicalization with Phase 6. It is not polish because timeline joins depend on stable identity:
   - PR #840 covers pure helper/benchmark groundwork, and PR #852 covers the first local-only production `memory_entities` / `memory_entity_aliases` schema plus store helpers.
...
1. **PR A: Eval Harness**
   ...
   - Remaining: private fixture secret provisioning/rotation and richer private held-out/full failure-mode examples.
...
8. **PR H: Temporal Timeline**
   ...
   - Remaining: edge temporal fields, control-plane recursive traversal, production canonicalization backfill/apply/runtime integration beyond PR #852's local schema/store, timeline routes, and `as_of` validity-window behavior only after mesh sync gate.
...
- [ ] Phase 6 includes production entity canonicalization backfill/apply/runtime integration beyond PR #852's local schema/store, or ships with an explicit UI warning and follow-up PR.
