...
failed eval runs. The public sample fixture now includes one sanitized, focused
row per default failure-mode tag plus a refreshed facts-only baseline snapshot;
...
- [x] Public sample fixture has one sanitized, focused failure-mode-targeted row for vocabulary gap, temporal ambiguity, assistant-reference, person-name, and noisy-distractor failure modes.
