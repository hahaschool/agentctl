...
5. ✅ JSONL CLI writes through `(source_type, source_id, chunk_index)` and resumes by cursor; this follow-up extends source-local idempotency coverage to Memory Import API `claude-mem` observation and structured child-fact import ids.
...
3. Ship entity canonicalization with Phase 6. It is not polish because timeline joins depend on stable identity:
   - PR #840 covers pure helper/benchmark groundwork only; the production tables, backfill, and approved apply workflow below remain open.
...
3. **PR C: Backfill**
   ...
   - PR #841: Memory Import API jobs get persisted job state, restart-visible interruption, a resume endpoint, and a web summary resume action on top of PR #836's job-id provenance plus rollback endpoint/UI. PR #847 closes the structured `facts[]` graph gap for `claude-mem` observations by creating child facts and child-to-parent `summarizes` edges. This follow-up adds source-keyed `claude-mem` import ids so source-local idempotency survives overlapping observation ids across different local databases; embedding-similarity dedup remains a follow-up.
...
8. **PR H: Temporal Timeline**
   ...
   - Remaining: edge temporal fields, control-plane recursive traversal, production canonicalization schema/backfill/apply beyond PR #840 helper/benchmark, timeline routes, and `as_of` validity-window behavior only after mesh sync gate.
...
- [ ] Phase 6 includes production entity canonicalization schema/backfill/apply beyond PR #840 helper/benchmark, or ships with an explicit UI warning and follow-up PR.
