...
**Status sync (2026-04-25):** `origin/main@18abf0df` is current through PR #841 after PR #839 and PR #840. The earlier Phase 0/1.5/3/4/7 and Memory Operations checkpoint history is retained below and in `docs/ROADMAP.md`; the live remaining work is operator private-eval provisioning, richer private held-out/full examples, import edge creation, broader `claude-mem` source-local idempotency, crash-atomic import progress accounting, non-positional JSONL resume cursors, embedding-similarity dedup, drawer-aware fusion default-on criteria, Memory Browser evidence filters, why-this-matched/raw-source filters, Diary/Timeline result treatment, temporal edge fields, typed entity alias scoping and benchmark sampling hardening, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification.

**Post-PR #841 hotfix note:** PR #839 and PR #840 also landed before PR #841. PR #841 added persisted Memory Import API jobs and resume UI/API; this hotfix repairs direct resume of restart-orphaned `running` jobs by reconciling them to `interrupted` before reacquiring the singleton running slot. Remaining import-resume hardening is crash-atomic per-unit progress accounting and a non-positional JSONL cursor so file churn cannot shift the resume index.
