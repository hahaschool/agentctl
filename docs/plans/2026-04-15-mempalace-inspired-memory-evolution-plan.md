...
4. Partially delivered in this follow-up branch: store-level helper copies and de-duplicates `memory_fact_sources` spans onto a survivor fact. Remaining work is API-level near-duplicate merge wiring that calls it before invalidating duplicate facts.
...
5. **PR E: Provenance + Injection Budget**
   ...
   - Remaining: true full-drawer budget treatment, merge-time provenance preservation, and promotion of the current Surface A dry-run generator into the final write-gated approval flow.
