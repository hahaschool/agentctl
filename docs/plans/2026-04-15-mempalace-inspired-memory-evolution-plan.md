...
**Status sync (2026-04-25):** `origin/main@b36171aa` is current through PR #825. Phase 0 still has the deterministic eval foundation, planted-needle bench, facts-only baseline, cold-start MCP contracts, opt-in live `pnpm memory:eval --no-mock` wiring, richer human-readable reporting, workflow-owned weekly/release automation, the private-fixture failure-mode coverage gate, and fixture coverage surfacing in CLI/report/workflow output (PRs #655, #660, #667, #681, #685, #713, #722, #729, #796, #802). PR #815 gives the public sample fixture one sanitized, focused row per default failure-mode tag and refreshes the facts-only baseline snapshot. Phase 1/1.5 still has drawer schema/chunking/sanitization/audit, resumable JSONL and `claude-mem` backfill, fact-source provenance repair, and dry-run chunk/token/cost/storage estimates (PRs #671, #679, #682, #686, #692, #699, #703, #708, #721). Phase 3 now includes provenance schema/write/read paths, stable empty-provenance envelopes, legacy no-provenance compatibility guards, `InjectionBudget.tierTokenCaps`, Surface A dry-run generation/API sourcing, bounded `fact-plus-snippet` result mode, store-level and API-level near-duplicate merge provenance preservation (PRs #684, #688, #720, #728, #730, #734, #735, #736, #737, #738, #748, #801), and nonblocking PreCompact checkpoint capture with timeout/stdout/ack-before-capture hardening (PRs #776, #784, #800). Phase 4 drawer/fact fusion hardening remains complete (PRs #707, #712, #716, #717). Phase 7 has the first web visibility slices, Memory Browser drawer/evidence coverage, and mobile-sized session/agent fact-detail evidence rendering (PRs #709, #719, #723, #726, #728, #803). Adjacent Memory Operations work now includes the DB-backed embedding provider backend, Settings -> Memory & Embeddings provider UI, saved-provider test persistence, safe/stale-write hardening, backend job-orchestration shell, embedding/drawer backfill workers with boot reconciliation and source-root safety, and the `/memory/operations` page with alerts/egress flow (PRs #797, #806-#808, #812, #816, #820), which clears the operator configuration, orchestration, worker, and page path before PR G consolidation/synthesis and e2e coverage. Adjacent hardening includes memory identifier/path validation, cleared dependency/security alert waves, exact-hit TypeScript build-cache restores after PR #818 fixed stale shared declaration reuse in main CI, and PR #822/#823/#825's Docker scanner/runtime refresh plus fixed-CVE reporting policy after post-#821/post-#822 container annotations (PRs #732, #733, #777, #818, #822, #823, #825). Basic JSONL history import is wired through the Memory Import UI/API (PR #788), PR #794 adds duplicate re-import prevention, aligned preview/import counts, bounded fd-based reads, file-count overflow errors, and visible import-start failures. Remaining plan work is private fixture secret provisioning/changelog discipline, richer private held-out/full failure-mode examples under the coverage gate, JSONL rollback/resume plus embedding-similarity dedup, true full-drawer behavior and deeper additive budget accounting, promotion of the current Surface A dry-run bridge into the final write-gated approval flow, drawer-aware fusion default-on criteria, Memory Browser evidence filters, why-this-matched/raw-source filters, Diary/Timeline result treatment, temporal edge fields, entity canonicalization, dev-1 backfill verification, and Memory Operations PR G e2e/consolidation-synthesis follow-through.
...
- Context-budget enforcement respects per-tier token caps; PR #736 adds bounded snippet evidence caps, while true full-drawer treatment and deeper additive-budget accounting remain open.
...
5. **PR E: Provenance + Injection Budget**
   ...
   - Remaining: true full-drawer budget treatment and promotion of the current Surface A dry-run generator into the final write-gated approval flow.
