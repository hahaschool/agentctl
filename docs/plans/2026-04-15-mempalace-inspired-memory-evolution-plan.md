...
6. Partially delivered: `InjectionBudget.tierTokenCaps` landed in PR #720, PR #736 adds `resultMode` plus bounded injector wrapper plumbing for PostgreSQL `fact-plus-snippet`, and PR #828 adds true PostgreSQL `full-drawer` handling for on-demand facts with source-drawer hydration and global/on-demand tier caps; broader additive budget accounting remains open.
...
- Context-budget enforcement respects per-tier token caps; PR #736 adds bounded snippet evidence caps, and PR #828 adds first-slice full-drawer caps for on-demand facts while broader additive-budget accounting remains open.
...
5. **PR E: Provenance + Injection Budget**
   ...
   - Remaining: broader additive budget accounting beyond the first full-drawer cap and promotion of the current Surface A dry-run generator into the final write-gated approval flow.
