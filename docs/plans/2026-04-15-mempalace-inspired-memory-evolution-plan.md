...
**Status:** State persistence landed in PR #682. PR #686 added the JSONL drawer backfill CLI foundation, PR #692 added the `--source-type claude-mem` drawer-write path, PR #699 added observation atomic-fact writes with offset source spans, PR #703 added session-summary atomic-fact writes, and PR #708 repaired source-key compatibility plus missing-source-span retry edge cases. PR #721 added dry-run estimates for drawer chunks, embedding tokens, embedding cost, and storage bytes. Current `main` finishes embedding generation/backfill for CLI-written facts and adds bounded batch/retry behavior to the backfill embedding path. The in-flight JSONL import rollback slice adds job-id provenance to imported facts plus a rollback endpoint/UI action for completed import jobs. Remaining scope is broader source-local idempotency coverage for the claude-mem path, durable resume state for Memory Import API jobs beyond the singleton active job, and embedding-similarity dedup.
...
7. In flight: stamp Memory Import API facts with `import_job_id`, allow finished import jobs to roll back their own rows, and show the rollback action in the web import summary.
...
3. **PR C: Backfill**
   ...
   - Current in-flight follow-up: JSONL Memory Import API jobs get job-id provenance, a rollback endpoint, and a web summary rollback action; durable resume state and embedding-similarity dedup remain follow-ups.
