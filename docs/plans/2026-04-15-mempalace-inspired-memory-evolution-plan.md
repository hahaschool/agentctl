...
3. **PR C: Backfill**
   ...
   - PR #841: Memory Import API jobs get persisted job state, restart-visible interruption, a resume endpoint, and a web summary resume action on top of PR #836's job-id provenance plus rollback endpoint/UI. Import edge creation, broader `claude-mem` source-local idempotency, and embedding-similarity dedup remain follow-ups.
