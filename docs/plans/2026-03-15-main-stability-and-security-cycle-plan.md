...
- PR #410 closed the manual-takeover GET route CodeQL `#579` false positive with a narrow suppression marker and source-shape coverage, keeping the real `@fastify/rate-limit` preHandler before authorization.
- PR #411 repaired the pre-existing AgentsPage and AgentDetailPage targeted web test failures, restoring the 123-test targeted suite before merge.
