...
- There is no remaining open PR, Dependabot, or secret-scanning backlog from this cycle on `main`.
- The only currently open code-scanning alert on `main` is `#560` (`js/missing-rate-limiting`) in `packages/control-plane/src/api/routes/sync.ts`; treat it as a fresh mesh sync follow-up rather than as a regression in the earlier worker/discovery/scripts backlog.
