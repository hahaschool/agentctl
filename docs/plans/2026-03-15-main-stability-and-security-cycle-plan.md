...
> Status note (2026-04-01): the original stabilization/security wave remains delivered on `main` for the CI + CodeQL + Dependabot + DAST backlog through PR #230, with follow-on docs sync/consistency landed in PRs #232-#233. The later 2026-04-01 follow-up then closed via PR #373 (restore the Security Audit path and sanitize remote-error logging) and PR #380 (brace-expansion override follow-through, historical gitleaks fingerprints, and remaining scripts/security cleanup). There are currently no open PRs or Dependabot alerts on `main`; the only remaining open code-scanning item is alert `#560` (`js/missing-rate-limiting`) in `packages/control-plane/src/api/routes/sync.ts`. Treat that as a narrow sync-route follow-up, not a reopening of the broader CI/security cycle.
...
- The broad historical backlog from this cycle is closed on `main`, and the 2026-04-01 follow-up landed via PRs #373 and #380.
- There are currently no open PRs or Dependabot alerts on `main`; the only remaining open code-scanning item touching this area is alert `#560` (`js/missing-rate-limiting`) in `packages/control-plane/src/api/routes/sync.ts`.
