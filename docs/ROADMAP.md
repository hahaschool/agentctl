...
- [ ] Import completeness — durable job state plus restart-visible resume/rollback UX is covered through PR #841 and PR #842; PR #845 adds a persisted JSONL resume cursor plus same-job crash-before-checkpoint accounting so file churn and stale checkpoints no longer shift JSONL resume work into skipped rows. Edge creation from imported structured fields, broader `claude-mem` source-local idempotency, and embedding-similarity dedup remain follow-up work.
