...
- [x] Persist drawer backfill state: `0032_add_memory_drawer_backfill_state.sql`, Drizzle schema/journal/schema tests, shared `MemoryDrawerBackfill*` contracts, and `MemoryDrawerBackfillStateStore` create/list/update status/cursor coverage *(PR #682)*.
