...
**7,260+ unit tests** across 111+ files + **218+ Playwright e2e checks**, with the backend-independent webhooks, agent-profiles, audit, machines, and agents list slices now gated in CI. The scheduler jobs page is now reachable from the web sidebar and covered by focused mocked-query unit tests. All packages build and lint cleanly (TypeScript 0 errors, Biome 0 errors).
...
> Docker action refs to their Node 24 majors, disables the Buildx binary cache
> that was still invoking Node20 `actions/cache`, and removes the temporary
> forced Node 24 runtime env from publish/audit workflows without changing
> deploy-dev, promote-beta, deploy-prod, or the dev-1/dev-2 runtime
> configuration.
...
- [x] Docker publish, Security Audit container scan, and local-mode DAST Docker action refs upgraded to Node 24 majors with Buildx binary caching disabled to avoid the internal Node20 cache action; dev and beta deployment workflows intentionally unchanged in this lane
