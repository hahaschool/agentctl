...
> Last updated: 2026-04-15 (post-#558 mesh version UI checkpoint). Latest mesh landing set: PR #550 persisted peer ping diagnostics, PR #551 added in-place mesh peer editing, PR #552 added the rate-limited signed reverse-registration endpoint, PR #553 registered 33.9-33.11 planning, PR #554 added machine-origin provenance badges, PR #555 added nullable peer-version columns, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #557 added mesh envelope schema/protocol metadata plus apply-side compat gates and `docs/MESH_COMPAT.md`, and PR #558 added `/mesh-peers` version/drift UI plus the lazy-loaded sidebar peer-version tooltip. This hotfix also renumbers the peer-version migration from duplicate `0024` to `0025`. Earlier 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
...
### 33.9 Mesh Version Observability (P1) — In Progress
...
- [ ] **Persist peer version on ping.** Have `pingPeer()` capture `appVersion`, `gitSha`, and `schemaVersion` from `/health` and store them alongside `sync_status`/`last_seen`.
...
- [ ] **Tests.** Unit coverage for `/health` shape plus `/mesh-peers` version/drift/sidebar behavior landed in PRs #556/#558; remaining coverage is the ping-persistence integration test once ping captures peer metadata.
...
| **P1** | Mesh Version Observability | 33.9 | In progress — PR #555 added nullable `sync_nodes` peer-version columns plus schema/shared contracts, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #558 delivered `/mesh-peers` version/drift UI plus the lazy sidebar peer-version tooltip, and this hotfix renumbers the peer-version migration to `0025` so deploy migrations do not share the `0024` tag. Remaining work: persist peer metadata on ping and add that integration coverage. See [plans/2026-04-15-mesh-version-observability-plan.md](plans/2026-04-15-mesh-version-observability-plan.md). |
...
| [mesh-version-observability-plan](plans/2026-04-15-mesh-version-observability-plan.md) | In progress — PR #555 delivered nullable peer-version schema/shared groundwork, PR #556 delivered `/health` build metadata, PR #558 delivered the version/drift UI, and the post-merge hotfix renumbers the peer-version migration to `0025`; remaining scope is ping persistence plus integration coverage | 33.9 |
...
| [mesh-fleet-rollout-peer-update-plan](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md) | Planned — defines Docker fleet activation plus PM2 mesh peer-update CLI/timers/UI/rollback, gated on 33.9 ping/UI drift visibility and the remaining 33.10 operator-warning/two-node proof | 33.11 |
