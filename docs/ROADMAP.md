...
> Last updated: 2026-04-25 (post-#777 memory-ops, PreCompact checkpoint, synthesis, and postcss security sync).
>
> Current checkpoint: `main@a2704b1a` is current through PR #777. PRs #770–#777 merged since the last checkpoint: PR #770 (`aea2c43d`) synced the roadmap after PR #769, PR #771 (`72ada103`) gated the memory import and scopes E2E slices into CI, PR #774 (`c3b1680d`) surfaced LLM-proposed synthesis principle candidates from the weekly cron in the Memory Synthesis page with a dedicated "Principle Candidates" tab, accept/dismiss actions, and badge counts, PR #775 (`7ab21539`) merged the Memory Operations UI v5 design spec plus per-PR implementation plans (PRs A–G), PR #776 (`1a9f687f`) added a nonblocking PreCompact memory checkpoint (`POST /api/sessions/:sessionId/pre-compact`) with 202-immediate response and async fact capture, and PR #777 (`a2704b1a`) pinned transitive PostCSS to patched 8.5.10 (GHSA-qx2v-qp2m-jg93). Recent memory/security/runtime landings through PR #769 include PR #728 fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 workflow-owned memory-eval automation and staged private-fixture docs, PR #730 legacy empty-provenance stabilization, PR #731 roadmap sync after PR #730, PR #732 shared `sanitizeName()` hardening across control-plane and worker memory routes, PR #733 merged the `@xmldom/xmldom` + `uuid` security override fix onto `main`, PR #734 added the first script-only `MEMORY.md` dry-run generator that preserves human-curated content while proposing bounded generated sections from reviewed facts, PR #735 corrected the Claude project memory target to `~/.claude/projects/<encoded>/memory/MEMORY.md`, PR #736 added bounded injector result modes with default `fact-only`, PostgreSQL `fact-plus-snippet`, and explicit `full-drawer` fallback to `fact-only`, PR #737 advanced Surface A by allowing the dry-run generator to source reviewed facts from the control-plane `/api/memory/facts` API as well as JSON fixtures, PR #738 started merge-time provenance preservation with a store-level helper that copies and de-duplicates `memory_fact_sources` spans from duplicate facts onto the survivor fact, PR #739 upgraded Docker publish attestations to the current Node 24-backed `actions/attest-build-provenance@v4.1.0` action, PR #740 bounded Codex JSONL discovery and repaired Discover preview content lookup, PR #741 registered the missing `/api/security/findings` routes in the control-plane server, PR #742 aligned Codex content lookup with `CODEX_HOME` and worktree-derived project paths while preserving bounded tail reads, PR #743 aligned the audit summary backend shape with the frontend contract, PR #744 synced the roadmap and audit plan through that backend-contract repair, PR #745 aligned the mobile `AuditSummary` client contract and focused tests with the same backend/web shape, PR #748 wired API-level near-duplicate memory consolidation merges, PR #752 made memory eval release enforcement configurable, PR #754 surfaced peer-update stderr tails while guarding too-old peers, PR #755 persisted session run summaries, PR #756 forwarded database passwords into remote deployment migrations, PR #757 aligned notification user identity defaults, PRs #758/#759/#761 added router/runtime settings browser coverage, PR #760 added a confirmed Web `/scheduler` remove-all action over the existing bulk scheduler deletion API, PR #762 moved `spaces-tasks-detail.spec.ts` into the focused Web E2E gate while isolating Security Audit container-scan caches by image, PRs #763/#764 corrected memory UI delivery gaps and follow-up regressions, and PRs #765-#769 expanded the focused Web E2E gate to cover agent detail, emergency stop, tasks/spaces, task auto-decompose, and machine terminal slices.
>
> GitHub triage on 2026-04-25 has the roadmap current through PR #777. GitHub currently reports 0 open code-scanning alerts, 0 open Dependabot alerts, and 0 open secret-scanning alerts after PR #777 fixed Dependabot #39 (`postcss < 8.5.10`). Beta is intentionally left on the v0.8.2 promotion stack; the post-beta mainline changes are recorded here for the next full promotion rather than hot-applied into beta.
>
...
> Coordination note addendum 4: PR #770 merged as `aea2c43d`, PR #771 as `72ada103`, PR #774 as `c3b1680d`, PR #775 as `7ab21539`, PR #776 as `1a9f687f`, and PR #777 as `a2704b1a`; PR #772 was closed unmerged after PR #775 superseded it with a rewritten clean history for the memory-ops v5 spec and per-PR plans.
>
...
> to 30 spec files / 110 executed tests in latest CI by adding agent detail, emergency stop, tasks/spaces,
> task auto-decompose, and machine terminal coverage.
...
- [x] Add worker-side nonblocking PreCompact checkpoint capture with explicit hook/API timeout semantics and diary-as-fact continuity so long sessions keep context without blocking compaction *(PR #776; follow-up: automatic per-session hook registration)*.
...
- [ ] **PR A** — DB migration `0033` (api_accounts extensions + 3 new tables), Drizzle schema, shared types (`providers.ts`, `ops.ts`, `ops-audit.ts`), EmbeddingClient additive extensions, `credential_kind='runtime'` filter on 6 routes. Gate 1 contract test. *(branch: `agent/claude-1/feat/memory-ops-pr-a`)*
...
>
> Reality checkpoint (2026-04-24): the memory route family and core PostgreSQL
...
> explicit `memory_edges` and imported claude-mem observations currently arrive
> mostly as isolated facts; reports are deterministic SQL summaries rather than
> LLM-generated narratives; JSONL history import is not wired; and rich
> consolidation/graph modes from the original design remain follow-up work.
...
- [x] Consolidation Board (`/memory/consolidation`) — basic review queue and accept/skip/delete actions over consolidation items.
- [ ] Consolidation completeness — restore richer contradiction/near-duplicate merge workflows, editable resolutions, and clearer post-action state instead of treating the current stale-fact queue as the full design.
...
- [ ] Import completeness — JSONL history import, rollback/resume UX, edge creation from imported structured fields, and embedding-similarity dedup are not delivered; current claude-mem dedup is source-id based.
...
- Latest focused Web E2E gate sync (2026-04-24): PR #762 moved the dynamic spaces/tasks detail spec into CI, PR #765 moved agent-detail coverage into CI, PR #766 moved emergency-stop UI coverage into CI, PR #767 moved the broader tasks/spaces spec into CI while fixing stale space-link/cache behavior, PR #768 moved task auto-decompose coverage into CI, and PR #769 moved machine-terminal coverage into CI. The latest main CI Web E2E job runs 30 spec files / 110 executed tests.
