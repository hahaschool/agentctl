...
> Coordination note addendum 3: PR #746 merged as `c6a7d502`, PR #747 as `86606cae`, PR #748 as `ba00aca5`, PR #749 as `fe63ad30`, PR #750 as `6b481af3`, and PR #751 as `0457d779`.
>
> Current follow-up note: the remaining MemPalace work is now narrower than the older PR #716-#723 checkpoint implied. Drawer-aware fusion hardening, fact-detail provenance reads, eval split gating/reporting, the script-only Surface A dry-run bridge, PR #736's bounded `fact-plus-snippet` injection mode, PR #737's paged API source for Surface A dry runs, PR #738's store-level merge-provenance primitive, and PR #748's API-level near-duplicate merge wiring are covered. The next honest gaps remain private fixture secret provisioning/changelog discipline, richer failure-mode-targeted eval examples, true `full-drawer` result handling plus deeper additive budget accounting, broader evidence UI/default-on rollout criteria, and turning the dry-run bridge into a write-gated approval flow. Until the private eval secrets exist, scheduled and release-tag memory evals should skip with an explicit summary rather than fail release hygiene; manual dispatch remains the strict path for validating the private setup.
