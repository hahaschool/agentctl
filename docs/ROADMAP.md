...
- [x] Add worker-side nonblocking PreCompact checkpoint capture with explicit hook/API timeout semantics and diary-as-fact continuity so long sessions keep context without blocking compaction *(PR #776; PR #784 keeps the hook under the timeout budget and lets stdout flush; follow-up: automatic per-session hook registration)*.
...
- [ ] **PR C** — Settings UI: `ApiError.details` extension, `memoryProvidersApi` web client, `ProviderDialog` (test-before-save, verified-only catalog), `MemoryEmbeddingsSection` in Settings. Minor version bump. *(branch: `agent/claude-1/feat/memory-ops-pr-c`)*
