...
> Last updated: 2026-04-25 (post-PR #841 durable Memory Import resume hotfix).
>
> Current checkpoint: `main@18abf0df` is current through PR #841 after PR #839's dashboard quick-action accessibility CI coverage and PR #840's entity canonicalization/extraction benchmark groundwork. The historical PR #779-#797 Memory Operations provider-backend checkpoint remains recorded in the coordination addenda below; current unresolved memory gaps are listed in the follow-up note and Memory UI rows.
>
...
> Post-#841 durability note: direct resume now reconciles orphaned `running` import rows before acquiring the running slot. Remaining import-resume hardening is crash-atomic per-unit progress accounting and a non-positional JSONL cursor so file churn cannot shift the resume index.
