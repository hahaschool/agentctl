...
> Last updated: 2026-04-25 (post-#855 roadmap sync).
>
> Current checkpoint: `main@f00e4c91` is current through PR #855 after PR #853's drawer/temporal mesh-sync compatibility contract docs, PR #854's workflow-owned private fixture validation gate, and PR #855's dry-run canonicalization proposal tooling. PR #849 qualifies `claude-mem` Memory Import `import_source_id` values with a stable source key so overlapping observation IDs from different local databases no longer collide while repeated imports from the same source still skip. PR #851 adds aggregate-only `pnpm memory:eval:fixture-check` coverage/changelog/split validation without printing fixture contents. PR #852 adds local-only `memory_entities` and `memory_entity_aliases` tables plus store helpers without mesh sync triggers or historical fact/edge rewrites. PR #853 documents the sync compatibility gate for future drawer payload and temporal edge changes. PR #854 wires `pnpm memory:eval:fixture-check` into `.github/workflows/memory-evals.yml` before live eval runs and uploads only aggregate fixture-check reports. PR #855 adds `pnpm memory:canonicalization:propose` for dry-run JSON/JSONL candidate rows plus alias JSON/JSONL or local DB alias reads, with JSON/CSV resolved/ambiguous/unresolved output, bounded sanitized previews, CSV formula hardening, and no `--apply`/`--execute` mutation path. The historical PR #779-#797 Memory Operations provider-backend checkpoint remains recorded in the coordination addenda below; current unresolved memory gaps are listed in the follow-up note and Memory UI rows.
>
...
| **P1** | MemPalace-Inspired Memory Evolution | 3.6 / 4.8 / 7.3 | In progress — current through PR #855 on `main@f00e4c91`. Delivered slices include the eval harness/live wiring and workflow-owned private fixture gates (PRs #655/#660/#722/#729/#796/#802/#815/#833/#835/#851/#854), drawer schema/backfill/import durability and structured `claude-mem` import work (PRs #671/#682/#686/#721/#836/#841/#842/#845/#847/#849), provenance/injection/Surface A approval-gated writes plus full-drawer evidence budgeting (PRs #684/#728/#738/#748/#828/#830/#832), Memory Operations provider/job/UI/consolidation workers (PRs #797/#806-#808/#812/#816/#820/#827), and Phase 6 entity groundwork through helpers/benchmark, local-only schema/store, and dry-run proposal tooling (PRs #840/#852/#855). v1.3 keeps the existing PostgreSQL-native memory core; the sync precondition for future drawer payloads and temporal edge fields is explicit in [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). Remaining work continues with operator application of validated eval secrets, richer private held-out/full examples, Memory Browser evidence filters/default-on rollout criteria, raw-source filters, fuller why-this-matched explanations, Diary/Timeline result treatment, rank-bucket boosts, retention, control-plane `memory_traverse` traversal/timeline gates, dedicated temporal edge fields/timeline API/as_of validity-window behavior, human-approved canonicalization apply/runtime wiring beyond PR #855's dry-run proposals, embedding-similarity dedup, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification. See [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md) and [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). |
...
| **P0** | Agent Run Quality | 16.1 | Current loop is closed through PR #855. PR #853 documented the drawer/temporal mesh sync compatibility contract, PR #854 put aggregate-only private fixture validation directly in the Memory Evals workflow before live runs, and PR #855 landed dry-run canonicalization proposal tooling; post-merge main CI `24929456518`, Security Audit `24929456512`, and Build & Publish Docker Images `24929456505` passed at `f00e4c91`. There are no open PRs during this sync, and code-scanning, Dependabot, and secret-scanning alert lists report 0 open alerts. Historical stability work through PR #852 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. |
