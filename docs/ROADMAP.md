...
> Last updated: 2026-04-25 (post-#852 roadmap sync).
>
> Current checkpoint: `main@bf0bf676` is current through PR #852 after PR #839's dashboard quick-action accessibility CI coverage, PR #840's entity canonicalization/extraction benchmark groundwork, PR #841's durable Memory Import job state plus resume UI/API, PR #842's restart-orphaned running-job resume fix, PR #845's persisted Memory Import cursor/resume hardening, PR #846's roadmap/plan sync, PR #847's structured `claude-mem` import child facts plus `summarizes` edges, PR #848's post-#847 roadmap/plan sync, PR #849's source-keyed `claude-mem` import ids, PR #850's docs-only checkpoint sync, PR #851's private memory eval fixture guard, and PR #852's local-only entity canonicalization schema/store slice. PR #849 qualifies `claude-mem` Memory Import `import_source_id` values with a stable source key so overlapping observation IDs from different local databases no longer collide while repeated imports from the same source still skip. PR #851 adds aggregate-only `pnpm memory:eval:fixture-check` coverage/changelog/split validation without printing fixture contents. PR #852 adds local-only `memory_entities` and `memory_entity_aliases` tables plus store helpers without mesh sync triggers or historical fact/edge rewrites. PR #850 changed docs only and did not alter runtime or schema behavior. The historical PR #779-#797 Memory Operations provider-backend checkpoint remains recorded in the coordination addenda below; current unresolved memory gaps are listed in the follow-up note and Memory UI rows.
>
> Post-#797 delta: PR #799 (`368da836`) synced the roadmap after the provider-backend merge, PR #800 (`c1f28d8e`) deferred PreCompact checkpoint capture so the API acknowledges before capture work starts, PR #801 (`d64f3ba3`) guarded legacy no-provenance memory fact/detail/search compatibility, PR #802 (`253a77db`) surfaced private-fixture coverage counts in memory eval reports and kept workflow summaries/artifacts available on failed eval runs, PR #803 (`234da9e4`) hardened session/agent memory evidence rendering with tap-through detail sheets, bounded previews, expand/collapse, and overflow-safe wrapping, PR #804 (`a4ca2538`) synced the roadmap through PR #802 before PR #805 corrected it through PR #803, PR #805 (`d3ce9c2c`) recorded the PR #803 evidence-rendering landing, PR #806 (`f99d42a8`) landed Memory Operations UI PR C with the Settings -> Memory & Embeddings surface and provider CRUD/test controls, PR #807 (`7d8f2594`) persisted saved-provider test success/failure metadata so row-level test results survive refreshes and new sessions, PR #808 (`dd9af010`) hardened that saved-provider status path with safe upstream-error summaries, credential-guarded metadata writes, stale-write 409s, and post-refetch UI override cleanup, PR #809 (`da4674c6`) synced roadmap/plan docs through the Memory Operations settings landing, PR #810 (`67deaf39`) folded the concurrent PR #808/#809 status back into the roadmap and Memory Ops plan docs, PR #811 (`d64ae2f2`) reconciled the remaining post-#810 Memory Ops/MemPalace plan drift after the concurrent docs branches, PR #812 (`b58a798f`) delivered Memory Operations PR D with the backend job-orchestration infrastructure, PR #813 (`63792a8d`) synced the roadmap and Memory Ops plan docs after that PR D landing, PR #814 (`f80abb05`) verified the post-#813 mainline checks and resynced the roadmap checkpoint, PR #815 (`b553868b`) added focused public memory-eval failure-mode samples plus the refreshed facts-only baseline, PR #816 (`3ed7c4c`) delivered Memory Operations PR E workers for embedding and drawer backfills with boot reconciliation, cancellation/fail-ratio guards, bounded cost tracking, source-root containment, symlink escape rejection, and the `MEMORY_OPS_ENABLED_KINDS=embedding-backfill,drawer-backfill` unlock, PR #818 (`13e3fa2a`) removed broad TypeScript build-cache fallback restores after the post-#816 main Build exposed stale `@agentctl/shared` declaration reuse, restoring main Build stability, PR #817 (`1b4b9854`) merged the post-#818 roadmap sync and verified the resulting mainline checks and alert counts, PR #819 (`644ac311`) corrected the stale post-#817 checkpoint record, PR #820 (`01d71b42`) delivered Memory Operations PR F with the `/memory/operations` page, web client/query helpers, job cards, detail drawer/SSE stream, egress confirmation flow, mixed-model banner, sidebar nav, missing-provider alerts across the memory surfaces, and `MEMORY_OPS_ENABLED=true`, PR #821 (`5444e283`) synced the roadmap and Memory Operations plan after PR #820, PR #822 (`42635458`) refreshed production Docker apt layers and the agent-worker libngtcp2 runtime package/cache-bust path after post-#821 container scans emitted high vulnerability annotations, PR #823 (`df12baea`) pinned the production control-plane and agent-worker Node runtime images to 22.22.2 to clear the remaining Node 22.22.1 Grype high finding after PR #822, PR #824 (`37a552f6`) synced roadmap and plan docs through that Docker runtime follow-up, and PR #825 (`b36171aa`) aligned Docker publish Grype scanning with Trivy's ignore-unfixed policy by reporting only fixed high/critical vulnerabilities.
>
> Post-#825 delta: PR #826 (`595822a4`) synced roadmap and plan docs through the Docker fixed-CVE reporting policy, PR #827 (`7bc1a639`) delivered Memory Operations PR G with consolidation/synthesis worker handlers, env/docs rollout notes, changelog/quickstart updates, and focused unit plus Playwright coverage, PR #828 (`751fb236`) enabled PostgreSQL `full-drawer` MemoryInjector mode for on-demand facts with source-drawer hydration and global/on-demand tier caps, PR #829 (`6ba2a622`) synced roadmap and plan docs through PR #828, PR #830 (`d9f49985`) applied additive global/per-tier evidence token accounting to `fact-plus-snippet` and `full-drawer` rendering, PR #831 (`5729a74f`) synced roadmap and plan docs after PR #830, PR #832 (`122e47cf`) gated Surface A `MEMORY.md` writes behind deterministic approval tokens plus durable `/api/approvals` gates, PR #833 (`3eca0152`) made private memory eval runs require a dated fixture changelog artifact, PR #834 (`04158328`) reconciled the roadmap/plan records after those Surface A and eval-gate landings, PR #836 (`1f7e4649`) added JSONL import job provenance plus rollback UI/API for completed memory imports, PR #835 (`4bc82924`) added local private fixture/changelog secret validation plus stdin-based `gh` rotation tooling, PR #837 (`d3c2228f`) synced the roadmap/plan state after those landings, PR #840 (`350517dc`) added pure entity canonicalization helpers plus a read-only extraction benchmark, PR #839 (`4b209322`) added dashboard quick-action accessibility/unit coverage and put the dashboard contrast spec in focused Web E2E CI, PR #841 (`18abf0df`) added durable Memory Import job state plus restart-visible resume, PR #842 (`7d13de12`) reconciled restart-orphaned running import jobs before direct resume, PR #845 (`e12f82bc`) added `memory_import_jobs.cursor_json`, stable JSONL resume cursors, same-job JSONL crash-before-checkpoint accounting, and per-row `claude-mem` resume progress cursors, PR #846 (`966ca34d`) synced roadmap/plan state after PR #845, PR #847 (`8ccff1de`) imports structured `claude-mem` `facts[]` as child `memory_facts` with child-to-parent `summarizes` edges, PR #848 (`8526fabe`) synced roadmap/plan state after PR #847, PR #849 (`f304e40b`) scoped `claude-mem` Memory Import ids by source key for source-local idempotency, PR #851 (`83e4e1e3`) added the private fixture guard utility, and PR #852 (`bf0bf676`) added the first production local-only entity canonicalization schema/store slice.
>
> GitHub triage on 2026-04-25 has the roadmap current through PR #852. GitHub reported no open PRs after PR #852 merged; post-merge main CI `24928487675`, Security Audit `24928487676`, and Build & Publish Docker Images `24928487694` passed at `bf0bf676`. Direct alert-list API calls reported 0 open code-scanning, Dependabot, and secret-scanning alerts. Beta is intentionally left on the v0.8.2 promotion stack; the post-beta mainline changes are recorded here for the next full promotion rather than hot-applied into beta.
>
> Post-#520 work note: PR #508 removed the nonblocking SARIF-upload dependency from the Docker publish workflow after `github/codeql-action/upload-sarif` download 429s, kept scanner reports as artifacts, left Security Audit responsible for Security-tab uploads, and serialized Security Audit's container-scan matrix. PR #509 added the missing audit-trail page plan and roadmap links. PR #510 opted `build-images.yml` and `security-audit.yml` into GitHub Actions' Node 24 JavaScript action runtime after Build & Publish Docker Images run `24405567447` passed but still emitted Node.js 20 deprecation annotations for Docker/artifact/cache actions. PR #512 fixed the subsequent Node 20 compat TypeScript failure in the control-plane agent runtime mapping. PR #513 added backend-independent `/audit` Playwright coverage. PR #515 migrates the blocking Security Audit job away from `pnpm audit` after npm retired its legacy audit endpoints and onto npm's bulk advisory endpoint while keeping high/critical advisories blocking. PR #516 adds the already-backend-independent `/machines` spec to the focused web e2e CI lane. PR #517 exposes scheduler job management in the web app over the existing scheduler API and adds unit coverage for the not-configured/create/delete paths. PR #518 adds the already-backend-independent `/agents` spec to the focused web e2e CI lane and builds `@agentctl/shared` first so cold CI checkouts can compile routes that import shared contracts.
>
> PR #520 upgrades the Docker publish Grype scanner to `anchore/scan-action@v7.4.0`, pins Grype `v0.111.0`, caches the DB, and validates/upload artifacts from explicit `grype-${{ matrix.package }}.sarif` paths. PR #522 scopes the explicit CI `node_modules` cache keys by run/attempt so parallel PR/main runs no longer compete for the same immutable cache-save key. PR #523 moves Docker publish, Security Audit container scans, and local-mode DAST Docker builds onto Node 24 action majors, disables Buildx binary caching, disables Trivy's internal action cache so nested Node20 `actions/cache` downloads no longer produce job-level Node20 deprecation annotations, and removes the temporary forced Node 24 runtime env from the publish/audit workflows. PR #739 then upgraded the remaining build-provenance attestation action to the Node 24-backed `v4.1.0`, removing the last known Docker publish Node 20 deprecation annotation source. PR #825 then aligned Docker publish Grype output with Trivy's ignore-unfixed posture by setting `only-fixed: true`, so no-fix OS CVEs should not produce high/critical publish annotations while fixed-version findings remain visible. PR #524 synced the roadmap/main-stability checkpoint after the earlier workflow landings.
>
> PR #525 extends sidebar keyboard navigation with `1-9,0` digit shortcuts plus `g`-prefix Go To chords for every sidebar destination, renders the grouped shortcut catalog in Settings, and adds a regression so modified second keys such as `g` then `Ctrl+S` preserve browser/app shortcuts instead of navigating. PR #526/#527 synced the roadmap and related plan histories after that keyboard-accessibility landing. PR #529 and PR #528 added thin backend-independent route-smoke coverage for `/sessions/[id]` and `/memory/dashboard`, PR #530 recorded the row-level roadmap ledger after those landings, PR #532 added client-side URL-length validation to webhooks and sync peers, PR #533 added backend-independent `/scheduler` browser coverage, PR #536 removed the non-functional Consolidation Board Edit action, PR #537 fixed discovery of current Codex session files, PR #538 added approvals expander accessibility semantics, PR #539 and PR #541 recorded roadmap syncs for that landing set, PR #540 wrapped `/logs` plus `/settings` with the shared ErrorBoundary, PR #542 added an inline Accounts empty-state credential CTA plus screen-reader-visible credential validation wiring, PR #740/#742 repaired Codex Discover/content path handling, and PR #741/#743 fixed Security Findings and Audit backend contract drift. Dev-1/dev-2, production deploy, and beta promotion workflows stay untouched.
>
> Coordination note: PR #461 merged as `e6fd4c2e`, PR #462 as `193e6b02`, PR #465 as `2b950c96`, PR #466 as `9fbc2fe9`, PR #467 as `620de798`, PR #468 as `4291f520`, PR #470 as `5a480a23`, PR #471 as `2d61cb8a`, PR #472 as `7b2f9b8c`, PR #473 as `22fbe6ae`, PR #474 as `a8189309`, PR #475 as `1f4007c1`, PR #476 as `fa632acd`, PR #477 as `be1765d3`, PR #478 as `082deedb`, PR #479 as `9f984523`, PR #480 as `0f9fb538`, PR #481 as `0668db95`, PR #482 as `04e1e80d`, PR #483 as `88ccf816`, PR #485 as `071b7e3d`, PR #486 as `bfd9e74f`, PR #487 as `f28a2a85`, PR #488 as `eb9ae933`, PR #489 as `c01300b2`, PR #490 as `d868742b`, PR #491 as `f3ad66dc`, PR #492 as `8b716d6f`, PR #493 as `808556b0`, PR #494 as `95e7c8a2`, PR #495 as `5635abdf`, PR #496 as `64de9152`, PR #497 as `1b95f9f8`, PR #499 as `05972feb`, PR #498 as `3eebce46`, PR #500 as `e20e5682`, PR #501 as `d1c9635c`, PR #503 as `3b3ecf3f`, PR #504 as `8a9cda1b`, PR #505 as `bde0f343`, PR #506 as `caf3038f`, PR #507 as `d3fb1a0b`, PR #508 as `547fe24b`, PR #509 as `b0464f2c`, PR #510 as `3c694d79`, PR #512 as `62a06878`, PR #513 as `5eb844ff`, PR #511 as `52dccc06`, PR #515 as `22767f6d`, PR #516 as `4a0d67e4`, PR #517 as `2c31bcd6`, PR #518 as `00b25acf`, PR #520 as `8d6051a7`, PR #521 as `0a55bcc1`, PR #522 as `d48b0954`, PR #523 as `1d77f4e9`, PR #524 as `cbf75017`, PR #525 as `0f49e772`, PR #526 as `56ae0b24`, PR #527 as `25eac47e`, PR #529 as `1f70727e`, PR #528 as `089d6739`, PR #530 as `536480ae`, PR #536 as `aff8a80f`, PR #537 as `8150750e`, PR #538 as `7f629f57`, PR #539 as `d3a75dca`, PR #540 as `64e3668a`, PR #541 as `772fb22c`, PR #542 as `ed9ee0ee`, PR #543 as `8b44df27`, PR #544 as `3b00d8c1`, PR #546 as `9a3e698e`, PR #547 as `78c56428`, PR #548 as `33c53dc2`, PR #627 as `22391c0d`, PR #628 as `2fe44dd1`, PR #629 as `86843710`, PR #630 as `067340ba`, PR #631 as `42ddb88a`, PR #632 as `696d18ac`, PR #633 as `f8756e60`, PR #634 as `7d3b0cef`, PR #635 as `03e116a0`, PR #636 as `1c798720`, PR #637 as `9d91e2e9`, PR #638 as `97e814cd`, PR #639 as `bdbafb97`, PR #640 as `80f35fe6`, PR #641 as `7b25b39f`, PR #643 as `58767c70`, PR #644 as `05d4f1bb`, PR #645 as `8ee9b312`, PR #646 as `fced0fcf`, PR #647 as `4204430b`, PR #649 as `0070445c`, PR #650 as `81f25c26`, PR #651 as `4ecb7f54`, PR #653 as `5a3308a0`, PR #654 as `886c3360`, PR #655 as `24c2ccac`, PR #656 as `62856195`, PR #657 as `0c7e7407`, PR #658 as `2c3dd832`, PR #659 as `31d2ca97`, PR #660 as `6bcac0fd`, PR #661 as `3937128c`, and PR #662 as `f42e7ff5`; duplicate PRs #463, #464, and #519 were closed after their stronger sibling branches merged.
> Coordination note addendum: PR #663 merged as `d53e94af`, PR #664 as `5a433d68`, PR #665 as `2271bf72`, PR #666 as `ea82fec9`, PR #667 as `48bc709f`, PR #668 as `f77f8074`, PR #669 as `811bdf31`, PR #670 as `c1e1add6`, PR #671 as `e19dd7d5`, PR #672 as `b7273f5e`, PR #673 as `f106f4f7`, PR #674 as `bfe9759c`, PR #675 as `92ab8c97`, PR #677 as `1dfadaa3`, PR #678 as `bfbca627`, PR #679 as `eb4f19b2`, PR #681 as `a976cae2`, PR #682 as `a66c9bc1`, PR #684 as `a2cfdda8`, PR #685 as `150c3be7`, PR #686 as `b8ed8ce4`, PR #687 as `99e36894`, PR #688 as `135344e1`, PR #689 as `e6164cfa`, PR #691 as `465ebb2e`, PR #692 as `ef93b76d`, PR #693 as `2d5ea917`, PR #694 as `f0c632e4`, PR #695 as `47243f58`, PR #696 as `efb87d41`, PR #697 as `5ca38166`, PR #698 as `aa90762f`, PR #699 as `f7c15804`, PR #700 as `be1b4a83`, and PR #701 as `c9132f1b`, PR #702 as `6155f03a`, PR #703 as `524a32c5`, and PR #704 as `7ee21749`; PR #705 as `cdb3bb65`, PR #707 as `8ad648a5`, PR #709 as `c97b61df`, PR #708 as `c8e9d376`, PR #710 as `9ec3e1e2`, PR #711 as `dbd17a07`, PR #712 as `f8e9b1db`, PR #713 as `ec749b77`, PR #714 as `b6ab5277`, PR #716 as `f0b9b6fc`, PR #717 as `8eb872cd`, PR #718 as `afca944d`, PR #719 as `eab86e12`, PR #720 as `94081135`, PR #721 as `5592d63c`, PR #722 as `38a49e82`, PR #723 as `1d4ef0cd`, PR #725 as `48f7cf71`, PR #726 as `85a6c45a`, and PR #728 as `1d54e62d`; duplicate docs PR #724 was closed unmerged after #725 superseded it.
> Coordination note addendum 2: PR #729 merged as `e0f1003a`, PR #730 as `7b90a605`, PR #731 as `243b651f`, PR #732 as `3b3af386`, PR #733 as `3f855d48`, PR #734 as `fbf27801`, PR #735 as `6b5d2f27`, PR #736 as `eebaa49c`, PR #737 as `56b4a598`, PR #738 as `9013a908`, PR #739 as `f91340f5`, PR #740 as `04331fc1`, PR #741 as `ad58a917`, PR #742 as `da0da992`, PR #743 as `0a29dc69`, PR #744 as `2c776763`, and PR #745 as `d2a12945`.
> Coordination note addendum 3: PR #746 merged as `c6a7d502`, PR #747 as `86606cae`, PR #748 as `ba00aca5`, PR #749 as `fe63ad30`, PR #750 as `6b481af3`, PR #751 as `0457d779`, PR #752 as `cb3ecc6f`, PR #754 as `ccbca847`, PR #755 as `497dc82e`, PR #756 as `cf156f17`, PR #757 as `8dff867f`, PR #758 as `6ac89502`, PR #759 as `edfd0a1c`, PR #760 as `c1d1a0c6`, PR #761 as `5db7e8b4`, PR #762 as `9338529f`, PR #763 as `0d740472`, PR #764 as `befef71a`, PR #765 as `94440740`, PR #766 as `e5884f7d`, PR #767 as `487f6bfd`, PR #768 as `b9debd6f`, and PR #769 as `8efb447f`; duplicate PR #753 was closed unmerged after PR #752 covered the release-eval gating path.
> Coordination note addendum 4: PR #770 merged as `aea2c43d`, PR #771 as `72ada103`, PR #774 as `c3b1680d`, PR #775 as `7ab21539`, PR #776 as `1a9f687f`, PR #777 as `a2704b1a`, PR #779 as `eca52aac`, PR #780 as `800c6f3d`, PR #781 as `940a6a17`, PR #782 as `c1eb2158`, PR #783 as `4578728f`, PR #784 as `742d354c`, PR #785 as `36c808d5`, PR #786 as `78202bdf`, PR #787 as `603dcd5d`, PR #788 as `c44cade8`, PR #789 as `63ba8f1a`, PR #790 as `2e69ae0b`, PR #791 as `0b156707`, PR #792 as `e2fd1b29`, PR #793 as `40cedf96`, PR #794 as `7bb0f3e2`, PR #795 as `df17bb2f`, PR #796 as `6d9ffa53`, and PR #797 as `04ade063`; PR #772 was closed unmerged after PR #775 superseded it with a rewritten clean history for the memory-ops v5 spec and per-PR plans, PR #778 was closed after PR #780 superseded the roadmap sync, and follow-up PR #792 replaced the post-merge metrics sparkline branch state after PR #790 was already merged.
> Coordination note addendum 5: PR #798 merged as `5d138c0f`, PR #799 as `368da836`, PR #800 as `c1f28d8e`, PR #801 as `d64f3ba3`, PR #802 as `253a77db`, PR #803 as `234da9e4`, PR #804 as `a4ca2538`, PR #805 as `d3ce9c2c`, PR #806 as `f99d42a8`, PR #807 as `7d8f2594`, PR #808 as `dd9af010`, PR #809 as `da4674c6`, PR #810 as `67deaf39`, PR #811 as `d64ae2f2`, PR #812 as `b58a798f`, PR #813 as `63792a8d`, PR #814 as `f80abb05`, PR #815 as `b553868b`, PR #816 as `3ed7c4c`, PR #818 as `13e3fa2a`, PR #817 as `1b4b9854`, PR #819 as `644ac311`, PR #820 as `01d71b42`, PR #821 as `5444e283`, PR #822 as `42635458`, PR #823 as `df12baea`, PR #824 as `37a552f6`, PR #825 as `b36171aa`, PR #826 as `595822a4`, PR #827 as `7bc1a639`, PR #828 as `751fb236`, PR #829 as `6ba2a622`, and PR #830 as `d9f49985`.
> Coordination note addendum 6: PR #831 merged as `5729a74f`, PR #832 as `122e47cf`, PR #833 as `3eca0152`, PR #834 as `04158328`, PR #836 as `1f7e4649`, PR #835 as `4bc82924`, PR #837 as `d3c2228f`, PR #840 as `350517dc`, PR #839 as `4b209322`, PR #841 as `18abf0df`, PR #842 as `7d13de12`, PR #845 as `e12f82bc`, PR #846 as `966ca34d`, PR #847 as `8ccff1de`, PR #848 as `8526fabe`, PR #849 as `f304e40b`, PR #850 as `53d4c550`, PR #851 as `83e4e1e3`, and PR #852 as `bf0bf676`.
> Coordination note addendum 7: PR #840 added entity canonicalization/extraction benchmark groundwork, PR #839 gated dashboard quick-action accessibility coverage in focused Web E2E CI, PR #841 added durable Memory Import API job state plus resume UI/API, PR #842 closed the post-#841 direct-resume bug where restart-orphaned `running` import jobs were not reconciled before `/api/memory/import/:id/resume`, PR #845 made Memory Import resume progress durable at the per-unit cursor level for JSONL and `claude-mem` imports, PR #846 synced the roadmap/plan records after PR #845, PR #847 closed the structured `claude-mem` `facts[]` graph gap by creating child facts and child-to-parent `summarizes` edges, PR #848 recorded that post-merge state in the roadmap, PR #849 closed the source-local `claude-mem` import-id collision/preview over-count follow-up, PR #850 was the docs-only checkpoint sync through `main@f304e40b`, PR #851 added aggregate-only private eval fixture validation, and PR #852 landed the first production local-only entity canonicalization schema/store slice.
>
> Current follow-up note: the remaining MemPalace work is now narrower than the older PR #716-#723 checkpoint implied. Drawer-aware fusion hardening, fact-detail provenance reads, eval split gating/reporting, the script-only Surface A dry-run bridge, PR #736's bounded `fact-plus-snippet` injection mode, PR #737's paged API source for Surface A dry runs, PR #738's store-level merge-provenance primitive, PR #748's API-level near-duplicate merge wiring, PR #796's workflow-enforced failure-mode coverage floor, PR #801's legacy no-provenance compatibility guard, PR #802's fixture coverage surfacing, PR #803's mobile/session-agent evidence detail hardening, Memory Operations PR C's Settings provider UI plus saved-provider persistence/hardening (PR #806-#808), Memory Operations PR D's backend job-orchestration shell (PR #812), Memory Operations PR E's backfill workers (PR #816), Memory Operations PR F's operations page/alerts/egress flow (PR #820), Memory Operations PR G consolidation/synthesis workers and coverage (PR #827), PR #828's first true `full-drawer` injection mode, PR #830's additive evidence budget accounting for `fact-plus-snippet`/`full-drawer`, PR #832's Surface A write-gated approval flow, PR #833's private eval changelog gate, PR #836's JSONL import job provenance and rollback UI/API, PR #835's local private fixture/changelog secret provisioning preflight and stdin-based `gh` rotation tooling, PR #840's pure entity canonicalization helper and read-only extraction benchmark, PR #841's durable Memory Import job state/resume path, PR #842's direct-resume orphan reconciliation, PR #845's `memory_import_jobs.cursor_json` resume cursor plus same-job JSONL crash-before-checkpoint accounting, PR #847's `claude-mem` `facts[]` child-fact import plus child-to-parent `summarizes` edge creation, PR #849's source-keyed `claude-mem` import ids for source-local idempotency, PR #851's private fixture guard utility, PR #852's local-only canonical entity schema/store slice, PR #818's exact-hit TypeScript build-cache hardening, PR #822/#823/#825's Docker scanner/runtime refresh and fixed-CVE reporting policy, PR #839's dashboard quick-action accessibility gate, and the PR #815 public sample failure-mode fixture rows are covered. The drawer and temporal-edge mesh precondition is now explicit in [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). The next honest gaps remain operator application of the validated eval secrets with richer private failure-mode-targeted held-out/full examples, broader Memory Browser evidence filters/default-on rollout criteria, raw-source filters, fuller why-this-matched explanations, Diary/Timeline result treatment, embedding-similarity dedup, entity canonicalization backfill/apply and runtime integration beyond PR #852's local-only schema/store, temporal edge fields/timeline API/as_of validity-window behavior, Gate 2/live rollout verification for memory operations, and dev-1 backfill verification once the rollout env is intentionally enabled. Separately, the latest runtime/control-plane reliability gaps closed by PR #740-#743 and focused Web E2E gates closed by PRs #762/#765-#769/#839 should be preserved in the next beta promotion rather than reworked in parallel branches.
...
**7,320+ unit tests** across 113+ files + **220+ Playwright e2e checks**, with the backend-independent approvals, notification-bell approvals, webhooks, webhook-deliveries, agent-profiles, agent-settings, audit, machines, machine terminal, agents list, agent detail, emergency stop, scheduler, settings, settings-push-devices, router config, deployment, discover, conflicts, logs-security-findings, security-findings, session-detail, spaces-tasks detail, tasks/spaces, task auto-decompose, memory-dashboard, session-config, memory-index, memory-reports, memory-graph, memory-import, and memory-scopes slices now gated in CI (32 spec files / 118 executed tests in latest CI). The live two-node mesh fixture covers version drift, A-to-B machine visibility, peer-update dry-run, and schema-ahead rejection behind opt-in gates outside the default lane. The scheduler jobs page is reachable from the web sidebar and covered by focused mocked-query unit tests. All packages build and lint cleanly (TypeScript 0 errors, Biome 0 errors).
...
> that import shared contracts. This follow-up adds the already-backend-independent
> `/settings` notification-preferences suite to the same focused Web E2E gate.
> The current Docker-action runtime follow-up
...
> configuration. PR #762 then moved the existing backend-independent
> `/spaces/[id]` and `/tasks/[id]` detail coverage into the focused Web E2E CI
> gate and scoped Security Audit container-scan caches per image to avoid
> cross-image cache contention. PRs #765-#769 then expanded that focused gate
> with agent detail, emergency stop, tasks/spaces, task auto-decompose, and
> machine terminal coverage; PR #771 added memory import and memory scopes,
> bringing that CI lane to 32 spec files / 118 executed tests, and PR #839
> later added dashboard quick-action accessibility coverage to the focused
> lane. PR #822
> refreshed production Docker apt layers plus the agent-worker libngtcp2 runtime
> package/cache-bust path after post-#821 container scans emitted high
...
> post-#849 main CI, Security Audit, and Docker publish passed at `f304e40b`.
...
- [x] Backend-independent web Playwright CI gating for `/webhooks`, `/agent-profiles`, `/audit`, `/machines`, `/machines/[id]/terminal`, `/agents` list/detail/settings, emergency stop, `/scheduler`, `/settings`, deployment, discover, conflicts, logs-security-findings, security-findings, session-detail, spaces-tasks-detail, tasks-spaces, task-auto-decompose, memory-dashboard, session-config, memory-index, memory-reports, memory-graph, memory-import, and memory-scopes (32 spec files / 118 executed tests in latest CI) using `WEB_PORT`-aware web scripts and a shared-package build preflight; runtime-sessions removed from CI due to WebSocket timing flakiness *(PRs #445, #505, #513, #516, #518, #533, #546, #608, #657, #662, #665, #700, #762, #765, #766, #767, #768, #769, #771)*
...
- [x] Production Docker apt layers, agent-worker libngtcp2 runtime package/cache-bust path, production Node runtime image pins, and Docker publish Grype fixed-CVE reporting policy refreshed after post-#821/post-#822 scanner annotations; dev and beta deployment workflows remain untouched *(PRs #822-#823, #825)*
...
- [x] Migration-check destructive-operation comments quote multiline SQL safely in GitHub Script so Markdown/backtick payloads cannot break the workflow *(PR #787)*
...
- [x] Web `/scheduler` jobs page with sidebar entry, list/empty/error/not-configured states, heartbeat job creation, per-agent delete confirmation, confirmed remove-all action over the existing bulk deletion API, API-client wiring, and focused unit/browser coverage *(PRs #517, #760; see [plans/2026-04-15-scheduler-jobs-page-plan.md](plans/2026-04-15-scheduler-jobs-page-plan.md))*
...
- [x] Agent detail loop control surface with start/stop buttons, loop status rendering, and default `LoopConfig` request payloads *(PRs #785, #791)*
...
- [x] Discover/Codex session parity: bounded JSONL discovery, `.codex` content preview lookup, `CODEX_HOME` path resolution, worktree-derived project-path matching, and tail-boundary preservation *(PRs #740, #742)*
...
> MemPalace-inspired evolution plan v1.3: [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md) plus supplements [plans/2026-04-15-mempalace-additional-findings.md](plans/2026-04-15-mempalace-additional-findings.md), [plans/2026-04-16-mempalace-additional-findings-round-3.md](plans/2026-04-16-mempalace-additional-findings-round-3.md), and the sync precondition contract [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md)
>
...
- [x] Phase 0 eval harness foundation: fixture schema/sanitization, deterministic seed-42 split helpers, mock scoring for R@5/R@10/MRR/NDCG@10/grounding/drawer-hit/p95, by-category/by-tag summaries, sanitized public sample fixture, and `pnpm memory:eval` CLI *(PR #655)*.
- [x] Phase 1 drawer schema/chunker/sanitizer/store foundation: `0030_add_memory_drawers.sql`, no `wing`/`room` hierarchy, non-unique content hashes, shared memory redaction/validation constants, raw-secret redaction before hash/embed/store, and no drawer sync triggers or search default changes *(PR #671)*.
- [x] Phase 1 memory write audit foundation: shared `memory_write` audit entry builder, hash-chain logger/reporter support, and `MemoryDrawerStore` success/failure audit emission with raw-content key stripping and sensitive metadata redaction *(PR #679)*.
- [x] Persist drawer backfill state: `0032_add_memory_drawer_backfill_state.sql`, Drizzle schema/journal/schema tests, shared `MemoryDrawerBackfill*` contracts, and `MemoryDrawerBackfillStateStore` coverage for start/resume, cursor update, and safe failed-state errors *(PR #682)*.
- [x] Add resumable JSONL drawer backfill CLI on top of the persisted state foundation: dry-run default, recursive JSONL discovery, streaming line parsing, cursor resume, execute mode through `MemoryDrawerStore`, safe parse-error metadata, and text/JSON summaries *(PR #686)*.
- [x] Extend drawer backfill to `claude-mem` narrative/fact mapping, `memory_fact_sources` write links, and dry-run cost/storage estimates *(PRs #692, #699, #703, #708, #721)*.
- [x] Finish drawer backfill batching/backoff and embedding generation/backfill for CLI-written facts *(current `main` follow-up after PRs #721/#722/#726: execute-mode backfills now batch/retry embedding calls, generate embeddings for newly written atomic facts, and backfill embeddings onto existing CLI-written facts during idempotent resumes).*
- [x] Add deterministic planted `NEEDLE_` recall regression bench over mock ranking/scoring with env-configurable needle/noise counts, recall@5 threshold enforcement, p50/p95/p99 latency reporting, and `pnpm memory:bench`.
- [x] Wire `pnpm memory:eval --no-mock` to live control-plane `MemorySearch` using `DATABASE_URL` plus embedding config, while preserving mock-by-default runs, dev/held-out/full split behavior, and the full-set guard *(PR #722)*.
- [x] Enrich the human-readable `pnpm memory:eval` report with By Tag summaries plus capped failure examples while keeping `--json` output unchanged *(current `main`)*.
- [ ] Extend eval honesty beyond live wiring with full private fixture coverage, release/weekly held-out gates, and no held-out tuning outside release/weekly eval jobs. Current `main` already gates `held-out/full` execution behind workflow-owned env flags and documents the private-fixture delivery contract, and PR #729 adds `.github/workflows/memory-evals.yml` to run weekly `held-out`, release-tag `full`, and manual dispatch against gitignored staged fixtures under `tmp/memory-eval/`. PR #796 makes private fixture workflow runs enforce the default failure-mode coverage floor through `MEMORY_EVAL_REQUIRE_FAILURE_MODE_COVERAGE=true` (with optional `MEMORY_EVAL_FAILURE_MODE_MIN_ROWS`), PR #802 surfaces required-tag coverage counts in CLI/report output while preserving workflow summaries/artifacts when evals fail, PR #815 gives the public sample fixture one sanitized, focused row for each default failure-mode tag plus a refreshed facts-only baseline, and PR #833 makes private eval runs require a dated fixture changelog via `MEMORY_EVAL_REQUIRE_FIXTURE_CHANGELOG=true` plus `--fixture-changelog`. The remaining work narrows to provisioning/rotating the repo secrets and richer private held-out/full examples.
- [x] Lock first-run behavior for current worker memory MCP routes: empty search/recall/report contracts and `{ arguments: null }` fail-fast tests so bad calls return structured 400s instead of hanging.
- [x] Add first-run contract for `memory_dedup_check`: empty DB recommends `store_new` with no nearest matches, invalid/null arguments fail fast, and candidate-present calls return an explicit scoring-unavailable response until the full pre-store scorer lands *(PR #681)*.
- [x] Add first-run worker contract for `memory_traverse`: missing/no-data responses return an empty graph, null/invalid arguments fail fast, hop caps are validated, and relation/min-confidence filters proxy to the planned control-plane traversal endpoint *(PR #685)*.
- [x] Add `memory_fact_sources` / `embedding_version` schema groundwork with migration `0031`, source offsets, fact/edge embedding-version defaults, Drizzle journal/schema updates, and legacy-compatible schema tests *(PR #684)*.
- [x] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734/#735/#736/#737: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator, PR #735 fixes its Claude project memory target path, PR #736 adds bounded default-off PostgreSQL `fact-plus-snippet` injection, and PR #737 adds a paged control-plane API fact source for the Surface A dry run. PR #738 added store-level `memory_fact_sources` copying/deduping so duplicate-fact merges can preserve provenance, PR #748 wires accepted near-duplicate consolidation actions into that helper before invalidating duplicate facts, PR #801 keeps legacy fact-only/no-provenance search and detail responses stable when preview lookup is unavailable, PR #828 adds true PostgreSQL `full-drawer` injection for on-demand facts with source-drawer hydration plus global/on-demand tier caps, PR #830 applies additive global/per-tier evidence token accounting to `fact-plus-snippet` and `full-drawer` rendering, and PR #832 promotes the Surface A dry-run bridge into a write-gated approval flow with deterministic proposal tokens plus durable `/api/approvals` gate verification.
- [x] Add the three-stage query sanitizer before worker proxying and control-plane embedding/search so prompt-contaminated memory queries are narrowed before retrieval *(PR #677)*.
- [x] Add the optional `InjectionBudget.tierTokenCaps` contract and context-budget enforcement so callers can cap pinned/on-demand/triggered tiers independently without changing defaults *(PR #720)*.
- [x] Surface fact-side match-source badges from `results[].source_path` and render a compact raw-drawer results section below Memory Browser facts when `/api/memory/facts` returns non-empty `drawerResults` *(PRs #719, #723)*.
- [ ] Extend retrieval only after eval/backfill with rank-bucket boosts, drawer-aware search, drawer-grep enrichment, source-neighbor hydration, capped additive boosts, MCP drawer get/search parity, full `memory_dedup_check` skip/merge scoring, richer result modes beyond the bounded `fact-plus-snippet` / first `full-drawer` slices, and additive accounting for future source-neighbor hydration beyond current evidence rendering.
- [x] Add worker-side nonblocking PreCompact checkpoint capture with explicit hook/API timeout semantics and diary-as-fact continuity so long sessions keep context without blocking compaction *(PR #776; PR #784 keeps the hook under the timeout budget and lets stdout flush; PR #800 defers capture onto the next tick so the route returns `202` before capture starts; follow-up: automatic per-session hook registration)*.
- [ ] Extend memory edges with temporal validity/source references and `memory_traverse` only after reviewing mesh `sync_capture_change()` payload compatibility, schema-version gates, and entity canonicalization/backfill requirements; PR #840 adds the first pure helper and extraction benchmark but no schema/backfill/apply path yet.

### 3.7 Memory Operations UI — P0 (In Progress)

> Design spec v5: [specs/2026-04-24-memory-operations-ui-design-v5.md](superpowers/specs/2026-04-24-memory-operations-ui-design-v5.md)
> Implementation plans: [plans/2026-04-24-memory-operations-ui/index.md](superpowers/plans/2026-04-24-memory-operations-ui/index.md)
>
> Root problem: 19,226 `memory_facts` rows with `embedding IS NULL` — no UI path to configure an embedding provider or trigger backfill. This feature closes that gap end-to-end.

Embedding provider CRUD, durable BullMQ-backed maintenance jobs, and a `/memory/operations` page — all gated behind `MEMORY_OPS_ENABLED` env flag with a phased env-var unlock schedule across 7 sequential PRs.

**7-PR implementation plan (merge in order A → B → C → D → E → F → G):**

- [x] **PR A** — DB migration `0033` (api_accounts extensions + 3 new tables), Drizzle schema, shared types (`providers.ts`, `ops.ts`, `ops-audit.ts`), EmbeddingClient additive extensions, `credential_kind='runtime'` filter on 6 routes. Gate 1 contract test. *(PR #783)*
- [x] **PR B** — `resolveEmbeddingClient()` factory with 60s TTL cache + provider-invalidation bus, `memory-providers` route (CRUD + switch-mode + hardened test-before-save token), audit logger, memory read/write rewiring to use factory, `MEMORY_OPS_STATUS_MAP` in `controlPlaneErrorToStatus()`, and LITELLM embedding fallback removal. *(PR #797)*
- [x] **PR C** — Settings UI: `ApiError.details` extension, `memoryProvidersApi` web client, `ProviderDialog` (test-before-save, verified-only catalog), `MemoryEmbeddingsSection` in Settings, successful DELETE 204 handling, persisted saved-provider test metadata for success/failure rows, safe failure summaries, credential-guarded status writes, stale-write 409s, and post-refetch UI status cleanup. *(PR #806; persistence follow-up PR #807; hardening follow-up PR #808)*
- [x] **PR D** — Job infrastructure: `memory/ops/config.ts`, BullMQ queue singleton, `JobsRepository` (advisory lock + fleet check + durable cancel), `JobEventsRepository`, SSE stream with pg_notify + replay, preview endpoint + capabilities route, startup worker runtime, and retention for local memory-ops event/audit tables. `MEMORY_OPS_ENABLED_KINDS=` empty. *(PR #812; CI, Security Audit, Docker publish, container scans, focused local tests/build, and independent CodeQL alert check passed)*
- [x] **PR E** — Workers: `embedding-backfill` (batch loop, cancel polling, fail-ratio guard), `drawer-backfill` (sourceRoot containment, symlink escape detection), BullMQ Worker + boot reconciliation, integration test, local cost tracking, and `MEMORY_OPS_ENABLED_KINDS=embedding-backfill,drawer-backfill`. *(PR #816)*
- [x] **PR F** — `/memory/operations` page: `memoryOpsApi` web client/query helpers, `MissingEmbeddingAlert` across the memory surfaces, `JobCard`, `RecentJobsTable`, `JobDetailDrawer` with local SSE stream support, `EgressConfirmationDialog` (preview → confirm flow), `MixedModelsBanner`, `MemoryOperationsPage` orchestrator, sidebar Operations nav item, and `MEMORY_OPS_ENABLED=true`. *(PR #820; branch: `codex/memory-ops-pr-f`; checks passed)*
- [x] **PR G** — Consolidation + synthesis workers, focused unit coverage, Memory Ops Playwright coverage for the OpenAI mocked provider flow, missing-provider alerts, and gated Gemini dimensionality smoke, plus Gate 2 rollout notes in CHANGELOG/QUICKSTART and `MEMORY_OPS_ENABLED_KINDS` expansion for consolidation + synthesis. *(PR #827)*

**Key design decisions:**
- `api_accounts` is local-only (never mesh-synced); embedding provider rows stay per-machine
- `memory_ops_jobs` is mesh-synced mutable; `memory_ops_job_events`/`memory_ops_audit` are local-only
- BullMQ `queue.add(kind, { dbJobId }, { jobId: dbJobId })` — jobId in opts (3rd arg) for boot recovery
- Durable cancel via `cancel_requested_at timestamptz` + `status='cancelling'` — no complete-after-cancel race
- Preview endpoint returns `{ snapshot, egressToken }` — user confirms with the signed token to avoid circular dependency
- Gemini `verified: false` in catalog until Gate 2 confirms dim=1536 + `output_dimensionality` honored
- Fleet backfill race unmitigated in v1 — operator guidance documented in plan
...
- [x] Loading spinners added to CreateSessionForm, AgentFormDialog, ConvertToAgentForm, and RuntimeSessionsPage form buttons; `aria-live` regions added to SessionsPage and AgentsPage skeleton loaders *(PR #611)*
- [x] ErrorBoundary wrapped around 11 router pages: 9 memory sub-pages (`/memory/browser`, `/memory/graph`, `/memory/entities`, `/memory/synthesis`, `/memory/search`, `/memory/timeline`, `/memory/stats`, `/memory/maintenance`, `/memory/consolidation`) plus `/tasks` and `/settings/router` *(PR #616)*
- [x] ErrorBoundary wrapped around remaining 3 detail pages: `sessions/[id]`, `machines/[id]`, and `machines/[id]/terminal` — completes ErrorBoundary coverage for all web pages *(PR #629)*
...
- [x] Replace remaining hardcoded `bg-[#3b82f6]`/`hover:bg-[#2563eb]` inline buttons with `bg-primary`/`hover:bg-primary/90` tokens across WebhooksPage, MeshPeersPage, DiscoverPeersDialog, AgentProfilesPage, SchedulerPage; replace `bg-[#0a0a0a]` terminal backgrounds with `bg-[var(--color-terminal-bg)]` in InteractiveTerminal, TerminalView, TakeoverPanel, PeerUpdateLogModal; delete unused `MemoryPlaceholderView.tsx` *(PR #645)*
...
>
> Reality checkpoint (2026-04-25): the memory route family and core PostgreSQL
> facts APIs exist, but earlier roadmap wording overstated several design-spec
> items as fully delivered. The current product should be treated as a usable
> foundation, not a complete memory operations system: graph quality depends on
> explicit `memory_edges` and imported observations currently arrive mostly as
> isolated facts; reports are deterministic SQL summaries rather than
> LLM-generated narratives; JSONL history import now has a backend/frontend path
> with duplicate re-import prevention, aligned preview/import counts, file-count
> overflow errors, bounded fd-based reads, and visible import-start failures; and
> rich consolidation/graph modes from the original design remain follow-up work.
...
- [x] Knowledge Graph (`/memory/graph`) — basic Graph/Table shell over `GET /api/memory/graph`; the table now remains useful for isolated imported facts with no edges.
- [ ] Knowledge Graph completeness — generate/curate useful `memory_edges` during import and maintenance, and finish the original Timeline/Clusters/focus/time-lapse affordances from the design spec.
...
- [x] Consolidation Board (`/memory/consolidation`) — basic review queue and accept/skip/delete actions over consolidation items, with editable resolution text in the accept flow *(PR #786)*.
- [ ] Consolidation completeness — restore richer contradiction/near-duplicate merge workflows and clearer post-action state instead of treating the current stale-fact queue as the full design.
- [x] Reports (`/memory/reports`) — 3 deterministic report types (Project Progress, Knowledge Health, Activity Digest); scope + time range selector; rendered markdown with download/copy; generation success/failure Playwright coverage *(PR #53, PR #415)*
- [ ] LLM narrative reports — if still desired, add an explicit model-backed report path with audit/cost controls; the current backend intentionally uses SQL aggregation and in-memory report caching.
- [x] Import Wizard (`/memory/import`) — 4-step claude-mem migration wizard (source detection → preview/mapping → progress → summary).
- [x] Basic JSONL history import — backend preview/import routes plus web source selection and happy-path E2E/unit coverage *(PR #788)*.
- [x] JSONL import hardening — duplicate re-import prevention via `import_source_id`, preview/import count alignment, 1000-file overflow errors, bounded fd-based reads, visible import-start failures, and regression coverage *(PR #794)*.
- [ ] Import completeness — durable job state plus restart-visible resume/rollback UX is covered through PR #841 and PR #842; PR #845 adds a persisted JSONL resume cursor plus same-job crash-before-checkpoint accounting so file churn and stale checkpoints no longer shift JSONL resume work into skipped rows. PR #847 splits imported `claude-mem` `facts[]` entries into child facts and creates child-to-parent `summarizes` edges. PR #849 qualifies `claude-mem` observation and structured child-fact import ids by source key so overlapping row ids from different local databases do not collide; embedding-similarity dedup remains follow-up work.
- [x] Real claude-mem SQLite import backend — reads claude-mem.db directly, deduplicates against existing facts, provides preview before commit *(PR #630)*
...
- [x] Synthesis Page (`/memory/synthesis`) — read-only structural-lint surface for near-duplicates, stale facts, orphan facts, and entity-type grouping over `POST /api/memory/synthesis`; this is not an LLM synthesis engine *(PR #470)*.
- [x] Synthesis quality — provide human-readable principle candidates, richer examples, and optional model-backed synthesis before calling this feature complete. *(PR #774 — principle candidates tab with accept/dismiss delivered)*
...
- [x] Main Dashboard: live metrics sparklines with stable SVG gradient IDs *(PRs #790, #792)*
...
### 5.4 Mobile API Contract Hygiene — P2 ✅

- [x] Mobile `AuditSummary` client type and focused API-client tests now match the post-#743 backend/web contract (`toolBreakdown`, `actionTypeBreakdown`, `avgDurationMs`) *(PR #745)*
...
- [x] Web `/audit` trail page with filters, empty/error/loading states, focused unit coverage, backend-independent Playwright coverage in the web e2e CI lane, and a backend summary contract that returns the frontend-consumed `toolBreakdown`, `actionTypeBreakdown`, and `avgDurationMs` shape *(PRs #507, #513, #743; see [plans/2026-04-14-audit-trail-page-plan.md](plans/2026-04-14-audit-trail-page-plan.md))*
- [x] Audit session replay and suspicious-pattern detection surface on `/audit`, including mocked Web E2E coverage for the new `/api/audit/suspicious` read path *(PR #789)*
- [x] Logs UI Security Findings tab over `GET /api/security/findings`, with focused API/query/view tests and explicit control-plane route registration *(PR #416; Playwright coverage for happy-path rows/summary badges, empty state, severity filter, and error banner in PR #436; route registration in PR #741)*
...
- [x] Web task-graph create/delete controls over the existing graph APIs *(PR #779)*
...
- [x] Space detail ContextRefsPanel with add/delete UI for context bridge references *(PR #782)*
...
- [x] Backend-independent `/spaces/[id]` and `/tasks/[id]` detail-route coverage for page render, 404 paths, space thread/event/member actions, task run start, and back navigation; the detail spec also runs in the focused Web E2E CI lane *(PRs #468, #762)*
- [x] Backend-independent `/memory/maintenance` and `/tasks/[id]` auto-decompose Playwright coverage, including scope-aware maintenance POST bodies, clean-memory copy, error retry, two-step preview/apply dialog flow, the stale-preview guard, and auto-decompose apply-failure retry posture; the auto-decompose spec also runs in the focused Web E2E CI lane *(PRs #478, #487, #768)*
- [x] Backend-independent `/agents/[id]` detail-route coverage for agent metadata/config/cost/session/memory/run-history render plus `POST /api/agents/:id/start` prompt wiring; the spec also runs in the focused Web E2E CI lane *(PRs #492, #765; see [plans/2026-04-14-agent-detail-e2e-plan.md](plans/2026-04-14-agent-detail-e2e-plan.md))*
...
- [x] Backend-independent emergency-stop, tasks/spaces, and machine-terminal Playwright specs run in the focused Web E2E CI lane, keeping the kill-switch UI, task/space workflows, and terminal route covered before merge *(PRs #766, #767, #769)*
...
### 33.7 Mesh Peer UX Overhaul (P1) — Delivered
...
2. Once a peer row exists, the UI offers only Ping and Delete — there is no Edit. Fixing the scheme above required a direct `POST /api/sync/peers` upsert from the CLI; deleting and re-adding would reset row-local status/config metadata such as `lastSeen` and `syncIntervalMs`, and makes cursor continuity harder to reason about even though the cursor table is not currently FK-cascaded from `sync_nodes`.
3. Adding a peer is a 6-field form (`machineId`, `hostname`, `tailscaleIp`, `syncUrl`, `syncInterval`, `publicKey`). The identity, address, URL, and key fields can be derived from Tailscale plus a single `/health` probe; `syncIntervalMs` remains an operator policy with a sensible default and an edit affordance.

**Scope:**

- [x] **Persist and return ping failure reason in the API.** `POST /api/sync/peers/:machineId/ping` now classifies failures, stores `last_ping_error` + `last_ping_status_code` on `sync_nodes`, and returns the diagnostics with peer rows *(PR #550)*.
- [x] **Render ping failure details in the UI.** `/mesh-peers` now shows the persisted failure category + HTTP status next to the STATUS pill, and the ping toast surfaces the specific failure reason *(PR #582)*.
- [x] **Edit existing peer.** `/mesh-peers` now has a per-row Edit action that opens the peer dialog pre-populated, posts through the existing idempotent `POST /api/sync/peers` path, and keeps self-peer rows protected *(PR #551)*.
- [x] **Tailscale discovery + probe endpoints.** `GET /api/sync/peers/discover` shells out to `tailscale status --json`, filters `tag:mesh-node` peers, probes each `http://<tailscaleIp>:8080/health`, and returns `{ machineId, hostname, tailscaleIp, syncUrl, nodePublicKey, reachable }[]` excluding already-registered peers. `GET /api/sync/peers/probe?target=…` accepts a bare hostname, IP, or URL, SSRF-validates via the same block list as `validateSyncUrl` (`deriveSyncUrlFromTarget`), and returns probe results with identity/version metadata. Both are rate-limited and auth-gated. Core helpers (`parseTailscalePeers`, `deriveSyncUrlFromTarget`, `probePeerHealth`) extracted to `sync/peer-discovery.ts` with full unit coverage *(PR #586)*.
- [x] **"Discover peers" UI flow.** "Discover Peers" button opens a `DiscoverPeersDialog` with a two-step flow: step 1 lists Tailscale candidates with checkboxes, reachability badges, and identity/version info; step 2 previews the selection with a bulk `syncIntervalMs` input before sequential upsert. "Add manually" link preserved for air-gapped cases. 10 unit tests *(PR #590)*.
- [x] **Auto-fill by hostname or IP (probe).** "Probe" button in the manual add/edit form calls `GET /api/sync/peers/probe?target=…` and auto-fills machineId, hostname, syncUrl, and publicKey from the probe identity. Loading/success/error states displayed inline *(PR #590)*.
- [x] **Default scheme + inline hint.** Sync URL field pre-fills `http://` on focus when empty and shows "Tailscale already encrypts — HTTPS is only needed for public endpoints" helper text *(PR #590)*.
- [x] **Playwright coverage.** 10 new E2E tests added to `mesh-peers.spec.ts`: ping failure category badge + diagnostic line, HTTP status prefix, ping state transitions, hidden diagnostics for healthy peers, MeshHealthSummary counts, one-way badge + reverse retry, update-available banner, hidden states. 2 pre-existing add-peer tests fixed for probe gate. All 21 tests pass *(PR #591)*.

**Depends on:** 33.4 (peer registry), 33.6 (ACL tagging). Contributes to 16.1 observability (surfacing mesh ping diagnostics).

### 33.8 Mesh Bidirectional Registration + Cross-Node Visibility (P0) — Delivered
...
- [x] **Authenticated reverse-registration endpoint.** `POST /api/sync/peers/register` now rate-limits before bootstrap auth, requires `SYNC_PEER_REGISTRATION_TOKEN` / `X-Sync-Registration-Token`, validates a signed `register-peer` envelope, and upserts the reverse `sync_nodes` row with `sync_status = 'unknown'` *(PR #552)*.
- [x] **Auto-call reverse registration during peer add.** `POST /api/sync/peers` now calls the target's `/api/sync/peers/register` with the existing signed envelope path, persists `reverse_registration_status` / `reverse_registration_error` / `reverse_registration_at`, and keeps the local insert even when the target is old or unreachable *(PR #564)*.
- [x] **Auto-register self on peer-add.** Failed handshakes now store a redacted/truncated failure reason and surface one-way state in the API/UI instead of silently looking healthy *(PR #564)*.
- [x] **"Register this CP with peer" action.** `/mesh-peers` now shows a compact "One-way" badge plus Retry action for failed peers; the retry calls `POST /api/sync/peers/:peerId/register-reverse` and refreshes the peer list after completion *(PR #564)*.
- [x] **Machines / workers cross-visibility.** Integration test `two-node-replication.integration.test.ts` proves end-to-end: INSERT on node A → `sync_capture_change` trigger fires → change log entry → `applyChange()` on node B → machine row appears with correct `origin_node_id`. Also covers UPDATE propagation, stale-update rejection (vclock dominance), and apply-guard (no trigger re-fire on remote apply). 5 tests, requires `DATABASE_URL` *(PR #589)*.
- [x] **Fleet view cross-node badge.** `machines.origin_node_id` now records remote machine provenance, sync apply stamps missing remote provenance from the source node, `listMachines`/`getMachine` resolve the origin hostname from `sync_nodes`, and `/machines` badges rows as local vs synced from peer *(PR #554)*.
- [x] **Mesh health panel.** `/mesh-peers` now renders a `MeshHealthSummary` above the table (`N peers · bidirectional · M one-way · K stale (>10 min)`) and row-click expands a `PeerCursorDetail` panel that fetches `GET /api/sync/peers/:peerId/cursors` for `lastPullAt`/`lastAckAt` from `sync_peer_cursors`. `summarizeMeshHealth()` lives in `packages/shared/src/mesh/health.ts` *(PR #576)*.
- [x] **Playwright coverage.** PR #654 added the opt-in live two-node fixture foundation and first 33.11 version-drift assertion. PR #673 added the separately gated 33.8 browser assertion for a primary-authored machine row appearing on the secondary node with its `Synced from ...` provenance badge. PR #675 added the remaining 33.8 browser assertions: add peer on A through the real `/mesh-peers` form and confirm the reverse row on B, plus a browser-only one-way warning/retry failure-path assertion that does not mutate live peer state.

**Post-delivery fix:** PR #596 added `SYNC_PEER_REGISTRATION_TOKEN` to the explicit `env` block in `ecosystem.beta.config.cjs` and `ecosystem.mesh.config.cjs`. PM2's explicit env overrides inherited `process.env`, so the token was invisible to the CP, causing reverse-registration to fail with `PEER_REGISTRATION_DISABLED` ("one-way" peering) on the live fleet despite the feature being correctly implemented at the application layer.
...
### 33.9 Mesh Version Observability (P1) — Delivered

**Plan:** [plans/2026-04-15-mesh-version-observability-plan.md](plans/2026-04-15-mesh-version-observability-plan.md)
...
- [x] **Expose `appVersion` + `gitSha` + `schemaVersion` on `/health`.** `appVersion` comes from the control-plane package metadata, `gitSha` comes from `GIT_SHA` / `GITHUB_SHA` with an `unknown` fallback, and `schemaVersion` is the highest shipped migration numeric prefix, not a database-applied ledger read *(PR #556)*.
- [x] **Schema groundwork for peer version persistence.** Add nullable `peer_version`, `peer_git_sha`, and `peer_schema_version` columns to `sync_nodes`, mirror them in Drizzle, and expose optional nullable fields in the shared `SyncNode` type *(PR #555; migration renumbered to `0025_sync_nodes_peer_version.sql` in the post-merge docs/hotfix sync)*.
- [x] **Persist peer version on ping.** `pingPeer()` and the background peer-health loop now capture nullable `appVersion`, `gitSha`, and `schemaVersion` from `/health` and store them alongside `sync_status`/`last_seen` while keeping old peers compatible *(PR #561)*.
- [x] **Render version per row on `/mesh-peers`.** New VERSION column renders persisted peer version metadata with behind/ahead/unknown indicators *(PR #558)*.
- [x] **Mesh-wide drift banner.** Mixed `appVersion` groups now render an operator-visible drift banner with counts by version *(PR #558)*.
- [x] **Sidebar footer tooltip.** Local-node footer now lazy-loads peer version summaries on focus/hover/touch so drift can be checked from any page without adding a global background `/api/sync/peers` request *(PR #558)*.
- [x] **Tests.** Unit coverage for `/health` shape plus `/mesh-peers` version/drift/sidebar behavior landed in PRs #556/#558; PR #561 adds route and peer-health coverage for persisting version metadata from on-demand and background pings.

**Post-delivery fix:** PR #596 updated `LOCAL_APP_VERSION` from v0.5.0 to v0.5.1 in `packages/web/src/lib/mesh-version.ts` to match the current release, fixing a false "mixed mesh version" drift banner that appeared because the hardcoded local version lagged behind the actual deployed version.
...
**Plan:** [plans/2026-04-15-mesh-schema-protocol-compat-plan.md](plans/2026-04-15-mesh-schema-protocol-compat-plan.md)

**Motivation:** PR #557 added schema/protocol metadata to mesh change-log envelopes and gates apply before database side effects. The remaining risk is operator visibility: during a rolling upgrade, a lagging peer can reject an envelope authored by a schema-ahead producer, but `/mesh-peers` still needs to surface the reason and guide the operator to upgrade the laggard before any automated rollout (33.11) lands.

**Scope:**

- [x] **Tag every envelope.** `change_log.envelope` now serializes `{ schemaVersion, protocolVersion, producerVersion }` metadata without requiring a trigger migration *(PR #557)*.
- [x] **Apply-side compat gate.** `apply-change.ts` now rejects envelopes whose `schemaVersion` is more than +1 newer than the local schema with typed `MESH_ENVELOPE_SCHEMA_AHEAD`, rejects unsupported protocol versions, tolerates legacy envelopes, and keeps older-than-local envelopes compatible *(PR #557)*.
- [x] **"Peer ahead — please update" banner.** Apply-side rejections now persist to `sync_nodes` (`last_schema_ahead_version`, `last_schema_ahead_at`, `schema_ahead_count` via migration 0027) and `/mesh-peers` surfaces a `PeerSchemaAheadBadge` in the Version cell with the rejected schema and retry count *(PRs #567, #573)*.
- [x] **Documented compat window.** `docs/MESH_COMPAT.md` documents the one-schema-version skew policy, legacy tolerance, protocol deprecation window, and operator remediation path *(PR #557)*.
- [x] **Tests.** Unit tests for the apply gate landed in PR #557. Two-node integration proof in `schema-compat.integration.test.ts` covers 6 scenarios: same schema → applies, +1 grace → applies, +2 ahead → rejected `MESH_ENVELOPE_SCHEMA_AHEAD`, -1 older → applies, legacy (no meta) → applies, +3 ahead → rejected. Playwright E2E covers `PeerSchemaAheadBadge` rendering, self-row exclusion, and tooltip singular/plural grammar (3 tests) *(PR #594)*.

**Depends on:** 33.9 (uses same `appVersion`/`schemaVersion` surface). **Prereq for:** 33.11 (auto-update without this gate is unsafe).

### 33.11 Fleet Rollout & Peer Auto-Update (P1, two-topology) — In Progress

**Plan:** [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md)
...
- **Docker topology** (`deploy-fleet.yml`, `docker-compose.prod.yml`, GHCR images) — 874-line workflow already exists with canary/rolling/all-at-once strategies. `infra/machines.yml` now has a structured two-topology layout with placeholder CGNAT IPs for Docker fleet members and `labels.topology` for programmatic filtering (PR #593). Secrets (`DEPLOY_SSH_KEY`, `TS_OAUTH_*`, `POSTGRES_PASSWORD`) are not yet wired.
...
- [x] **Populate `infra/machines.yml`** with two-topology fleet structure: Docker topology section (2 placeholder entries for future EC2/cloud fleet) + PM2 mesh topology section (laptop + macmini with placeholder CGNAT IPs and MagicDNS hostname refs). Added `labels.topology` field for programmatic filtering. `fleet-bootstrap.ts --dry-run` validates all 4 entries *(PR #593)*.
- [x] **Wire build provenance + verification.** `build-images.yml` publishes signed attestations for both GHCR images via `actions/attest-build-provenance` (originally `v2` in PR #575, upgraded to Node 24-backed `v4.1.0` in this follow-up) with `id-token: write` + `attestations: write`; `deploy-fleet.yml` resolves the tag→digest via `docker buildx imagetools inspect` and runs `gh attestation verify oci://...@<digest> --repo $repo` in the `validate` job before any deploy job pulls the image. A `skip_attestation_verification` workflow_dispatch boolean is reserved for incident recovery. Runbook: `docs/FLEET_ATTESTATIONS.md` *(PR #575)*.
- [ ] **End-to-end canary dry-run.** Run `deploy-fleet.yml --dry-run` against the populated inventory, then once for real with `strategy: canary`, `canary_percentage: 20`, against a non-critical target. The preparatory safety slice now keeps `deploy-fleet.yml` and `fleet-bootstrap.ts` scoped to `labels.topology: docker`, documents the exact dry-run/canary invocation sequence, and has focused workflow/bootstrap coverage; the live workflow runs remain to be executed by an operator *(prep PR #678)*.
- [x] **Migration gate reuse.** `deploy-fleet.yml` now invokes `migration-check.yml` via `workflow_call` as a required upstream job before `validate`, blocking destructive migrations (`DROP TABLE`, `DROP COLUMN`, `TRUNCATE TABLE`, `DROP CONSTRAINT`, `ALTER COLUMN ... TYPE`) from rolling out unless the operator re-runs with `allow_destructive_migrations=true`; every destructive statement is logged to the step summary for audit *(PR #579)*.

**Scope — PM2 mesh topology (new):**

- [x] **Manual PM2 peer-update script + signed self-update route.** PR #563 added `scripts/peer-update.sh`, a signed peer-auth-protected `POST /api/sync/peers/:peerId/update` route, route-local Fastify rate limiting before authorization, mutex protection for overlapping updates, structured update responses, and focused route/UI coverage.
- [x] **`pnpm agentctl peer update` subcommand + full provenance/rollback flow** (sibling to existing `agentctl deploy` in `scripts/deploy.ts`). `scripts/peer-update.ts` implements latest-tag discovery via `gh release view`, git fetch/checkout, migrate, build, PM2 reload, `/health` polling, automatic rollback on failure, and persists a structured history to `~/.agentctl/update-history.json` *(PR #571)*.
- [x] **Opt-in schedulers.** `infra/launchd/com.agentctl.peer-update.plist` (macOS, `Disabled=true`) and `infra/systemd/agentctl-peer-update.service + .timer` (Linux, `OnCalendar=*-*-* 03:00:00` with `Persistent=true`), both disabled by default; `AGENTCTL_UPDATE_WINDOW` documents the update-window override. READMEs in `infra/launchd/` and `infra/systemd/` cover install/enable/customise/disable *(PR #572)*.
- [x] **Settings UI toggle.** `/settings` now mounts a `MeshAutoUpdateSection` (after `WorkersSyncSection`) with a toggle that wires `launchctl bootstrap`/`bootout` (darwin) or `systemctl --user enable/disable --now` (linux) via `POST /api/mesh/auto-update/toggle`, renders next-scheduled-run and last-run summary sourced from `~/.agentctl/update-history.json`, and exposes a "Run dry-run now" button that streams `pnpm agentctl peer update --dry-run` output over SSE from `POST /api/mesh/auto-update/dry-run` *(PR #577)*.
- [x] **Manual "Update peer" action.** `/mesh-peers` now exposes a per-row Update action for peers with a reachable sync URL, signs the request with the same Ed25519 peer-auth path used by sync envelopes, and reports queued/running/success/failure outcomes in the UI *(PR #563)*.
- [x] **Rollback story.** `scripts/peer-update.ts` now supports `--rollback` (auto-target from `~/.agentctl/update-history.json`) and `--tag vX.Y.Z-1 --rollback` (explicit prior target); both refuse to proceed unless the target tag appears as a previously-successful, non-dry-run entry in history, and only those verified-applied rollbacks bypass fresh attestation verification. Forward-rolls via explicit `--tag` still run `gh attestation verify`. Docker topology documented to reuse `rollback.yml` with `skip_attestation_verification` as the pre-33.11-tag escape hatch. Runbook updated in `docs/FLEET_ATTESTATIONS.md` *(PR #580)*.

**Scope — cross-topology:**

- [x] **iOS + web client compat surface.** `/api/version-compat` returns `{ appVersion, gitSha, schemaVersion, minSupportedMobileBuild, minSupportedWebBuild }`; web reads it on bootstrap via `VersionCompatBanner` mounted in the root layout and shows an "update your client" banner on mismatch *(PR #570)*.
- [x] **"Update available" banner in web.** `/mesh-peers` renders a `MeshVersionBanner` above the peer table when any reachable peer reports a newer `appVersion` than the local CP's `VersionCompat.appVersion`; the banner surfaces the highest peer version + a link to `./scripts/peer-update.sh --dry-run` instructions *(PR #582)*.
- [x] **Playwright two-node fixture.** PR #654 added the opt-in live fixture foundation and first version-drift assertion: one live peer ping updates the mesh-peers row and update-available banner with the expected peer version. PR #659 added a separately gated dry-run assertion that streams `pnpm agentctl peer update --dry-run` through `/api/mesh/auto-update/dry-run` and verifies the reported planned steps are marked `dryRun`. PR #668 added the final separately gated schema-ahead assertion: force a `schemaVersion + 2` envelope through the primary node apply gate, persist the 33.10 rejection marker, and assert the schema-ahead badge on `/mesh-peers`.
...
### 33.12 Mesh Peering UX & Reliability Overhaul (P0) — Delivered

**Plan:** [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) (v4)

**Motivation:** Adding a mesh peer requires SSH to set env vars, rebuild, and restart PM2. A 2026-04-16 live fleet exercise exposed that configuration brittleness is the primary failure mode. The mechanical peering flow works; the gaps are: Tailscale IP auto-detection, web-editable local config, structured error codes reaching the frontend, and a dynamic config provider that eliminates restarts.

**Architectural constraints addressed:**
- `settings` table is mesh-synced (`sync.ts:164`, trigger in `0021:177`) — CANNOT store node-local mesh config there (token would leak into `sync_change_log.payload`)
- Self-identity and registration token are frozen at startup (`server.ts:801-808`) — requires `MeshConfigProvider` refactor for restart-free config changes
- Reverse registration errors lose structured codes before reaching frontend (`peer-reverse-registration.ts:144`) — requires backend contract change before frontend mapping

**Scope (5 phases):**

0. **Bug Fixes (P0):** ✅ PR #610 — Fixed `version-bump.sh` missing `git add` for `mesh-version.ts`; added `reverse_registration_*` columns to Drizzle schema (drift from migration 0026); deprecated old `GET /api/sync/discover` with RFC 8594 Deprecation/Sunset headers; updated stale `LOCAL_APP_VERSION`.
1. **Tailscale IP Auto-Detection (P0):** ✅ PR #617 — 3-step `selfSyncUrl` resolution chain (env var → `tailscale ip -4` CLI → fallback) with CGNAT/loopback validation; `/health` endpoint now exposes `selfSyncUrl` and `selfSyncUrlSource`.
2. **Local-Only Mesh Config + Dynamic Provider (P0):** ✅ PRs #619, #622 — New `mesh_local_config` table (migration 0028, NOT synced — no trigger). `MeshConfigProvider` replaces static startup opts for restart-free config changes. Authenticated `GET/PUT /api/mesh/config` routes in `mesh-config.ts`. Settings → Mesh UI (`MeshConfigSection.tsx`) with source badges and write-only token UX (browser `crypto.getRandomValues`, not Node `crypto.randomBytes`). PR #622 added the mesh identity settings UI.
3. **Structured Error Contract + Frontend Mapping (P0):** ✅ PR #623 — `mesh-errors.ts` with user-friendly descriptions for all mesh error codes; migration 0029 adds `reverse_registration_error_code` column to `sync_nodes`; preflight token compatibility check via `GET /api/mesh/config/preflight`; SSRF guard (`isAllowedPeerTarget()` restricting fetch to Tailscale CGNAT range) and ReDoS-safe trailing-slash removal replacing polynomial regex. ✅ PR #627 — extracted `isAllowedPeerTarget()` and `stripTrailingSlashes()` to shared `url-guards.ts`, fixed remaining unguarded instances in `peer-reverse-registration.ts` and `sync-peer-update.ts`, 22 new unit tests. ✅ PR #628 — wired mesh preflight token check into the add-peer dialog via `usePreflightMeshConfig` React Query hook and `PreflightStatusIndicator` component (auto-fires after successful URL probe), 14 new tests.
4. **Retry + Operational (P1):** ✅ PR #623 — `reverse-registration-retry.ts` with automatic retry for failed reverse registrations; `SelfIdentityCard.tsx` showing local node identity (machineId, hostname, Tailscale IP, syncUrl); `isPeerStale()` helper + visual indicators for peers not seen in 7+ days; async peer update via `POST /api/sync/peers/:id/update` with SSE log streaming (`GET /:id/update/:jobId/log`) and `PeerUpdateLogModal` real-time viewer.

**Success criteria:** (1) Zero-SSH peering from web UI. (2) `TAILSCALE_IP=127.0.0.1` does not break peering (auto-detect falls through with warning). (3) `mesh_local_config` never appears in `sync_change_log` (integration test). (4) Config changes without CP restart. (5) Every error code maps to an actionable fix.
...
|----------|------|---------|--------|
| **P0** | ~~Emergency Stop UI~~ | — | ✅ Delivered — PR #408 exposed existing emergency-stop backend routes in the web UI with confirmation dialogs, tests, and fleet-wide kill; PR #766 moved the backend-independent emergency-stop Playwright spec into the focused Web E2E CI gate. |
...
| **P1** | ~~Mesh Peer UX Overhaul~~ | 33.7 | ✅ Delivered — PR #550 ping failure diagnostics, PR #551 in-place editing, PR #582 failure details UI, PR #586 discover/probe backend, PR #590 discover UI flow + probe auto-fill + scheme hint (10 unit tests), PR #591 Playwright coverage (10 new E2E tests, 21 total). See [plans/2026-04-15-mesh-peer-ux-overhaul-plan.md](plans/2026-04-15-mesh-peer-ux-overhaul-plan.md). |
| **P0** | ~~Mesh Bidirectional Registration + Cross-Node Visibility~~ | 33.8 | ✅ Delivered — PR #552 bootstrap-token register endpoint, PR #554 machine provenance badges, PR #564 auto reverse-registration + one-way badge/retry, PR #576 mesh health panel, PR #589 two-node machine replication integration proof (5 tests: A→B, B→A, UPDATE propagation, stale rejection, apply guard), PR #596 fixed PM2 env passthrough for `SYNC_PEER_REGISTRATION_TOKEN` (one-way peering on live fleet), PR #654 added the opt-in live two-node Playwright fixture foundation, PR #673 added the separately gated A-to-B machine visibility browser assertion, and PR #675 added the remaining add-peer/reverse-registration plus one-way warning/retry fixture assertions. See [plans/2026-04-15-mesh-bidirectional-registration-plan.md](plans/2026-04-15-mesh-bidirectional-registration-plan.md). |
| **P1** | ~~Mesh Version Observability~~ | 33.9 | ✅ Delivered — PR #555 added nullable `sync_nodes` peer-version columns plus schema/shared contracts, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #558 delivered `/mesh-peers` version/drift UI plus the lazy sidebar peer-version tooltip, PR #560 renumbered the peer-version migration to `0025`, and PR #561 persisted peer metadata from on-demand and background pings with coverage. See [plans/2026-04-15-mesh-version-observability-plan.md](plans/2026-04-15-mesh-version-observability-plan.md). |
| **P0** | ~~Mesh Schema + Protocol Compat Policy~~ | 33.10 | ✅ Delivered — PR #557 delivered envelope meta/protocol constants, apply-side compat gates, legacy tolerance, and `MESH_COMPAT.md`. PRs #567/#573 delivered "Peer ahead" banner with persistent rejection tracking. PR #594 delivered two-node integration proof (6 schema scenarios) + PeerSchemaAheadBadge Playwright E2E (3 tests). See [plans/2026-04-15-mesh-schema-protocol-compat-plan.md](plans/2026-04-15-mesh-schema-protocol-compat-plan.md). |
| **P1** | Fleet Rollout & Peer Auto-Update (two-topology) | 33.11 | In progress — PM2 topology: PR #563 (manual update), PR #571 (`agentctl peer update` CLI), PR #572 (opt-in schedulers), PR #577 (settings UI), PR #580 (rollback). Docker topology: PR #575 (build provenance), PR #579 (migration gate), PR #593 (machines.yml fleet structure), PR #678 (docker-topology deploy-fleet dry-run/canary guard + runbook). Cross-topology: PR #570 (version-compat API), PR #582 (update-available banner), PR #654 (opt-in live two-node fixture plus first version-drift assertion), PR #659 (separately gated peer-update dry-run fixture assertion), PR #668 (separately gated schema-ahead fixture assertion). Remaining: operator-executed end-to-end dry-run and canary run. See [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md). |
| **P0** | ~~Mesh Peering UX & Reliability Overhaul~~ | 33.12 | ✅ Delivered — Phase 0: PR #610 (version-bump.sh fix + Drizzle schema drift + deprecate old discover). Phase 1: PR #617 (Tailscale IP auto-detect with CGNAT validation + selfSyncUrl on /health). Phase 2: PRs #619/#622 (local-only `mesh_local_config` table + `MeshConfigProvider` for restart-free config + `GET/PUT /api/mesh/config` + Settings → Mesh UI with source badges and write-only token UX + mesh identity settings UI). Phase 3: PRs #623/#627/#628 (structured mesh error codes + preflight token check + SSRF/ReDoS guards + migration 0029 + add-peer preflight UX). Phase 4: PR #623 (reverse-registration retry + self-identity card + stale peer cleanup + async peer update with SSE log streaming + PeerUpdateLogModal). See [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md). |
...
| **P1** | Unified Memory System UI | 4.8 | Partial — route family, facts browser, dashboard, claude-mem import, hardened basic JSONL history import, JSONL import job provenance plus rollback UI/API (PR #836), durable Memory Import job state with restart-visible resume (PR #841) plus orphan-running resume reconciliation (PR #842), PR #845's persisted JSONL cursor/same-job crash-before-checkpoint accounting, PR #847's structured `claude-mem` child facts plus `summarizes` edges, and PR #849's source-keyed `claude-mem` import ids, basic graph/table, deterministic reports, synthesis lint, maintenance, and core API routes exist. Not complete: embedding-similarity dedup, rich graph Timeline/Clusters/focus/time-lapse, LLM narrative reports, and full consolidation merge/resolution workflows. |
...
| **P1** | MemPalace-Inspired Memory Evolution | 3.6 / 4.8 / 7.3 | In progress — PR #655 delivered the Phase 0 eval foundation (fixture schema/sanitization, seed-42 split helpers, mock metrics, sanitized sample fixture, and `pnpm memory:eval` CLI), PR #660 added deterministic mock recall@5 threshold enforcement plus `pnpm memory:bench` without live DB/embedding dependencies, PR #667 locked first-run / `{ arguments: null }` MCP contracts for the current search/recall/report routes, PR #671 delivered the Phase 1 drawer schema/chunker/sanitizer/store foundation (`0030`, shared redaction/validation, deterministic chunking, sanitized hash/embed/store tests) without drawer sync triggers or search default changes, PR #677 added the three-stage query sanitizer on worker/control-plane memory search paths, PR #679 added the redacted `memory_write` audit entry path, PR #681 added the first `memory_dedup_check` worker route/cold-start contract, PR #682 added persisted drawer backfill state (`0032`) with store coverage, PR #684 added `memory_fact_sources` / `embedding_version` schema groundwork, PR #685 added the first `memory_traverse` worker contract, PR #686 added the JSONL drawer backfill CLI foundation, PR #719 added fact-side Memory Browser match-source badges, PR #720 added optional tier token caps to context-budget enforcement, PR #721 added dry-run backfill token/cost/storage estimates, PR #722 added opt-in live eval wiring, PR #723 added Memory Browser raw drawer results, PR #726 added browser-level Memory Browser enrichment coverage, PR #728 added fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 added workflow-owned weekly/release eval automation via `.github/workflows/memory-evals.yml`, PR #730 stabilized empty-provenance fact-detail responses for legacy facts, PR #732 hardened memory identifier/name validation across control-plane and worker routes, PR #733 merged the `xmldom`/`uuid` dependency overrides that cleared the 2026-04-23 security alert wave, PR #734 added the first script-only Surface A `MEMORY.md` dry-run generator, PR #735 fixed the Claude project memory target to `~/.claude/projects/<encoded>/memory/MEMORY.md`, PR #736 added bounded injector result modes (`fact-only` default, PostgreSQL `fact-plus-snippet`, explicit `full-drawer` fallback), PR #737 added the paged control-plane API source for Surface A dry runs, PR #738 added merge-time provenance preservation with store-level `memory_fact_sources` copy/dedupe support, PR #748 wired accepted near-duplicate consolidation actions into provenance-preserving duplicate invalidation, PRs #776/#784/#800 hardened nonblocking PreCompact checkpoint capture, PR #788 added basic JSONL history import wiring, PR #794 hardened JSONL import idempotency/count/bounded-read/error handling, PR #796 added the workflow/CLI failure-mode coverage gate for private memory eval fixtures, PR #802 surfaced fixture coverage in eval reports/workflow artifacts, PR #803 hardened session/agent evidence detail rendering for mobile-sized surfaces, PR #815 gave the public sample fixture one sanitized focused row per default failure-mode tag with a refreshed facts-only baseline, PR #816 delivered Memory Operations PR E embedding/drawer backfill workers with boot reconciliation and source-root safety, PR #820 delivered Memory Operations PR F with the operations page, missing-provider alerts, job detail drawer, and egress confirmation flow, PR #827 delivered Memory Operations PR G consolidation/synthesis workers plus focused unit/Playwright coverage, PR #828 delivered PostgreSQL `full-drawer` injection for on-demand facts with source-drawer hydration and budget caps, PR #830 delivered additive evidence budget accounting for `fact-plus-snippet` and `full-drawer` rendering, PR #832 delivered the Surface A `MEMORY.md` approval-token/durable-gate write flow, PR #833 gates private eval fixtures on a dated changelog artifact, PR #836 adds import job provenance plus rollback UI/API for completed JSONL imports, PR #835 adds `pnpm memory:eval:secrets` for local private fixture/changelog secret validation and stdin-based `gh` rotation, PR #840 adds pure entity canonicalization helpers plus a read-only extraction benchmark, PR #841 adds persisted `memory_import_jobs`, restart-visible interrupted job state, and a resume API/UI path, PR #842 reconciles restart-orphaned running jobs before direct resume, PR #845 adds `memory_import_jobs.cursor_json`, non-positional JSONL resume cursors, same-job JSONL crash-before-checkpoint accounting, per-row `claude-mem` progress cursors, PR #847 adds `claude-mem` `facts[]` child-fact import with child-to-parent `summarizes` edges, PR #849 adds source-keyed `claude-mem` import ids, PR #851 adds aggregate-only private fixture validation, and PR #852 adds the first production local-only canonical entity schema/store slice. v1.3 keeps the existing PostgreSQL-native memory core; the sync precondition for future drawer payloads and temporal edge fields is now explicit in [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). Remaining work now continues with operator application of validated eval secrets, richer private held-out/full examples under the coverage gate, Memory Browser evidence filters/default-on rollout criteria, raw-source filters, fuller why-this-matched explanations, Diary/Timeline result treatment, rank-bucket boosts, retention, control-plane `memory_traverse` traversal/timeline gates, dedicated temporal edge fields/timeline API/as_of validity-window behavior, entity canonicalization backfill/apply and runtime integration beyond PR #852's local schema/store, embedding-similarity dedup, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification. See [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md) and [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). |
| **P1** | ~~UI Quality & Accessibility~~ | 4.7 | ✅ Delivered — all original ARIA items complete (PRs #51,#54,#59), StatusBadge descriptions refreshed in PR #417, slash-search/hotkey discoverability improved in PRs #446/#449, shared Sidebar/Sessions `?` event ownership centralized in PR #450, sidebar Go To keyboard coverage plus Settings shortcut docs expanded in PR #525, loading spinners + aria-live skeleton loaders in PR #611, ErrorBoundary for 11 pages in PR #616, ErrorBoundary for remaining 3 detail pages in PR #629 (all web pages now covered), and dashboard quick-action accessibility/contrast coverage in focused Web E2E CI in PR #839 |
...
| **P1** | Machines / Terminal E2E Follow-up | 29 | ✅ Delivered — PR #346 shipped dedicated Playwright coverage for the existing machine terminal page without widening into §27.3; PR #769 moved it into the focused Web E2E CI gate. |
...
| **P0** | Agent Run Quality | 16.1 | Current loop is closed through PR #852. PR #851 landed the private fixture guard utility and PR #852 landed the local-only entity canonicalization schema/store slice; post-merge main CI `24928487675`, Security Audit `24928487676`, and Build & Publish Docker Images `24928487694` passed at `bf0bf676`. There are no open PRs during this sync, and code-scanning, Dependabot, and secret-scanning alert lists report 0 open alerts. Historical stability work through PR #797 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. The latest delta is PR #800 deferred PreCompact capture startup, PR #801 preserved legacy no-provenance memory compatibility, PR #802 surfaced memory-eval fixture coverage while keeping failed-run artifacts visible, PR #803 hardened mobile evidence detail rendering, PR #806 landed the Settings -> Memory & Embeddings provider UI, PR #807 persisted saved-provider test results, PR #808 hardened the saved-provider status path against raw upstream error storage and stale metadata writes, PR #809/#810 synced and reconciled docs, PR #811 closed remaining Memory Ops/MemPalace plan drift, PR #812 delivered the Memory Operations PR D job-orchestration infrastructure, PR #815 added focused public memory-eval sample coverage, PR #816 delivered Memory Operations PR E workers, PR #818 restored main Build stability by requiring exact TypeScript build-cache matches, PR #817/#819 kept the roadmap checkpoint accurate after the CI follow-up, PR #820 landed the Memory Operations PR F operations page, alerts, and egress flow, PR #821 synced docs after PR #820, PR #822 refreshed Docker runtime apt layers/scanner cache-busting after post-#821 container scan annotations, PR #823 pinned production Node runtime images to 22.22.2 after the remaining Node 22.22.1 Grype finding, PR #824 synced roadmap/plan docs through that checkpoint, PR #825 aligned Docker publish Grype reporting with fixed-version vulnerabilities only, PR #826 synced that state into docs, PR #827 delivered Memory Operations PR G, PR #828 delivered the full-drawer injector slice, PR #829/#831 synced docs, PR #830 delivered additive injector evidence budget accounting, PR #832 delivered Surface A write approval gates, PR #833 delivered private memory-eval changelog discipline, PR #834 reconciled the roadmap/plan state after those landings, PR #836 delivered JSONL import rollback, PR #835 delivered memory-eval secret preflight tooling, PR #837 synced the post-#835/#836 plan state, PR #839 gated dashboard quick-action accessibility coverage, PR #840 added entity canonicalization benchmark groundwork, PR #841 added durable Memory Import jobs and resume UI/API, PR #842 fixed orphan-running direct resume, PR #845 hardened per-unit Memory Import resume cursors, PR #846 synced roadmap/plan state after PR #845, PR #847 added structured `claude-mem` import child facts plus `summarizes` edges, PR #848 synced that post-merge state, PR #849 added source-keyed `claude-mem` import ids for source-local idempotency, PR #850 synced the docs checkpoint after PR #849, PR #851 added aggregate-only private fixture validation, and PR #852 added the local-only canonical entity schema/store slice. |
...
| [main-stability-and-security-cycle-plan](plans/2026-03-15-main-stability-and-security-cycle-plan.md) | Delivered on historical scope; this registry row is no longer the live checkpoint ledger. It records the tail through PR #686: PR #649 repaired the scheduled cleanup-images workflow, PR #650 cleared Dependabot alerts #33/#34, PR #651 silenced expected worker tailscale CLI stderr, PR #653 gated the current approvals Playwright surfaces in focused CI, PR #654 added the opt-in live two-node mesh fixture foundation, PR #655 delivered the MemPalace Phase 0 eval harness foundation, PR #659 added the mesh peer-update dry-run fixture assertion, PR #660 added deterministic MemPalace planted-needle recall threshold coverage, PR #662 expanded focused Web E2E CI to 20 specs / 68 tests, PR #663-#666 synced the roadmap/registry checkpoint, PR #667 delivered current memory MCP cold-start/null-arguments contracts, PR #668 added the schema-ahead fixture assertion, PR #669 synced the post-#668 docs checkpoint, PR #670 refreshed the post-#669 roadmap/plan audit, PR #671 added the MemPalace drawer schema/chunker/sanitizer/store foundation, PR #672 synced the post-#671 docs checkpoint, PR #673 added the Mesh 33.8 A-to-B machine visibility browser assertion, PR #674 synced the post-#673 docs checkpoint, PR #675 added the remaining 33.8 add-peer/reverse-registration plus one-way warning/retry fixture assertions, PR #677 added memory search query sanitization, PR #678 added Docker-topology fleet dry-run/canary safety guards, PR #679 added redacted memory write audit entries, PR #681 added the first memory dedup-check route contract, PR #682 added drawer backfill state persistence, PR #684 added fact-source schema groundwork, PR #685 added the first memory-traverse worker contract, and PR #686 added JSONL drawer backfill CLI foundation. The live post-#818 checkpoint is now maintained in the roadmap header/current-state ledger and Agent Run Quality row. | 16.1-16.3 |
| [mesh-runtime-e2e-follow-up-plan](plans/2026-04-01-mesh-runtime-e2e-follow-up-plan.md) | Delivered — PR #395 (session `Config` tab detail-state coverage) + PR #396 (`/conflicts` resolution-flow coverage) both merged 2026-04-13; PR #397 synced the tracking doc; PR #431 extended coverage to `/mesh-peers` render/empty/ping states; PR #483 added `/mesh-peers` create/delete/validation coverage; PR #662 added `session-config.spec.ts` to focused Web E2E CI | 16.1, 31.3, 33.3, 33.4 |
| [mesh-peer-ux-overhaul-plan](plans/2026-04-15-mesh-peer-ux-overhaul-plan.md) | Delivered — PR #550 backend ping diagnostics, PR #551 in-place peer editing, PR #582 failure-details UI, PR #586 discover/probe backend, PR #590 discover/probe UI, and PR #591 browser coverage | 33.7 |
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | Delivered — PR #552 signed reverse-registration endpoint, PR #554 machine provenance badges, PR #564 auto-register-on-add plus one-way retry, PR #576 mesh health summary, PR #589 two-node machine replication proof, PR #596 PM2 registration-token passthrough, PR #654 opt-in live two-node fixture foundation, PR #673 separately gated the A-to-B machine visibility fixture assertion, and PR #675 added the remaining add-peer/reverse-registration plus one-way warning/retry browser assertions | 33.8 |
| [mesh-version-observability-plan](plans/2026-04-15-mesh-version-observability-plan.md) | Delivered — PR #555 delivered nullable peer-version schema/shared groundwork, PR #556 delivered `/health` build metadata, PR #558 delivered the version/drift UI, PR #560 renumbered the peer-version migration to `0025`, and PR #561 persisted peer metadata from on-demand/background pings with coverage | 33.9 |
| [mesh-schema-protocol-compat-plan](plans/2026-04-15-mesh-schema-protocol-compat-plan.md) | Delivered — PR #557 envelope metadata/apply gates/compat docs, PRs #567/#573 operator surfacing, and PR #594 two-node schema scenarios plus PeerSchemaAheadBadge Playwright coverage | 33.10 |
| [mesh-fleet-rollout-peer-update-plan](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md) | In progress — PM2 CLI/provenance/rollback/schedulers/settings landed via PRs #563/#571/#572/#577/#580; Docker provenance/migration/fleet structure landed via PRs #575/#579/#593; PR #582 added the update-available banner; PR #654 added the opt-in live fixture plus first version-drift assertion; PR #659 added the separately gated peer-update dry-run fixture assertion; PR #668 added the separately gated schema-ahead fixture assertion; PR #678 added docker-topology deploy-fleet dry-run/canary guards and runbook. Remaining: operator-executed end-to-end dry-run and canary run | 33.11 |
| [mesh-peering-ux-reliability-overhaul-plan](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) | Delivered — Phase 0 PR #610, Phase 1 PR #617, Phase 2 PRs #619/#622, Phase 3 PRs #623/#627/#628, and Phase 4 PR #623. Remaining mesh two-node/browser fixture work is tracked by 33.8/33.11 | 33.12 |
| [coverage-feature-depth-batch-plan](plans/2026-03-19-coverage-feature-depth-batch-plan.md) | Delivered — §20.1-20.8 shipped on `main`; later §20.5 browser-depth follow-ups added `/logs` Security Findings coverage in PR #454, `/memory/browser` facts-flow coverage in PR #455, `/memory/import` wizard completion/cancellation coverage in PR #457, backend-independent `/settings` notification preference coverage in PR #458 with focused CI gating in PR #546, `/discover` grouped/filter/import/new-session coverage in PR #459, `/memory` index coverage in PR #461, `/memory/consolidation` coverage in PR #462, `/memory/scopes` coverage in PR #465, `/memory/graph` coverage in PR #466, `/spaces/[id]` + `/tasks/[id]` detail coverage in PR #468 with focused CI gating in PR #762, `/memory/synthesis` plus webhook-delivery plus push-device coverage in PR #475, `/memory/maintenance` plus auto-decompose coverage in PR #478, `/memory/browser` provenance-filter coverage in PR #486, task auto-decompose apply-failure coverage in PR #487 with focused CI gating in PR #768, the §20.9 webhook delivery retry follow-up in PR #488, backend-independent `/agents/[id]` detail/start coverage in PR #492 with focused CI gating in PR #765, backend-independent `/agents` list/create/start coverage in PR #494 with CI gating in PR #518, backend-independent `/agents/[id]/settings` coverage in PR #495, backend-independent `/machines` list/detail coverage in PR #499 with CI gating in PR #516, backend-independent `/agent-profiles` coverage in PR #505, dedicated `/audit` unit coverage in PR #507, backend-independent `/audit` Playwright coverage in PR #513, scheduler jobs page unit coverage in PR #517, backend-independent `/sessions/[id]` route smoke coverage in PR #529, backend-independent `/memory/dashboard` route smoke coverage in PR #528, emergency-stop CI gating in PR #766, broader tasks/spaces CI gating in PR #767, machine-terminal CI gating in PR #769, and memory-import plus memory-scopes CI gating in PR #771 | 20.1-20.9 |
| [task-auto-decompose-apply-failure-e2e-plan](plans/2026-04-14-task-auto-decompose-apply-failure-e2e-plan.md) | Delivered — PR #487 added the focused `/tasks/[id]` browser regression for `POST /api/decompose` apply failures after a valid preview; PR #768 moved the auto-decompose spec into the focused Web E2E CI gate | 10.5, 20.5 |
...
| [agent-detail-e2e-plan](plans/2026-04-14-agent-detail-e2e-plan.md) | Delivered in PR #492 with CI-gating follow-up in PR #765 — backend-independent `/agents/[id]` route coverage for metadata/config/cost/session/memory/run-history render plus start-run prompt wiring | 20.5 |
...
| [audit-trail-page-plan](plans/2026-04-14-audit-trail-page-plan.md) | Delivered in PR #507 with browser-depth follow-up in PR #513, backend contract repair in PR #743, and mobile contract alignment in PR #745 — dedicated `/audit` page, sidebar entry, audit summary/action-list surface, filters/pagination, focused mocked-query unit coverage, backend-independent Playwright coverage in the web E2E CI lane, `GET /api/audit/summary` returning the frontend-consumed breakdown shape, and the mobile API client now matching that contract. | 6.5, 20.5 |
| [scheduler-jobs-page-plan](plans/2026-04-15-scheduler-jobs-page-plan.md) | Delivered in PR #517 with PR #760 follow-up — `/scheduler` jobs page, sidebar entry, scheduler API client, not-configured/create/delete states, confirmed remove-all action over the existing bulk deletion API, and focused unit/browser coverage | 2.1, 20.5 |
...
| [machine-terminal-e2e-follow-up-plan](plans/2026-03-21-machine-terminal-e2e-follow-up-plan.md) | Delivered — PR #346 added the dedicated machine terminal Playwright coverage on `main`; PR #769 moved it into the focused Web E2E CI gate; no page-level hardening was required beyond the focused e2e spec | 29 |
