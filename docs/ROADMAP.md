...
> Last updated: 2026-04-20 (post-#704 -- PR #702 synced the roadmap/plan checkpoint for PRs #697-#701 without touching runtime code; PR #703 extended `scripts/backfill-memory-drawers.ts` to also map claude-mem `session_summaries` rows into atomic `memory_facts` with deterministic `session_summaries:<id>:parent` source-span keys, threading `sessionSummary` through `processClaudeMemCandidate`, adding `planSessionSummaryFactWrites` + `writeSessionSummaryFacts` helpers, three new counters (`sessionSummaryFactCandidates/Written/Skipped`), 4 new `describe('claude-mem session_summaries fact mapping')` tests (happy path / full-drawer fallback / idempotency / error summarization), with 22/22 backfill tests green (up from 18); PR #704 landed the control-plane MemPalace drawer MCP backend index: new `packages/control-plane/src/api/routes/memory-drawers.ts` (571 lines + 473-line test with 21 cases) exposes `GET /api/memory/drawers/search?q&scope&limit` using `content_tsv_simple` tsvector (config `simple`) with optional pgvector cosine via Reciprocal Rank Fusion (k=60) gated on an `EmbeddingClient` option, and `GET /api/memory/drawers/:drawerId` with the same safe-identifier pattern used by worker `memory-drawer-get` (blocks path traversal, control chars, leading dots, empty-after-trim, length > 128); empty-query-after-sanitization returns `200 { ok: true, results: [] }` (no 204) to match the worker cold-start contract; the route plugin is registered in `server.ts` but the `embeddingClient` option is intentionally not yet wired from `index.ts`, so drawer search runs keyword-only until the trivial follow-up lands. Previous checkpoint: 2026-04-20 (post-#698 -- PR #697 repaired the remote peer upgrade drift that bricked macmini in 2026-04 by adding `pnpm drizzle-kit migrate` between `pnpm build` and `pm2 reload` in `scripts/peer-update.sh`, polling `/health` with a deadline after reload, running a full git+install+build+reload rollback on probe failure, adding `AGENTCTL_PEER_MAX_VERSION_SKEW` plus `AGENTCTL_SKIP_MIGRATIONS` escape hatches, updating `packages/control-plane/src/api/routes/sync-peer-update.ts` doc comments, landing a new `scripts/peer-update-sh.test.ts` content-regression suite (9 invariant assertions), and appending the Fleet Upgrades RCA + physical recovery playbook to `docs/LESSONS_LEARNED.md`; PR #698 corrected the PR #695 score/results search-path docs from `POST /api/memory/facts` to `GET /api/memory/facts` and refreshed the post-#696 checkpoint without touching runtime code; PR #699 extended `scripts/backfill-memory-drawers.ts` to write claude-mem atomic facts with drawer-source links (not just drawers) by adding the `MemoryStoreLike`/`AddFactSourceSpan`/`AddFactLikeInput` types, four new helpers (`planObservationFactWrites`, `writeObservationFacts`, `computeFactSourceSpans`, `summarizeBackfillError`), `--machine-id` CLI flag, deterministic source keys `observations:<id>:parent` and `observations:<id>:fact:<idx>`, offset provenance computed via sanitized-text lookup with full-drawer fallback, and 7 new test cases under `describe('claude-mem fact mapping')`; `session_summaries` remain drawer-only in this slice. PR #700 expanded the focused web e2e CI lane from 21 → 23 specs / 72 → 79 tests by adding `memory-reports.spec.ts` (9.6s isolated) and `memory-graph.spec.ts` (18.4s isolated) to `packages/web/package.json` `test:e2e:ci`; 5 alternative specs were rejected after stability evaluation with PR #665-style rationale. PR #701 locked the MemPalace-inspired drawer MCP tool contracts at the worker tier: new `POST /api/mcp/memory-drawer-search` route (252 lines + 206-line test) validates `query`/`scope`/`limit (1-100)` with sanitized query, forwards to CP `GET /api/memory/drawers/search`, maps `404/501/204` to `{ ok: true, results: [] }` (cold-start contract), and returns 503 `MEMORY_DRAWER_SEARCH_UNREACHABLE` on network error; new `POST /api/mcp/memory-drawer-get` route (170 lines + 193-line test) validates `drawer_id` via a safe-identifier pattern that blocks path traversal, control chars, leading dots, empty-after-trim, and length > 128, forwards to CP `GET /api/memory/drawers/:encodedId`, and returns `404 DRAWER_NOT_FOUND` for missing/incomplete records; new shared types `MemoryDrawerSearchRequest/Response/Result` + `MemoryDrawerSearchResultMatchType` and `MemoryDrawerGetRequest/Response`; `memory-cold-start.test.ts` extended with null-arg fast-rejects (1s timeout race) + empty-index contract entries for both routes; 1485/1485 worker tests green; actual drawer-index backend integration (vector + keyword fusion over `memory_drawers`) intentionally deferred to the CP follow-up PR. Previous checkpoint: 2026-04-20 (post-#696 -- PR #696 synced the roadmap/plan checkpoint for PRs #692, #694, and #695 after all main checks passed; PR #692 extended the `memory:backfill-drawers` CLI with a `--source-type claude-mem` path that maps `observations` and `session_summaries` into deterministic drawer source refs (`observations:<id>`, `session_summaries:<id>`) with dry-run counts and resumable table/id cursors, keeping the default `session-jsonl` path untouched and deferring `memory_fact_sources` links; PR #694 replaced the worker `memory_dedup_check` 501 `DEDUP_SCORING_UNAVAILABLE` branch with full skip/merge/store_new scoring at the plan's Phase 4 Step 7 thresholds (0.92 skip, 0.82 merge, immutable `.slice().sort()` top-candidate select, rationale embeds `toFixed(3)` top score, defensive `store_new` on missing/non-finite score, new `match_id: string | null` on `MemoryDedupCheckResponse`, 1466/1466 worker tests green); PR #695 unblocked that dedup scoring by adding a new additive `results: [{fact, score, source_path}]` array alongside `facts` on the semantic-search branch of `GET /api/memory/facts`, preserving filter-then-paginate order across both arrays, leaving the ILIKE/empty-`q` branches untouched for back-compat, with 22/22 CP route tests green and full CP suite 3480/3480 green. Previous checkpoint: 2026-04-20 (post-#691 -- PR #688 linked `memory_facts` to drawer source spans by persisting offset-only provenance via `MemoryStore.addFact` + `POST /api/memory/facts` `sourceSpans`; PR #689 landed the control-plane `POST /api/memory/traverse` iterative BFS endpoint honoring hop/node caps, relation/confidence/temporal filters, and 404 for missing start entities with 15 Vitest cases; PR #691 replaced broken `pnpm drizzle-kit migrate` with a SHA-256-aware psql applier in `scripts/drizzle-migrate-apply.ts` wired through `env-migrate.sh`, `env-up.sh`, and `env-promote.sh`). Previous checkpoint: 2026-04-20 (post-#686 -- PR #684 `memory_fact_sources` / embedding-version schema groundwork, PR #685 `memory_traverse` worker contract, and PR #686 JSONL drawer backfill CLI foundation). Earlier checkpoint: 2026-04-20 (post-#682 -- PR #681 `memory_dedup_check` first route/cold-start contract and PR #682 persisted drawer backfill state foundation). PR #675 added the remaining 33.8 browser fixture assertions without changing default E2E or beta/dev/prod CD behavior. PR #673 added the separately gated live two-node A-to-B machine visibility browser assertion: the fixture opens the secondary node's `/machines` page, searches for a primary-authored machine row, and verifies the `Synced from ...` provenance badge without changing default e2e or beta/dev/prod CD behavior. PR #672 refreshed the post-#671 roadmap/plan checkpoint. PR #671 added the MemPalace Phase 1 drawer schema/chunker/sanitizer/store foundation without drawer sync triggers or search default changes. PR #670 refreshed the post-#669 roadmap/plan checkpoint. PR #669 synced this roadmap after the MemPalace cold-start MCP contracts and final 33.11 two-node fixture slice. PR #668 added the separately gated live two-node schema-ahead assertion: force a synthetic `schemaVersion + 2` envelope through the primary node apply gate, persist the 33.10 rejection marker, and assert the schema-ahead badge on `/mesh-peers` without changing default e2e or beta/dev/prod CD behavior. PR #667 added current worker memory MCP cold-start contracts for search/recall/report, structured `{ arguments: null }` 400s for existing memory routes, and left `memory_dedup_check` / `memory_traverse` contracts for their planned route phases. PR #666 synced the roadmap after PR #664/#665. PR #665 added `memory-index.spec.ts` (memory browser index drill-down, 4 mock-based tests) to the focused web e2e CI allowlist after evaluating and rejecting `memory-decay` + `memory-consolidation` (three memory specs at once caused `webhooks.spec.ts` parallel-worker contention flakes), `mesh-peers.spec.ts` (4 permanent failures even at 4 specs, not mock-safe), and `smoke.spec.ts` (8.5min / 124 tests would explode CI duration). Local validation ran 72 tests clean twice (21.7s, 24.8s). PR #664 re-aligned the post-#663 roadmap registry headline and checkpoint after PR #663's merge. PR #663 synced the roadmap after PR #662. PR #662 added `session-config.spec.ts` (session detail config tab) as a mock-based follow-up to PR #657, after evaluating and rejecting `emergency-stop.spec.ts` (flakes under parallel load), `memory-browser.spec.ts` (too slow, 40.2s isolation), and `mcp-skill-discovery.spec.ts` (requires real backend). PR #661 synced the roadmap after PR #660's MemPalace planted-needle bench slice. PR #657 expanded the focused e2e CI allowlist to 19 specs / 65 tests by adding webhook-deliveries, agent-settings, and settings-push-devices mock-based specs, and fixed the pre-existing `session-detail.spec.ts` strict-mode breadcrumb selector. PR #659 added the separately gated live two-node mesh dry-run assertion that streams `pnpm agentctl peer update --dry-run` through `/api/mesh/auto-update/dry-run` without changing default e2e or beta/dev/prod CD behavior. PR #660 extended the Phase 0 memory eval harness with a deterministic planted-needle recall regression bench, env-configurable `MEMORY_BENCH_NEEDLE_COUNT` / `MEMORY_BENCH_NOISE_COUNT` / `MEMORY_BENCH_MIN_RECALL`, threshold enforcement, p50/p95/p99 latency reporting, and `pnpm memory:bench` while still avoiding real DB or embedding dependencies in CI. PR #653 rewrote `/approvals` Playwright coverage for the current permission-request UI and added the approvals page plus notification-bell approvals specs to focused Web E2E CI. PR #654 added the opt-in live two-node mesh Playwright fixture foundation for version-drift verification without changing default E2E or beta/dev/prod CD behavior. PR #655 added the MemPalace Phase 0 deterministic memory eval harness, sanitized sample fixture, mock scoring tests, and `pnpm memory:eval` CLI. Earlier: PR #649 fixed the scheduled Cleanup Old Docker Images workflow by treating missing GHCR packages as a no-op and moving github-script to v8. PR #650 pinned patched @fastify/static 9.1.1 via pnpm override, clearing Dependabot alerts #33/#34. PR #651 silenced expected tailscale CLI stderr on hosts without the binary. PR #647 added automatic React Query cache invalidation for sessions, runtime-sessions, metrics, and health when agent runs end (completed/stopped/error) or loops complete, plus full cache refresh on WebSocket reconnect so stale data from disconnection periods is replaced. PR #643 updated tests for v0.7.0 and import preview changes. PR #644 wired WebSocket agent events to toast notifications. PR #645 replaced hardcoded hex colors with semantic design tokens across 10 files + deleted dead `MemoryPlaceholderView.tsx`. PR #646 synced roadmap for PRs #643-#645. Earlier: PR #608 expanded the e2e CI allowlist with 8 backend-independent specs (deployment, discover, conflicts, logs-security-findings, security-findings, session-detail, memory-dashboard) and removed runtime-sessions from CI due to WebSocket timing flakiness (now 14 specs in CI). PR #609 synced the roadmap for PRs #596-#607. PR #610 landed §33.12 Phase 0: Drizzle `reverse_registration_*` schema drift fix, deprecated old `GET /api/sync/discover` with RFC 8594 headers, and fixed `version-bump.sh` to include `mesh-version.ts` plus updated stale `LOCAL_APP_VERSION`. PR #611 added loading spinners to CreateSessionForm, AgentFormDialog, ConvertToAgentForm, and RuntimeSessionsPage form buttons, and `aria-live` to SessionsPage and AgentsPage skeleton loaders. PR #616 wrapped 11 pages with ErrorBoundary: 9 memory sub-pages plus the tasks and settings/router pages. PR #617 delivered §33.12 Phase 1: 3-step `selfSyncUrl` resolution chain (env var → `tailscale ip -4` → fallback) with CGNAT/loopback validation, and exposed `selfSyncUrl`/`selfSyncUrlSource` on the `/health` endpoint. PR #619 delivered §33.12 Phase 2: `mesh_local_config` table (migration 0028), `MeshConfigProvider` class for restart-free config, `GET/PUT /api/mesh/config` routes, and `MeshConfigSection.tsx` in settings with source badges and write-only token UX. PR #622 delivered §33.12 Phase 2.4: mesh identity settings UI. PR #623 delivered §33.12 Phases 3+4: structured mesh error codes (`mesh-errors.ts`), reverse-registration retry scheduler, async peer update with SSE log streaming, `SelfIdentityCard` component, stale peer cleanup (`isPeerStale()` + visual indicators), SSRF/ReDoS security guards (`isAllowedPeerTarget()` + safe trailing-slash removal), preflight token compatibility check (`GET /api/mesh/config/preflight`), `PeerUpdateLogModal` real-time SSE viewer, and migration 0029 (`reverse_registration_error_code` column). PR #627 extracted SSRF/ReDoS guards to a shared `url-guards.ts` utility and fixed remaining unguarded instances in `peer-reverse-registration.ts` and `sync-peer-update.ts` with 22 new unit tests. PR #628 wired the mesh preflight token check into the add-peer dialog via a `usePreflightMeshConfig` React Query hook and `PreflightStatusIndicator` component that auto-fires after a successful URL probe. PR #629 wrapped `sessions/[id]`, `machines/[id]`, and `machines/[id]/terminal` with ErrorBoundary, completing coverage for all web pages. PR #630 implemented real claude-mem SQLite import with deduplication and preview. Earlier mesh landing set (PRs #550-#565) and 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
> Current checkpoint: PR #470 through PR #704 plus version bumps v0.5.2-v0.8.0 are represented on `main@7ee21749`. GitHub reports 0 open PRs, 0 open code-scanning alerts, 0 open Dependabot alerts, and 0 open secret-scanning alerts as of 2026-04-20; post-#681 through post-#704 main CI, Security Audit, and Build & Publish Docker Images all passed. PR #691 is scripts-only (no package code path touched); PR #696/#698/#702 are docs-only; PR #697 is scripts + CP doc-comment code + a new content-regression test, restoring migrations + health probe parity with `env-promote.sh`; PR #699/#703 are backfill-script code extending claude-mem fact writes; PR #700 is a CI-only package.json allowlist expansion; PR #701 is shape-only worker MCP contract code (routes + shared types + cold-start tests) behind cold-start/null-arg contracts; PR #704 is additive CP routes (`GET /api/memory/drawers/search` + `/:drawerId`) with route-level `EmbeddingClient` injection point not yet wired from `index.ts`, so drawer search is keyword-only until the follow-up; PR #688/#689/#694/#695 are worker/control-plane code and PR #692 is backfill-script code; none of these shift any beta/dev/prod CD routing or restart requirements until the next dev-tier build. Major cleanup pass: 161 stale git worktrees removed (215 down to 54), reclaiming ~115GB of disk; 82 merged remote branches pruned.
>
...
> Coordination note addendum: PR #663 merged as `d53e94af`, PR #664 as `5a433d68`, PR #665 as `2271bf72`, PR #666 as `ea82fec9`, PR #667 as `48bc709f`, PR #668 as `f77f8074`, PR #669 as `811bdf31`, PR #670 as `c1e1add6`, PR #671 as `e19dd7d5`, PR #672 as `b7273f5e`, PR #673 as `f106f4f7`, PR #674 as `bfe9759c`, PR #675 as `92ab8c97`, PR #677 as `1dfadaa3`, PR #678 as `bfbca627`, PR #679 as `eb4f19b2`, PR #681 as `a976cae2`, PR #682 as `a66c9bc1`, PR #684 as `a2cfdda8`, PR #685 as `150c3be7`, PR #686 as `b8ed8ce4`, PR #687 as `99e36894`, PR #688 as `135344e1`, PR #689 as `e6164cfa`, PR #691 as `465ebb2e`, PR #692 as `ef93b76d`, PR #693 as `2d5ea917`, PR #694 as `f0c632e4`, PR #695 as `47243f58`, PR #696 as `efb87d41`, PR #697 as `5ca38166`, PR #698 as `aa90762f`, PR #699 as `f7c15804`, PR #700 as `be1b4a83`, and PR #701 as `c9132f1b`, PR #702 as `6155f03a`, PR #703 as `524a32c5`, and PR #704 as `7ee21749`.
>
> Current follow-up note: this pass records PRs #608-#696. Earlier PRs #608-#641 covered e2e CI allowlist expansion, roadmap sync, §33.12 Phases 0-4, loading spinners, ErrorBoundary coverage, SSRF/ReDoS guards, mesh preflight token, claude-mem import, security findings UI, empty state standardization, and unit test expansion. PR #643 updated tests for v0.7.0. PR #644 wired WebSocket agent events to toast notifications. PR #645 replaced hardcoded hex colors with semantic design tokens + deleted dead code. PR #646 synced roadmap. PR #647 added automatic React Query invalidation on agent status/loop WebSocket events + full cache refresh on reconnect, making dashboard/list pages update in real-time. PR #649 repaired the cleanup-images scheduled workflow, PR #650 cleared Dependabot alerts #33/#34, PR #651 silenced expected worker tailscale CLI stderr, PR #653 put both approvals Playwright surfaces into focused CI, PR #654 added the opt-in live two-node mesh fixture foundation, PR #655 delivered the first MemPalace Phase 0 eval harness slice, PR #656 synced the roadmap after mesh + memory landings, PR #657 expanded the focused web e2e CI allowlist by three mock-based specs (webhook-deliveries, agent-settings, settings-push-devices — 16→19 specs, 49→65 tests) and fixed a pre-existing `session-detail.spec.ts` strict-mode selector, PR #658 synced that e2e CI state into the roadmap, PR #659 added the separately gated mesh peer-update dry-run fixture assertion, PR #660 added deterministic MemPalace planted-needle recall threshold coverage, PR #661 synced the roadmap after PR #660, PR #662 added `session-config.spec.ts` to the focused web e2e CI allowlist (19 → 20 specs, 65 → 68 tests) after evaluating `emergency-stop`/`memory-browser`/`mcp-skill-discovery` and rejecting them for parallel flake, duration, and real-backend dependency respectively, PR #663 synced the post-#662 roadmap checkpoint, PR #664 re-aligned the roadmap registry headline and checkpoint after PR #663's merge, PR #665 added `memory-index.spec.ts` to the focused web e2e CI allowlist (20 → 21 specs, 68 → 72 tests) after evaluating and rejecting `memory-decay` + `memory-consolidation` (parallel-worker contention on webhooks), `mesh-peers.spec.ts` (not mock-safe, 4 permanent failures), and `smoke.spec.ts` (8.5min isolation duration), PR #666 synced the roadmap after PR #664/#665, PR #667 added current memory MCP cold-start/null-arguments contracts, PR #668 completed the 33.11 two-node fixture schema-ahead assertion, PR #669 synced the post-#668 docs checkpoint, PR #670 refreshed the post-#669 roadmap/plan audit, PR #671 added the MemPalace drawer schema/chunker/sanitizer/store foundation, PR #672 synced the post-#671 roadmap/plan checkpoint, PR #673 added the separately gated Mesh 33.8 A-to-B machine visibility browser assertion, PR #674 synced the post-#673 docs checkpoint, PR #675 added the remaining 33.8 add-peer/reverse-registration plus one-way warning/retry fixture assertions, PR #677 added query sanitization before memory embedding/search, PR #678 scoped deploy-fleet dry-run/canary validation to Docker topology, PR #679 added redacted memory_write audit entries, PR #681 added the first `memory_dedup_check` worker route contract, PR #682 added persisted drawer backfill state, PR #684 added the `memory_fact_sources` / `embedding_version` schema groundwork, PR #685 added the first `memory_traverse` worker contract, PR #686 added the JSONL drawer backfill CLI foundation, PR #687 synced the roadmap for post-#686, PR #688 linked `memory_facts` to drawer source spans via offset-only provenance on `MemoryStore.addFact` + `POST /api/memory/facts` `sourceSpans`, PR #689 landed the control-plane `POST /api/memory/traverse` iterative BFS route with hop/node caps, relation/confidence/temporal filters, and 404 for missing start entities (15 Vitest cases; iterative per-hop rather than recursive CTE), and PR #691 replaced the broken `pnpm drizzle-kit migrate` silent no-op with a SHA-256-aware psql-based applier at `scripts/drizzle-migrate-apply.ts` (537 lines + 704 lines of tests covering hash vector/journal parsing/diff/idempotency/failure modes) and wired it through `env-migrate.sh`, `env-up.sh`, and `env-promote.sh`; parity queries in `env-promote.sh` now use `hash FROM drizzle.__drizzle_migrations`. PR #692 extended `memory:backfill-drawers` with the `--source-type claude-mem` code path: it maps `observations` and `session_summaries` into deterministic drawer source refs (`observations:<id>`, `session_summaries:<id>`), adds dry-run counts, keeps resumable table/id cursors through `MemoryDrawerBackfillStateStore`, and defers writes to `memory_fact_sources` (drawer-only slice; the default `session-jsonl` path is unchanged). PR #693 synced the roadmap for PRs #688/#689/#691. PR #694 replaced the worker `memory_dedup_check` `501 DEDUP_SCORING_UNAVAILABLE` branch with full scoring aligned to the MemPalace plan's Phase 4 Step 7 thresholds: top-candidate select via immutable `.slice().sort()` by score descending, `skip` at `>= 0.92`, `merge` at `>= 0.82`, otherwise `store_new`; rationale strings embed the actual top score via `toFixed(3)`; defensive `store_new` when the top candidate is missing or non-finite; the shared `MemoryDedupCheckResponse` type gained a `match_id: string | null` field populated from the chosen candidate; 8 new Vitest cases cover boundary scores (0.92 / 0.82 / 0.4), defensive missing-score, and multi-candidate ordering, with 1466/1466 worker tests green. PR #695 added the CP score pass-through: `GET /api/memory/facts` now returns `results: [{fact, score, source_path}]` alongside `facts` on the semantic-search branch only, filter-then-paginate order preserved across both arrays, `total` still counts filtered results; the SQL ILIKE fallback and empty-`q` branches are untouched (no `results` field, back-compat preserved); 4 new CP route tests cover score/source_path pass-through across all three source paths, pagination parity, source-filter parity, and empty-`q` omission, plus a separate describe block for the SQL fallback absence. This unblocks the dedup route so scoring flows through real candidate records end-to-end instead of the defensive `store_new` fallback. PR #696 synced the roadmap/plan checkpoint for PRs #692, #694, and #695. PR #697 removed the `env-promote.sh` vs `peer-update.sh` drift that let the macmini remote upgrade cross a schema boundary without running migrations: the remote upgrade now runs `pnpm drizzle-kit migrate` before `pm2 reload`, polls `/health` with a deadline, runs a complete git+install+build+reload rollback if the probe fails, exposes `AGENTCTL_PEER_MAX_VERSION_SKEW` plus `AGENTCTL_SKIP_MIGRATIONS` escape hatches, and pins these invariants with a new 9-assertion `scripts/peer-update-sh.test.ts` content-regression suite plus the Fleet Upgrades RCA and physical recovery playbook in `docs/LESSONS_LEARNED.md`. PR #698 corrected the PR #695 score/results search-path docs from `POST` to `GET /api/memory/facts` and refreshed the post-#696 checkpoint. PR #699 extended `scripts/backfill-memory-drawers.ts` to write claude-mem atomic facts with drawer-source links by introducing `MemoryStoreLike`/`AddFactSourceSpan`/`AddFactLikeInput` types, four helpers (`planObservationFactWrites`, `writeObservationFacts`, `computeFactSourceSpans`, `summarizeBackfillError`), a `--machine-id` CLI flag, deterministic source keys (`observations:<id>:parent`, `observations:<id>:fact:<idx>`), offset provenance via sanitized-text lookup with full-drawer fallback, and 7 new test cases under `describe('claude-mem fact mapping')`; `session_summaries` remain drawer-only for this slice. PR #700 expanded the focused web e2e CI lane from 21 → 23 specs / 72 → 79 tests by adding `memory-reports.spec.ts` (9.6s isolated) and `memory-graph.spec.ts` (18.4s isolated) to `packages/web/package.json` `test:e2e:ci` after evaluating and rejecting 5 alternative specs with PR #665-style rationale. PR #701 locked the worker-side MemPalace drawer MCP contracts: `POST /api/mcp/memory-drawer-search` (252 lines + 206-line test) validates `query`/`scope`/`limit (1-100)` with sanitized query, forwards to CP `GET /api/memory/drawers/search`, and maps CP `404/501/204` to `{ ok: true, results: [] }` (cold-start contract); `POST /api/mcp/memory-drawer-get` (170 lines + 193-line test) validates `drawer_id` via a safe-identifier pattern (path traversal, control chars, leading dots, empty trim, length > 128), forwards to CP `GET /api/memory/drawers/:encodedId`, and returns `404 DRAWER_NOT_FOUND` for missing or incomplete records; new shared types cover both tools; `memory-cold-start.test.ts` extended with null-arg fast-rejects (1s timeout race) and empty-index contract entries for both routes; 1485/1485 worker tests green; actual drawer-index backend integration remains deferred to the CP follow-up PR. The latest landed main is `aa90762f` (post-#698); post-#698 main CI, Security Audit, and Build & Publish Docker Images all passed; beta/dev CD state remains untouched. PR #691 is scripts-only; PR #692 is scripts-only too (`scripts/backfill-memory-drawers.ts`/`.test.ts`); PR #696/#698 are docs-only; PR #697 is scripts + CP doc comment + a new content-regression test; PR #699 is backfill-script code; PR #700 is a CI allowlist change in `packages/web/package.json`; PR #701 is shape-only worker MCP contract code behind cold-start/null-arg gates; PR #694/#695 touch worker + CP + shared memory types; none require a worker/CP restart until the next dev-tier build.
...
- [ ] Extend drawer backfill to `claude-mem` narrative/fact mapping, `memory_fact_sources` write links, batching/backoff, and cost/storage estimates.
...
- [ ] Extend eval honesty beyond the mock foundation with live search wiring, full private fixture coverage, release/weekly held-out gates, and no held-out tuning outside release/weekly eval jobs.
...
- [ ] Wire fact-source write paths, legacy rendering/search behavior, and Surface A (`MEMORY.md`) bounded dry-run bridge.
...
- [ ] Extend retrieval only after eval/backfill with rank-bucket boosts, drawer-aware search, drawer-grep enrichment, source-neighbor hydration, capped additive boosts, MCP drawer get/search parity, full `memory_dedup_check` skip/merge scoring, and per-tier injector token budgets.
...
- [x] **Wire build provenance + verification.** `build-images.yml` publishes signed attestations for both GHCR images via `actions/attest-build-provenance@v2` (with `id-token: write` + `attestations: write`); `deploy-fleet.yml` resolves the tag→digest via `docker buildx imagetools inspect` and runs `gh attestation verify oci://...@<digest> --repo $repo` in the `validate` job before any deploy job pulls the image. A `skip_attestation_verification` workflow_dispatch boolean is reserved for incident recovery. Runbook: `docs/FLEET_ATTESTATIONS.md` *(PR #575)*.
