...
> Last updated: 2026-04-16 (post-#596 PM2 token passthrough + stale version fix). PR #596 added `SYNC_PEER_REGISTRATION_TOKEN` to the explicit PM2 `env` blocks in `ecosystem.beta.config.cjs` and `ecosystem.mesh.config.cjs` (fixing one-way peering caused by PM2's explicit env overriding inherited `process.env`) and updated `LOCAL_APP_VERSION` from v0.5.0 to v0.5.1 in `mesh-version.ts` (fixing a false "mixed mesh version" drift banner). Latest mesh landing set: PR #550 persisted peer ping diagnostics, PR #551 added in-place mesh peer editing, PR #552 added the rate-limited signed reverse-registration endpoint, PR #553 registered 33.9-33.11 planning, PR #554 added machine-origin provenance badges, PR #555 added nullable peer-version columns, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #557 added mesh envelope schema/protocol metadata plus apply-side compat gates and `docs/MESH_COMPAT.md`, PR #558 added `/mesh-peers` version/drift UI plus the lazy-loaded sidebar peer-version tooltip, PR #560 renumbered the peer-version migration from duplicate `0024` to `0025`, PR #561 persisted peer build/schema metadata from on-demand and background health pings, PR #562 added the missed background health-persistence regression and docs sync, PR #563 added the manual signed peer-update route plus `/mesh-peers` Update action with route-local rate limiting ahead of auth, PR #565 restored drizzle journal parity through migrations `0021`-`0025`, and PR #564 added auto reverse-registration on peer add plus one-way retry UI and journal entry `0026`. Earlier 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
> Current checkpoint: PR #470 through PR #565 plus the v0.4.0 version bump and version-bump SIGPIPE hotfix are represented on `main@ff4cbd34`. GitHub reported 0 open code-scanning, 0 open Dependabot, and 0 open secret-scanning alerts after the post-#562 check; refresh alerts again after this docs sync lands. Post-#563 main checks passed: CI `24447583242`, Security Audit `24447583305`, and Build & Publish Docker Images `24447583259`. Post-#565 main checks passed: CI `24447948909`, Security Audit `24447948922`, and Build & Publish Docker Images `24447948857`. Post-#564 main checks passed: CI `24448232134`, Security Audit `24448232146`, and Build & Publish Docker Images `24448232138` (Docker publish still reports the known nonblocking high-severity scanner annotations while concluding success).
>
...
>
> Current follow-up note: this pass records PRs #550-#565 across mesh peer diagnostics/editing/reverse-registration/provenance/version-schema/build-metadata/schema-protocol-compat/version UI, manual peer update, migration-journal parity, and auto reverse-registration. The duplicate `0024` peer-version migration was renumbered to `0025_sync_nodes_peer_version.sql`, and the drizzle journal now includes entries through `0026_sync_nodes_reverse_registration`. The latest landed main is `ff4cbd34`; beta/dev CD state remains untouched.
