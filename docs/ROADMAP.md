...
> Last updated: 2026-04-14 (PR #498 new `/agent-profiles` CRUD web page on top of the existing backend, after PR #497 routing API input-bounds hardening, PR #496 dev-tier env-up portability, PR #495 agent settings E2E coverage, and PR #499 backend-independent `/machines` list/detail coverage). The 2026-04-13/14 landing set includes PR #395 through PR #499, including the latest memory/web follow-ups: PR #457 (`/memory/import` wizard completion/cancellation coverage), PR #458 (`/settings` notification preferences backend-independent coverage), PR #459 (`/discover` grouped/filter/import/new-session coverage), PR #460 (roadmap checkpoint sync), PR #461 (`/memory` index redirect/sidebar coverage), PR #462 (`/memory/consolidation` queue/action coverage), PR #465 (`/memory/scopes` broader render/create/rename/promote/merge/delete/error/empty coverage), PR #466 (`/memory/graph` table/SVG/detail/empty coverage), PR #467 (webhook delivery history viewer over the existing delivery-list API), PR #468 (`/spaces/[id]` + `/tasks/[id]` detail-route coverage), PR #470 (`/memory/synthesis` structural-lint page over `POST /api/memory/synthesis`), PR #471 (Settings -> Notifications **Registered Push Devices** section over `/api/mobile-push-devices`), PR #472 (memory fact thumbs-up / irrelevant / outdated feedback buttons over `PATCH /api/memory/facts/:id/feedback`), PR #473 (`/memory/maintenance` page over `POST /api/memory/maintenance`), PR #474 (tasks auto-decompose action wiring `POST /api/decompose` + `/preview` with two-step preview/apply UX), PR #475 (browser coverage for memory synthesis, webhook deliveries, and push-device registration/revoke flows), PR #476 (shared ConfirmDialog primitive plus memory dashboard first-load/error states), PR #478 (browser coverage for memory maintenance and task auto-decompose, including the stale-preview guard), PR #479 (removal of stale agent-worker discovery type stubs now that shared `DiscoveredSkill` / `DiscoveredMcpServer.configFile` types are canonical), PR #480 (follow-up to PR #477 fixing roadmap checkpoint SHA, Unified Memory UI wording, and approval push plan ledger row), PR #481 (web `lib/api.ts` split from a 2174-line file into 11 domain modules plus a thin barrel, with no public `@/lib/api` import-surface change), PR #482 (dry-run-first `scripts/db-provision-tier.ts` helper for provisioning dev-1/dev-2 PostgreSQL databases plus least-privilege app roles, with beta/prod tier refusal), PR #483 (`/mesh-peers` Add peer dialog and per-row Delete confirmation wired to `POST`/`DELETE /api/sync/peers`, with self-peer delete disabled and create/validation/delete Playwright coverage), PR #486 (`/memory/browser` provenance filters for session/agent/machine IDs over the existing facts API), PR #487 (auto-decompose apply-failure browser regression coverage), PR #488 (webhook delivery-list GET failure and manual Retry browser regression coverage), PR #489 (roadmap checkpoint sync through `main@c01300b2`), PR #490 (Agent Run Quality summary sync through `main@d868742b`), PR #491 (roadmap checkpoint sync through `main@f3ad66dc`), PR #492 (backend-independent `/agents/[id]` detail/start coverage through `main@8b716d6f`), PR #493 (roadmap checkpoint sync through `main@808556b0`), PR #494 (backend-independent `/agents` list/create/start coverage), PR #495 (backend-independent `/agents/[id]/settings` coverage), PR #496 (portable dev-tier `env-up` locking and `WEB_PORT`-safe Next startup), PR #497 (routing API payload bounds hardening), PR #498 (new `/agent-profiles` CRUD web page — list, create dialog, delete confirm, sidebar nav, and `lib/api` module — frontend shipped on top of existing backend), and PR #499 (backend-independent `/machines` list/detail coverage). CodeQL `#579` remains formally dismissed as a false positive after fresh `main` analysis still flagged the real `@fastify/rate-limit` route shape; local source-shape coverage keeps the limiter before `authorizeManualTakeover`.
>
> Current checkpoint: PR #470 through PR #499 are represented on `main@3eebce46` after the PR #484/#485 roadmap syncs, PR #486 memory browser provenance merge, PR #487 task auto-decompose apply-failure coverage merge, PR #488 webhook delivery retry coverage merge, PR #489 roadmap checkpoint sync, PR #490 Agent Run Quality row sync, PR #491 post-#490 checkpoint sync, PR #492 agent detail browser coverage merge, PR #493 post-#492 checkpoint sync, PR #494 agents-index coverage, PR #495 agent-settings coverage, PR #496 env-up portability, PR #497 routing API input-bounds hardening, PR #498 `/agent-profiles` CRUD web page, and PR #499 machines list/detail coverage. GitHub reports 0 open code-scanning alerts, 0 open Dependabot alerts, and 0 open secret-scanning alerts at this checkpoint.
>
> Coordination note: PR #461 merged as `e6fd4c2e`, PR #462 as `193e6b02`, PR #465 as `2b950c96`, PR #466 as `9fbc2fe9`, PR #467 as `620de798`, PR #468 as `4291f520`, PR #470 as `5a480a23`, PR #471 as `2d61cb8a`, PR #472 as `7b2f9b8c`, PR #473 as `22fbe6ae`, PR #474 as `a8189309`, PR #475 as `1f4007c1`, PR #476 as `fa632acd`, PR #477 as `be1765d3`, PR #478 as `082deedb`, PR #479 as `9f984523`, PR #480 as `0f9fb538`, PR #481 as `0668db95`, PR #482 as `04e1e80d`, PR #483 as `88ccf816`, PR #485 as `071b7e3d`, PR #486 as `bfd9e74f`, PR #487 as `f28a2a85`, PR #488 as `eb9ae933`, PR #489 as `c01300b2`, PR #490 as `d868742b`, PR #491 as `f3ad66dc`, PR #492 as `8b716d6f`, PR #493 as `808556b0`, PR #494 as `95e7c8a2`, PR #495 as `5635abdf`, PR #496 as `64de9152`, PR #497 as `1b95f9f8`, PR #498 as `3eebce46`, and PR #499 as `05972feb`; duplicate PRs #463 and #464 were closed after their stronger sibling branches merged.
...
**7,255+ unit tests** across 111+ files + **207+ Playwright e2e checks**, with the backend-independent webhooks slice now gated in CI. All packages build and lint cleanly (TypeScript 0 errors, Biome 0 errors).
...
- [x] Agent Profiles web UI: `/agent-profiles` CRUD page — list, create dialog, delete confirm, sidebar entry, plus new `lib/api` agent-profiles module *(PR #498; frontend shipped on top of existing backend)*
...
- [x] Enhanced keyboard shortcuts *(PR #250; slash-search focus regression coverage in PR #446; Sessions `?` help overlay in PR #449; shared Sidebar/Sessions `?` event ownership centralized in PR #450)*
...
- [x] Backend-independent `/agents` index coverage for list render, machine-name mapping, search/status filters, `POST /api/agents/:id/start` prompt wiring, and create-from-scratch `POST /api/agents` payload capture *(PR #494; see [plans/2026-04-14-agents-list-e2e-plan.md](plans/2026-04-14-agents-list-e2e-plan.md))*
...
| [dev-environment-cd-strategy](plans/2026-03-12-dev-environment-cd-strategy.md) | Delivered — PR #445 closed the deferred web `WEB_PORT` package-script gap; PR #482 added the dry-run-first dev-1/dev-2 database/role provisioning helper while keeping actual provisioning as a user/admin step | 12.0-12.5 |
...
| [agents-list-e2e-plan](plans/2026-04-14-agents-list-e2e-plan.md) | Delivered in PR #494 — backend-independent `/agents` index coverage for list/search/status filters, start prompt wiring, and create-from-scratch payload capture | 20.5 |
