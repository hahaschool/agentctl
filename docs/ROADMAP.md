...
> Last updated: 2026-03-15 (stability/security cycle PRs #167-#180 landed on `main`; remaining alert backlog is legacy path/rate-limit plus dependency/base-image triage)
...
- Status note: `main` was re-stabilized through PRs #167-#180. Discovery/worktree hardening plus the `agents.ts` framework limiter and `mcp-discover.ts` safe file reads are now on `main`; the remaining backlog is the older `path-security.ts` / `git.ts` path findings, legacy missing-rate-limit findings, loop-controller resource exhaustion, and dependency/base-image triage.
...
| **P0** | Agent Run Quality | 16.1 | In progress: stability/security fixes through PR #180 are on `main`; remaining alert work is legacy path/rate-limit findings plus dependency/base-image sweep |
...
| [main-stability-and-security-cycle-plan](plans/2026-03-15-main-stability-and-security-cycle-plan.md) | Active — PRs #167-#180 landed; remaining alert triage continues here | 16.1-16.3 |
