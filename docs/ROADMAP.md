...
> Last updated: 2026-04-21 (post-#709 -- PR #705 synced the roadmap/plan checkpoint for #702-#704; PR #707 wired the existing production `EmbeddingClient` through `createServer` into `memoryDrawerRoutes`, so control-plane drawer search now uses keyword + pgvector RRF fusion when `LITELLM_URL` is configured and falls back to keyword-only when it is not; PR #709 added the web MemPalace drawer search page on top of the drawer search/get APIs. Open follow-ups are tracked separately: PR #706 hardens drawer search/get behavior after #707, PR #708 repairs claude-mem fact source-key/span edge cases after #703, and PR #710 hardens the peer-update migration and post-reload logging path after #697.)
>
> Current checkpoint: PR #470 through PR #709 plus version bumps v0.5.2-v0.8.0 are represented on `main@c97b61df`. PR #707 removes the old roadmap caveat that drawer search was keyword-only in production; vector fusion is now wired from `index.ts` whenever `LITELLM_URL` constructs an embedding client. PR #709 adds the first web drawer-search surface. Review-gated follow-ups #706, #708, and #710 remain open and should be merged before the next post-queue roadmap checkpoint. These changes do not alter beta/dev/prod CD routing.
>
...
>
> Current follow-up note: this pass records PRs #705, #707, and #709 while noting open follow-ups #706, #708, and #710. The MemPalace drawer stack now has worker contracts, CP search/get backend routes, production embedding-client wiring, and a web drawer-search page; remaining near-term hardening is strict search edge behavior (#706), source-span repair (#708), and peer-update migration/reload safety (#710).
