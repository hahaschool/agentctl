...
- [ ] **PR B** — `resolveEmbeddingClient()` factory with 60s TTL cache + provider-invalidation bus, `memory-providers` route (CRUD + switch-mode + signed ephemeral token), audit logger, memory read/write rewiring to use factory, `MEMORY_OPS_STATUS_MAP` in `controlPlaneErrorToStatus()`. *(branch: `agent/claude-1/feat/memory-ops-pr-b`)*
