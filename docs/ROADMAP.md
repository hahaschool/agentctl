...
| **P1** | Unified Memory System UI | 4.8 | Partial — route family, facts browser, dashboard, claude-mem import, hardened basic JSONL history import, basic graph/table, deterministic reports, synthesis lint, maintenance, and core API routes exist. In-flight: JSONL import job provenance plus rollback UI/API on `codex/memory-import-resume-rollback`. Not complete: durable import resume beyond active in-memory jobs, edge generation from imported observations, embedding-similarity dedup, rich graph Timeline/Clusters/focus/time-lapse, LLM narrative reports, and full consolidation merge/resolution workflows. |
