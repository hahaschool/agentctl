...
- [ ] Start with eval + drawer schema/chunker/sanitizer/backfill only: `0030_add_memory_drawers.sql`, no `wing`/`room` hierarchy, non-unique content hashes, raw-secret redaction before hash/embed/store, and resumable JSONL/claude-mem drawer backfill.
