...
> MemPalace-inspired evolution plan v1.2: [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md) plus supplement [plans/2026-04-15-mempalace-additional-findings.md](plans/2026-04-15-mempalace-additional-findings.md)
>
...
- [ ] Keep eval honest with seed-42 dev/held-out split discipline, per-category reporting, known failure-mode fixtures, and no held-out tuning outside release/weekly eval jobs.
...
- [ ] Extend retrieval only after eval/backfill with query hygiene, rank-bucket boosts, drawer-aware search, drawer-grep enrichment, source-neighbor hydration, capped additive boosts, MCP drawer get/search parity, and per-tier injector token budgets.
...
- [ ] Extend memory edges with temporal validity/source references only after reviewing mesh `sync_capture_change()` payload compatibility, schema-version gates, and entity canonicalization/backfill requirements.
...
### 33.7 Mesh Peer UX Overhaul (P1) — In Progress
...
| **P1** | Mesh Peer UX Overhaul | 33.7 | In progress — PR #550 delivered backend/persistence for ping failure diagnostics and PR #551 delivered in-place peer editing. Remaining work: render diagnostics in the UI, add Tailscale-backed `/discover` + `/probe` endpoints, and add the remaining discovery/probe/ping-detail browser coverage. See [plans/2026-04-15-mesh-peer-ux-overhaul-plan.md](plans/2026-04-15-mesh-peer-ux-overhaul-plan.md). |
| **P0** | Mesh Bidirectional Registration + Cross-Node Visibility | 33.8 | In progress — PR #552 delivered the rate-limited bootstrap-token plus signed-envelope `POST /api/sync/peers/register` endpoint, PR #554 delivered machine-row provenance plus `/machines` origin badges, and PR #564 delivered auto reverse-registration on peer add plus the `/mesh-peers` one-way badge/retry action. Remaining work: prove two-node machine replication end-to-end, add the mesh health panel, and cover the two-node failure path in browser/integration tests. See [plans/2026-04-15-mesh-bidirectional-registration-plan.md](plans/2026-04-15-mesh-bidirectional-registration-plan.md). |
...
| **P1** | MemPalace-Inspired Memory Evolution | 3.6 / 4.8 / 7.3 | Planned — v1.2 keeps the existing PostgreSQL-native memory core and starts with eval + sanitized verbatim drawers + resumable backfill before search changes. It explicitly covers migration numbering from `0028`, mesh sync compatibility, raw-secret redaction, non-unique drawer hashes, embedding version stamping/rotation, Surface A bridge, audit logging/shared redaction keys, retention, injector token budgets, PreCompact timeout contracts, query hygiene, rank-bucket boosts, held-out eval discipline, MCP drawer parity, entity canonicalization, and timeline gates. See [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md). |
