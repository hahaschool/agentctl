...
| **P0** | Agent Run Quality | 16.1 | Current loop closed through PR #802; latest main CI, Security Audit, Build & Publish, code scanning, Dependabot, and secret scanning are green/empty. Historical stability work through PR #797 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. The latest delta is PR #800 deferred PreCompact capture startup, PR #801 preserved legacy no-provenance memory compatibility, and PR #802 surfaced memory-eval fixture coverage while keeping failed-run artifacts visible. |
