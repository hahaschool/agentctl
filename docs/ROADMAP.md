...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, and PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract; remaining work is injector result modes/wrapper plumbing, merge-time provenance preservation, the bounded Surface A (`MEMORY.md`) dry-run bridge, and any residual legacy search/sync compatibility.
