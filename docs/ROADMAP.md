...
> Last updated: 2026-04-25 (post-#862 roadmap sync).
>
> Current checkpoint: `main@0d507c5d` is current through PR #862 after PR #861's memory timeline query-alias compatibility and PR #862's worker-side MCP timeline route. PR #849 qualifies `claude-mem` Memory Import `import_source_id` values with a stable source key so overlapping observation IDs from different local databases no longer collide while repeated imports from the same source still skip. PR #851 adds aggregate-only `pnpm memory:eval:fixture-check` coverage/changelog/split validation without printing fixture contents. PR #852 adds local-only `memory_entities` and `memory_entity_aliases` tables plus store helpers without mesh sync triggers or historical fact/edge rewrites. PR #853 documents the sync compatibility gate for future drawer payload and temporal edge changes. PR #854 wires `pnpm memory:eval:fixture-check` into `.github/workflows/memory-evals.yml` before live eval runs and uploads only aggregate fixture-check reports. PR #855 adds `pnpm memory:canonicalization:propose` for dry-run JSON/JSONL candidate rows plus alias JSON/JSONL or local DB alias reads, with JSON/CSV resolved/ambiguous/unresolved output, bounded sanitized previews, CSV formula hardening, and no `--apply`/`--execute` mutation path. PR #857 adds `pnpm memory:eval:fixture-scaffold` for gitignored private held-out/full fixture authoring with excluded placeholder rows and a fixture changelog scaffold. PR #858 adds `GET /api/memory/timeline` as a bounded direct-edge, fact-validity-window timeline read path with opaque cursors and no schema/mesh payload expansion. PR #859 adds dry-run `pnpm memory:ops:preflight` checks for dev-1/live rollout readiness without starting jobs, mutating the DB, or calling live providers. PR #860 synced the roadmap/plan state through those landings. PR #861 lets `GET /api/memory/timeline` accept equivalent `as_of` or `asOf` timestamp aliases while rejecting conflicting aliases. PR #862 adds worker `POST /api/mcp/memory-timeline` registration with alias, limit, cursor, cold-start empty-response, and unreachable-control-plane contracts. The historical PR #779-#797 Memory Operations provider-backend checkpoint remains recorded in the coordination addenda below; current unresolved memory gaps are listed in the follow-up note and Memory UI rows.
>
...
- [ ] Extend retrieval only after eval/backfill with rank-bucket boosts, drawer-aware search, drawer-grep enrichment, source-neighbor hydration, capped additive boosts, MCP drawer get/search parity, full `memory_dedup_check` skip/merge scoring, richer result modes beyond the bounded `fact-plus-snippet` / first `full-drawer` slices, and additive accounting for future source-neighbor hydration beyond current evidence rendering. PR #869 adds the first rank-bucket boost layer (entity-type +0.03, recency +0.05/+0.02, scope-exact +0.02, cap 0.10) applied after RRF fusion.
...
- [ ] Extend memory edges with temporal validity/source references and `memory_traverse` only after reviewing mesh `sync_capture_change()` payload compatibility, schema-version gates, and entity canonicalization/backfill requirements; PR #840 adds the first pure helper and extraction benchmark, PR #852 adds the local-only canonical entity schema/store, PR #855 adds dry-run proposals, PR #858 adds the first read-only `GET /api/memory/timeline` slice over fact validity windows/direct edges without schema or mesh payload changes, PR #861 hardens equivalent `as_of`/`asOf` timeline aliases, and PR #862 adds the worker-side MCP timeline route. Human-approved apply/runtime wiring, dedicated edge temporal fields, UI timeline wiring, and edge-level recursive/as_of behavior remain open.

### 3.7 Memory Operations UI — P0 ✅
...
- [x] Consolidation completeness — richer MergePreviewPanel for near-duplicate/contradiction items with survivor selection and toast feedback, combining editable resolution and pre-selected winner flows *(PR #867)*.
...
| **P1** | MemPalace-Inspired Memory Evolution | 3.6 / 4.8 / 7.3 | In progress — current through PR #862 on `main@0d507c5d`. Delivered slices include the eval harness/live wiring and workflow-owned private fixture gates plus scaffold tooling (PRs #655/#660/#722/#729/#796/#802/#815/#833/#835/#851/#854/#857), drawer schema/backfill/import durability and structured `claude-mem` import work (PRs #671/#682/#686/#721/#836/#841/#842/#845/#847/#849), provenance/injection/Surface A approval-gated writes plus full-drawer evidence budgeting (PRs #684/#728/#738/#748/#828/#830/#832), Memory Operations provider/job/UI/consolidation workers plus rollout preflight (PRs #797/#806-#808/#812/#816/#820/#827/#859), and Phase 6 entity/timeline groundwork through helpers/benchmark, local-only schema/store, dry-run proposal tooling, first read-only timeline API slice, timeline alias compatibility, and worker MCP timeline route (PRs #840/#852/#855/#858/#861/#862). v1.3 keeps the existing PostgreSQL-native memory core; the sync precondition for future drawer payloads and temporal edge fields is explicit in [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). Remaining work continues with operator application of validated eval secrets, richer private held-out/full examples, Memory Browser evidence filters/default-on rollout criteria, raw-source filters, fuller why-this-matched explanations, Diary/Timeline result treatment, rank-bucket boosts, retention, edge-level `memory_traverse`/timeline behavior, dedicated temporal edge fields, UI timeline wiring, human-approved canonicalization apply/runtime wiring beyond PR #855's dry-run proposals, embedding-similarity dedup, dev-1 backfill verification, and Gate 2/live memory-operations rollout verification after PR #859 preflight. See [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md) and [plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md](plans/2026-04-25-memory-mesh-temporal-sync-compat-contract.md). |
...
| **P0** | Agent Run Quality | 16.1 | Current loop is closed through PR #862. PR #861 added memory timeline `as_of`/`asOf` alias compatibility, and PR #862 added the worker MCP `memory_timeline` route after the post-#859 roadmap sync; post-merge main CI `24931385074`, Security Audit `24931385101`, and Build & Publish Docker Images `24931385068` passed at `0d507c5d`. There are no open PRs during this sync, and code-scanning, Dependabot, and secret-scanning alert lists report 0 open alerts. Historical stability work through PR #860 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. |
