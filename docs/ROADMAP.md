...
2. **Local-Only Mesh Config + Dynamic Provider (P0):** New `mesh_local_config` table (NOT synced — no trigger). `MeshConfigProvider` replaces static startup opts. Authenticated `GET/PUT /api/mesh/config`. Settings → Mesh UI with write-only token UX (browser `crypto.getRandomValues`, not Node `crypto.randomBytes`).
...
| **P0** | Mesh Peering UX & Reliability Overhaul | 33.12 | In progress — Phase 0: ✅ PR #610 (version-bump.sh fix + Drizzle schema drift + deprecate old discover). Phase 1: ✅ PR #617 (Tailscale IP auto-detect with CGNAT validation + selfSyncUrl on /health). Phase 2: local-only `mesh_local_config` table + `MeshConfigProvider` + Settings → Mesh UI. Phase 3: ✅ PR #623 (structured mesh error codes + preflight token check + SSRF/ReDoS guards + migration 0029). Phase 4: ✅ PR #623 (reverse-registration retry + self-identity card + stale peer cleanup + async peer update with SSE log streaming + PeerUpdateLogModal). See [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md). |
