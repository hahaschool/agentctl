...
>
> Current follow-up note: the remaining MemPalace work is now narrower than the older PR #716-#723 checkpoint implied. Drawer-aware fusion hardening, fact-detail provenance reads, eval split gating/reporting, the first script-only Surface A dry-run bridge, and PR #736's bounded `fact-plus-snippet` injection mode are all covered on `origin/main`; this branch advances the Surface A bridge from JSON-only input to a paged control-plane API fact source. The next honest gaps remain private fixture secret provisioning/changelog discipline, richer failure-mode-targeted eval examples, true `full-drawer` result handling plus deeper additive budget accounting, merge-time provenance preservation, broader evidence UI/default-on rollout criteria, and turning the dry-run bridge into a write-gated approval flow.
...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734/#735/#736: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator, PR #735 fixes its Claude project memory target path, and PR #736 adds bounded default-off PostgreSQL `fact-plus-snippet` injection with a `full-drawer` fallback; the current follow-up branch adds a paged control-plane API fact source for the Surface A dry run. Remaining work is true `full-drawer` behavior, merge-time provenance preservation, promotion of the dry-run bridge into the final write-gated approval flow, and any residual legacy search/sync compatibility.
