...
- [ ] **Authenticated bidirectional handshake.** When `POST /api/sync/peers` creates a new peer on node A, A signs a `register-peer` envelope with its Ed25519 key (already used by `peer-auth.ts`) and calls `POST /api/sync/peers/register` on the target. This endpoint cannot rely only on the existing sync-route auth hook, because that hook trusts keys already present in `sync_nodes` and the incident target only had its self row. The plan must pick an explicit bootstrap trust model before implementation — for example Tailscale ACL/tag-gated TOFU with fingerprint confirmation, an enrollment token, or target-initiated key confirmation. After bootstrap trust passes, the target validates A's signed envelope, may corroborate A's Tailscale IP from the request source or `tailscale status --json`, and upserts the reverse row with `sync_status = 'unknown'` until its own ping confirms reachability. The target responds with its own identity so A can fill in `publicKey` if missing.
...
- [ ] **Fleet view cross-node badge.** Add explicit sync provenance for machine rows if needed (for example `origin_node_id` or `synced_from_machine_id`), then badge `/machines` rows with `Synced from <peer hostname>` vs `Local` so the operator can see which worker lives where.
...
| [mesh-peer-ux-overhaul-plan](plans/2026-04-15-mesh-peer-ux-overhaul-plan.md) | Planned — turns the 2026-04-15 beta peer-add incident into ping diagnostics, per-row Edit, Tailscale discovery, manual probe auto-fill, and `/mesh-peers` browser coverage without changing beta/CD workflows | 33.7 |
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | Planned — makes peer registration bidirectional, adds explicit machine sync provenance for cross-node visibility, and requires two-node verification that machines replicate after a peer handshake | 33.8 |
