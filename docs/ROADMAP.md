...
- [ ] **PR F** — `/memory/operations` page: `memoryOpsApi` web client, `MissingEmbeddingAlert` (8 view mounts), `JobCard`, `EgressConfirmationDialog` (preview → confirm flow), `MixedModelsBanner`, `MemoryOperationsPage` orchestrator, sidebar Operations nav item. `MEMORY_OPS_ENABLED=true`. *(branch: `agent/claude-1/feat/memory-ops-pr-f`)*
...
- Preview endpoint returns `{ snapshot, previewToken }` — user confirms with token to avoid circular dependency
