...
- [ ] **PR E** — Workers: `embedding-backfill` (batch loop, cancel polling, fail-ratio guard), `drawer-backfill` (sourceRoot containment, symlink escape detection), BullMQ Worker + boot reconciliation, integration test. `MEMORY_OPS_ENABLED_KINDS=embedding-backfill,drawer-backfill`. *(branch: `agent/claude-1/feat/memory-ops-pr-e`)*
