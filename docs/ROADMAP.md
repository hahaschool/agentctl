...
> Last updated: 2026-04-23 (post-#734 merge + dry-run path-fix follow-up).
>
> Current checkpoint: `origin/main@fbf27801` is current through PR #734. Recent memory/security landings now include PR #728 fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 workflow-owned memory-eval automation and staged private-fixture docs, PR #730 legacy empty-provenance stabilization, PR #731 roadmap sync after PR #730, PR #732 shared `sanitizeName()` hardening across control-plane and worker memory routes, PR #733 merged the `@xmldom/xmldom` + `uuid` security override fix onto `main`, and PR #734 added the first script-only `MEMORY.md` dry-run generator that preserves human-curated content while proposing bounded generated sections from reviewed facts. This checkout also carries a follow-up fix so Claude project memory resolves under `~/.claude/projects/<encoded>/memory/MEMORY.md`.
>
> GitHub triage on 2026-04-23 found `0` open PRs and `0` open CodeQL, Dependabot, and secret-scanning alerts. The latest `CI`, `Build & Publish Docker Images`, and `Security Audit` runs on `main` all succeeded after PR #733. These changes do not alter dev-1/dev-2, beta, or production CD routing.
>
...
>
> Current follow-up note: the remaining MemPalace work is now narrower than the older PR #716-#723 checkpoint implied. Drawer-aware fusion hardening, fact-detail provenance reads, eval split gating/reporting, and the first script-only Surface A dry-run bridge are all landed locally. The next honest gaps are private fixture secret provisioning/changelog discipline, richer failure-mode-targeted eval examples, real `full-drawer` result handling plus deeper additive budget accounting, merge-time provenance preservation, broader evidence UI/default-on rollout criteria, and promoting the current JSON-input dry-run bridge into the final DB/API-backed approval flow.
...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, and PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator; remaining work is injector result modes/wrapper plumbing, merge-time provenance preservation, promotion of the current JSON-input dry-run bridge into the final DB/API-backed approval flow, and any residual legacy search/sync compatibility.
