...
**Motivation:** Adding a mesh peer today requires SSH to each machine to set env vars (`TAILSCALE_IP`, `SYNC_PEER_REGISTRATION_TOKEN`), rebuild, and restart PM2. A 2026-04-16 live fleet exercise (laptop + macmini) exposed that configuration brittleness — wrong IPs, missing env vars, PM2 env shadowing — accounts for the majority of peering failures. The mechanical flow (add → reverse register → report result) works; the gap is auto-detection, web-editable config, and error clarity.

**Scope (4 phases):**

1. **Tailscale IP Auto-Detection (P0):** Auto-detect via `tailscale ip -4` at startup using existing `resolveTailscaleBin()`. Expose `selfSyncUrl` + source on `/health`. Eliminates the manual `TAILSCALE_IP` env var for most setups.
2. **DB-Stored Mesh Config + Settings UI (P0):** Extend existing `settings` table with mesh-prefixed keys (IP override, sync URL override, registration token). New `GET/PUT /api/mesh/config` endpoints. New Settings → Mesh section with token generation and source-badge display.
3. **Add Peer Flow Improvements (P0):** Pre-flight token compatibility check in the probe step. Frontend error-code-to-actionable-message mapping for all reverse registration failures. Backend automatic retry with exponential backoff (30s/60s/120s).
4. **Operational Improvements (P1):** Self-identity card on `/mesh-peers`, stale peer cleanup (>7 days).

**Success criteria:** (1) New operator can add a peer entirely from the web UI. (2) "Discover → Add → Bidirectional" < 30s with clear feedback. (3) Every reverse registration failure shows an actionable fix.
...
| **P0** | Mesh Peering UX & Reliability Overhaul | 33.12 | Planned — auto-detect Tailscale IP, DB-stored mesh config + Settings → Mesh UI, pre-flight token compat check, actionable error messages, backend retry with backoff, self-identity card, stale peer cleanup. See [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md). |
...
| [mesh-peering-ux-reliability-overhaul-plan](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) | Planned — Tailscale IP auto-detection, DB-stored mesh config via extended `settings` table, Settings → Mesh UI, pre-flight token compat in probe, frontend error-code mapping, backend retry with backoff, self-identity card, stale peer cleanup | 33.12 |
