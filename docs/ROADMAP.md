...
- [ ] Add resumable JSONL/claude-mem drawer backfill plus memory drawer audit/backfill state after the schema/store foundation.
...
- [ ] **End-to-end canary dry-run.** Run `deploy-fleet.yml --dry-run` against the populated inventory, then once for real with `strategy: canary`, `canary_percentage: 20`, against a non-critical target. The preparatory safety slice now keeps `deploy-fleet.yml` and `fleet-bootstrap.ts` scoped to `labels.topology: docker` and documents the exact dry-run/canary invocation sequence; the live workflow runs remain to be executed by an operator.
...
| **P1** | Fleet Rollout & Peer Auto-Update (two-topology) | 33.11 | In progress — PM2 topology: PR #563 (manual update), PR #571 (`agentctl peer update` CLI), PR #572 (opt-in schedulers), PR #577 (settings UI), PR #580 (rollback). Docker topology: PR #575 (build provenance), PR #579 (migration gate), PR #593 (machines.yml fleet structure). Cross-topology: PR #570 (version-compat API), PR #582 (update-available banner), PR #654 (opt-in live two-node fixture plus first version-drift assertion), PR #659 (separately gated peer-update dry-run fixture assertion), PR #668 (separately gated schema-ahead fixture assertion). Remaining: end-to-end canary dry-run. See [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md). |
...
| **P1** | MemPalace-Inspired Memory Evolution | 3.6 / 4.8 / 7.3 | In progress — PR #655 delivered the Phase 0 eval foundation (fixture schema/sanitization, seed-42 split helpers, mock metrics, sanitized sample fixture, and `pnpm memory:eval` CLI), PR #660 added deterministic mock recall@5 threshold enforcement plus `pnpm memory:bench` without live DB/embedding dependencies, PR #667 locked first-run / `{ arguments: null }` MCP contracts for the current search/recall/report routes, and PR #671 delivered the Phase 1 drawer schema/chunker/sanitizer/store foundation (`0030`, shared redaction/validation, deterministic chunking, sanitized hash/embed/store tests) without drawer sync triggers or search default changes. v1.3 keeps the existing PostgreSQL-native memory core and continues with audit/backfill state + resumable backfill before search changes. It explicitly covers mesh sync compatibility, raw-secret redaction, non-unique drawer hashes, embedding version stamping/rotation, Surface A bridge, audit logging/shared redaction keys, retention, injector token budgets, PreCompact timeout contracts, three-stage query sanitizer, rank-bucket boosts, held-out eval discipline, live eval wiring, future `memory_dedup_check` / `memory_traverse` contracts, entity canonicalization, and timeline gates. See [plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md](plans/2026-04-15-mempalace-inspired-memory-evolution-plan.md). |
