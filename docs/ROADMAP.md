...
| **P1** | Mesh Peer UX Overhaul | 33.7 | In progress — PR #550 delivered ping failure diagnostics, PR #551 delivered in-place peer editing, PR #582 rendered failure details in the UI, and PR #586 delivered `GET /discover` + `/probe` backend endpoints with SSRF validation and full unit coverage. Remaining work: discover-peers UI flow, probe auto-fill in manual form, default scheme hint, and Playwright coverage. See [plans/2026-04-15-mesh-peer-ux-overhaul-plan.md](plans/2026-04-15-mesh-peer-ux-overhaul-plan.md). |
