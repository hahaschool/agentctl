...
- [ ] Extend eval honesty beyond live wiring with full private fixture coverage, release/weekly held-out gates, and no held-out tuning outside release/weekly eval jobs. Narrow next slice: gate `held-out`/`full` execution behind workflow-owned env flags and document the private-fixture delivery contract before adding weekly/release workflows.
