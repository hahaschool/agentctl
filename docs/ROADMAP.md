...
>
> Current follow-up note: this pass records the audit browser-depth follow-up, the dependency-audit endpoint remediation needed after PR #514 exposed npm's retired legacy audit endpoint, and the `/machines` plus `/agents` list CI-gating follow-ups. Security Audit still emits GitHub's Node.js 20 deprecation annotation for Docker/cache/artifact actions running under the forced Node 24 runtime; the annotation is informational and independent from the dependency-audit endpoint failure.
...
**7,260+ unit tests** across 111+ files + **218+ Playwright e2e checks**, with the backend-independent webhooks, agent-profiles, audit, machines, and agents list slices now gated in CI. All packages build and lint cleanly (TypeScript 0 errors, Biome 0 errors).
...
> list spec to the same focused web e2e CI lane.
...
- [x] Backend-independent web Playwright CI gating for `/webhooks`, `/agent-profiles`, `/audit`, `/machines`, and `/agents` list using `WEB_PORT`-aware web scripts *(PRs #445, #505, #513, #516, #518)*
...
| [agents-list-e2e-plan](plans/2026-04-14-agents-list-e2e-plan.md) | Delivered in PR #494 with CI-gating follow-up in PR #518 — backend-independent `/agents` index coverage for list/search/status filters, start prompt wiring, and create-from-scratch payload capture | 20.5 |
