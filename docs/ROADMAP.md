...
### 33.10 Mesh Schema + Protocol Compat Policy (P0) — In Progress
...
- [ ] **Tests.** Unit tests for the apply gate (same / +1 / +2 / -1 schema versions and protocol window) landed in PR #557; remaining coverage is the two-node/integration proof plus `/mesh-peers` schema-ahead warning UI.
...
| **P0** | Mesh Schema + Protocol Compat Policy | 33.10 | In progress — PR #557 delivered shared envelope meta/protocol constants, producer stamping, apply-side schema/protocol gates, typed errors, legacy-envelope tolerance, `docs/MESH_COMPAT.md`, and focused unit coverage. Remaining work: persist/surface rejection reasons, show the `/mesh-peers` "Peer ahead" warning, and prove the two-node rolling-upgrade case. See [plans/2026-04-15-mesh-schema-protocol-compat-plan.md](plans/2026-04-15-mesh-schema-protocol-compat-plan.md). |
| **P1** | Fleet Rollout & Peer Auto-Update (two-topology) | 33.11 | In progress — PR #563 delivered the first PM2 manual update slice: `scripts/peer-update.sh`, signed self-update route, route-local rate limiting before auth, update mutex, structured responses, and `/mesh-peers` Update action. Remaining work: Docker fleet activation, `pnpm agentctl peer update` CLI/dry-run/provenance/health-gate/rollback history, opt-in schedulers/settings UI, and two-node dry-run coverage. Depends on the delivered 33.9 drift signal plus the remaining 33.10 operator-warning/two-node proof. See [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md). |
