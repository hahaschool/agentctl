...
> Last updated: 2026-04-13 (`main` is now at `e8607dca` after the 2026-04-13 landing set: PR #395 (session `Config` tab e2e depth), PR #396 (`/conflicts` resolution-flow e2e depth), PR #397 (mesh/runtime e2e follow-up tracking doc), PR #398 (dependabot `drizzle-orm 0.38.4 → 0.45.2`, closes `GHSA-gpj5-g38j-94v9`), and PR #401 (`chore(deps): patch vite and next CVEs` — adds `vite ">=7.3.2"` pnpm override closing `GHSA-v2wj-q39q-566r` and `GHSA-p9ff-h696-f583`, bumps `next ^16.1.7 → ^16.2.3` closing `GHSA-q4gf-8mx6-v5v3`). That clears 5 dep-audit CVEs (1 drizzle + 2 vite + 2 next); GitHub security alerts for the high-severity vite/next/drizzle findings should converge to `0` on the next scan. PR #399 was closed as a duplicate of #401 and PR #400 (CI pnpm setup for DAST fuzz scan) is still open. One moderate-only `@anthropic-ai/sdk` advisory remains out of scope for this cleanup batch.)
...
|----------|------|---------|--------|
| **P0** | ~~Emergency Stop UI~~ | — | ✅ Delivered — PR pending exposed existing emergency-stop backend routes in the web UI with confirmation dialogs, tests, and fleet-wide kill. |
