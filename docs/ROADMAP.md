...
### 33.8 Mesh Bidirectional Registration + Cross-Node Visibility (P0) — In Progress
...
- [ ] **Playwright coverage.** PR #654 added the opt-in live two-node fixture foundation and first 33.11 version-drift assertion. PR #673 added the separately gated 33.8 browser assertion for a primary-authored machine row appearing on the secondary node with its `Synced from ...` provenance badge. Remaining 33.8 browser coverage: add peer on A ⇒ sees self row on B, break handshake ⇒ UI shows one-way warning.
...
| **P0** | Mesh Bidirectional Registration + Cross-Node Visibility | 33.8 | In progress — PR #552 bootstrap-token register endpoint, PR #554 machine provenance badges, PR #564 auto reverse-registration + one-way badge/retry, PR #576 mesh health panel, PR #589 two-node machine replication integration proof (5 tests: A→B, B→A, UPDATE propagation, stale rejection, apply guard), PR #596 fixed PM2 env passthrough for `SYNC_PEER_REGISTRATION_TOKEN` (one-way peering on live fleet), PR #654 added the opt-in live two-node Playwright fixture foundation, and PR #673 added the separately gated A-to-B machine visibility browser assertion. Remaining: 33.8 browser assertions for add-peer/reverse-registration and one-way warning/retry. See [plans/2026-04-15-mesh-bidirectional-registration-plan.md](plans/2026-04-15-mesh-bidirectional-registration-plan.md). |
...
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | In progress — PR #552 signed reverse-registration endpoint, PR #554 machine provenance badges, PR #564 auto-register-on-add plus one-way retry, PR #576 mesh health summary, PR #589 two-node machine replication proof, PR #596 PM2 registration-token passthrough, PR #654 opt-in live two-node fixture foundation, and PR #673 separately gated the A-to-B machine visibility fixture assertion; remaining 33.8 scope is add-peer/reverse-registration and one-way warning/retry browser assertions | 33.8 |
