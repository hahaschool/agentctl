...
> Last updated: 2026-04-15 (post-#564/#565 mesh registration + migration-journal checkpoint). Latest mesh landing set: PR #550 persisted peer ping diagnostics, PR #551 added in-place mesh peer editing, PR #552 added the rate-limited signed reverse-registration endpoint, PR #553 registered 33.9-33.11 planning, PR #554 added machine-origin provenance badges, PR #555 added nullable peer-version columns, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #557 added mesh envelope schema/protocol metadata plus apply-side compat gates and `docs/MESH_COMPAT.md`, PR #558 added `/mesh-peers` version/drift UI plus the lazy-loaded sidebar peer-version tooltip, PR #560 renumbered the peer-version migration from duplicate `0024` to `0025`, PR #561 persisted peer build/schema metadata from on-demand and background health pings, PR #562 added the missed background health-persistence regression and docs sync, PR #563 added the manual signed peer-update route plus `/mesh-peers` Update action with route-local rate limiting ahead of auth, PR #565 restored drizzle journal parity through migrations `0021`-`0025`, and PR #564 added auto reverse-registration on peer add plus one-way retry UI and journal entry `0026`. Earlier 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
...
| **P0** | Mesh Bidirectional Registration + Cross-Node Visibility | 33.8 | In progress — PR #552 bootstrap-token register endpoint, PR #554 machine provenance badges, PR #564 auto reverse-registration + one-way badge/retry, PR #576 mesh health panel, PR #589 two-node machine replication integration proof (5 tests: A→B, B→A, UPDATE propagation, stale rejection, apply guard). Remaining: Playwright two-node fixture (docker compose). See [plans/2026-04-15-mesh-bidirectional-registration-plan.md](plans/2026-04-15-mesh-bidirectional-registration-plan.md). |
