...
> Last updated: 2026-04-21 (post-#713 -- PR #708 repaired the claude-mem session-summary backfill so new atomic facts still land under the legacy-compatible `session_summaries:<id>` source key while still deduping the short-lived #703 `:parent` key, repairs `memory_fact_sources` spans when `addFact()` retries find an existing fact, and now writes fallback provenance for every returned drawer chunk instead of only the first chunk; PR #710 replaced `scripts/peer-update.sh`'s direct `drizzle-kit migrate` call with the canonical `scripts/drizzle-migrate-apply.ts` psql-backed applier (drizzle-kit v0.31.9 silently no-ops pending SQL) and redirected post-reload probe/rollback output into `AGENTCTL_PEER_POST_RELOAD_LOG` before the final `pm2 reload` so the health-probe writer no longer SIGPIPEs on the restarting control plane, closing the 2026-04 macmini brick RCA; PR #711 synced the previous roadmap/plan checkpoint for #705, #707, and #709 while flagging #706, #708, and #710 as still-open follow-ups; PR #712 extracted the drawer keyword + pgvector + RRF(k=60) pipeline into a reusable `searchMemoryDrawers()` helper in `packages/control-plane/src/memory/memory-drawer-search.ts` and wired it into `GET /api/memory/facts?mode=semantic` behind a `MEMORY_DRAWER_FUSION=true` env flag resolved once at route registration (additive `drawerResults?: MemoryDrawerSearchResult[]` envelope field, new `drawerLimit` query param defaulting to 25 and capped at 100, drawer failures degrade to fact-only), delivering the MemPalace Phase 4 fusion-flag surface; PR #713 delivered the Phase 0 facts-only baseline snapshot by extending `scripts/memory-bench.ts` with a `--baseline facts-only` mode, stamping fixture SHA256 + git SHA + ISO timestamp, and committing the deterministic baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` (aggregate R@5 0.500, R@10 0.500, MRR 1.000, NDCG@10 0.787, drawer-only hit 0.000) alongside `docs/memory-evals/README.md` documenting regeneration, so future drawer-aware eval runs now have a real comparison baseline. Open follow-up: PR #706 still hardens drawer search/get strict-limit and archived-row behavior and must be rebased onto the post-#712 refactored `searchMemoryDrawers()` helper in `memory-drawer-search.ts` before it can land.)
>
> Current checkpoint: PR #470 through PR #713 plus version bumps v0.5.2-v0.8.0 are represented on `main@ec749b77`. PR #708 closes the claude-mem source-key/span edge cases after #703, PR #710 closes the peer-update migration/SIGPIPE RCA after #697/#702, PR #712 adds the first control-plane drawer-aware fusion surface behind a feature flag and the reusable `searchMemoryDrawers()` helper, and PR #713 commits the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json`. Review-gated follow-up #706 remains open and must rebase onto the extracted helper before it can merge; no other open PRs. These changes do not alter beta/dev/prod CD routing.
>
