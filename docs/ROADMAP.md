...
> Last updated: 2026-04-21 (post-#723 -- PR #708 repaired the claude-mem session-summary backfill so new atomic facts still land under the legacy-compatible `session_summaries:<id>` source key while still deduping the short-lived #703 `:parent` key, repairs `memory_fact_sources` spans when `addFact()` retries find an existing fact, and now writes fallback provenance for every returned drawer chunk instead of only the first chunk; PR #710 replaced `scripts/peer-update.sh`'s direct `drizzle-kit migrate` call with the canonical `scripts/drizzle-migrate-apply.ts` psql-backed applier (drizzle-kit v0.31.9 silently no-ops pending SQL) and redirected post-reload probe/rollback output into `AGENTCTL_PEER_POST_RELOAD_LOG` before the final `pm2 reload` so the health-probe writer no longer SIGPIPEs on the restarting control plane, closing the 2026-04 macmini brick RCA; PR #711 synced the previous roadmap/plan checkpoint for #705, #707, and #709 while flagging #706, #708, and #710 as still-open follow-ups; PR #712 extracted the drawer keyword + pgvector + RRF(k=60) pipeline into a reusable `searchMemoryDrawers()` helper in `packages/control-plane/src/memory/memory-drawer-search.ts` and wired it into `GET /api/memory/facts?mode=semantic` behind a `MEMORY_DRAWER_FUSION=true` env flag resolved once at route registration (additive `drawerResults?: MemoryDrawerSearchResult[]` envelope field, new `drawerLimit` query param defaulting to 25 and capped at 100, drawer failures degrade to fact-only), delivering the MemPalace Phase 4 fusion-flag surface; PR #713 delivered the Phase 0 facts-only baseline snapshot by extending `scripts/memory-bench.ts` with a `--baseline facts-only` mode, stamping fixture SHA256 + git SHA + ISO timestamp, and committing the deterministic baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` (aggregate R@5 0.500, R@10 0.500, MRR 1.000, NDCG@10 0.787, drawer-only hit 0.000) alongside `docs/memory-evals/README.md` documenting regeneration, so future drawer-aware eval runs now have a real comparison baseline; PR #716 hardened the drawer search backend post-#712 rebase by returning 404 from `GET /api/memory/drawers/:drawerId` for archived drawers via an `archived_at IS NULL` filter in `loadDrawerById`, throwing a typed `MemoryDrawerSearchDbError` from `memory-drawer-search.ts` that surfaces as HTTP 500 from `/api/memory/drawers/search` (opt-in per-caller via `degradeOnSqlError: false` so the fact-fusion caller still gets silent degradation), tightening `normalizeLimit` with a `/^\d+$/` predicate so `10abc`, `1.5`, `+5`, and `0x10` reject with 400, and raising the per-path candidate fetch to `max(CANDIDATE_LIMIT, limit)` so `limit > 50` actually reaches up to `MAX_LIMIT=100` (7 route tests + 6 helper tests), superseding the now-closed PR #706; PR #717 ported the facts-side piece #716 intentionally deferred by running `runDrawerFusion()`'s raw `q` through `sanitizeQuery()` from `@agentctl/shared` (empty/rejected verdict skips drawer fusion cleanly instead of crashing the `/api/memory/facts` envelope) and switching `drawerLimit` from loose `z.string().max(16).optional()` to a strict Zod refinement `^[0-9]+$` → `int().min(1).max(100)` that returns 400 `INVALID_DRAWER_LIMIT` for rejected values, while removing the dead `resolveDrawerLimit()` helper (11 new tests), so the drawer-aware fusion stack (Phase 4) now has both the `/api/memory/drawers/search` and `/api/memory/facts` surfaces fully hardened with no remaining open code follow-ups from this wave; PR #719 surfaced fact match-type badges in the web memory browser by extending `searchMemoryFacts` in `packages/web/src/lib/api/memory.ts` with optional `results?: MemorySearchResult[]` and `drawerResults?: MemoryDrawerSearchResult[]` envelope fields, adding a new `MatchSourceBadge` component under `packages/web/src/components/memory/`, and threading a `sourcePathByFactId` map built from `results` through `FactsList`/`FactCard` so the `MemoryBrowserView` renders per-fact vector/keyword/fusion badges as the first slice of MemPalace Phase 7 Step 1 plus a partial "why this matched" row (+4 web unit tests); PR #720 extended `InjectionBudget` in `@agentctl/shared` with an additive optional `tierTokenCaps?: Partial<Record<InjectionTier, number>>` field that `packages/control-plane/src/memory/context-budget.ts` enforces as a per-tier token ceiling after the existing global cap check, leaving `DEFAULT_INJECTION_BUDGET` unchanged and bringing context-budget tests from 32 to 37, delivering the `tierTokenCaps` half of MemPalace Phase 3 Step 6 while result modes remain a future slice; PR #721 added dry-run cost/storage estimates for memory drawer backfill by extending `scripts/backfill-memory-drawers.ts` with chunk-count, embedding-token, embedding-spend, and storage-byte estimators plus a configurable default embedding price across the JSONL, secret-sanitized, and claude-mem dry-run paths, closing the MemPalace Phase 1.5 backfill cost-estimate gap on plan line 1266; PR #722 wired an opt-in live-search mode into `scripts/memory-eval.ts` so `pnpm memory:eval --no-mock` runs the same fixture split through control-plane `MemorySearch` using `DATABASE_URL` plus embedding config while keeping mock ranking as the default and preserving the dev/held-out/full eval split guardrails (new `scripts/memory-eval.test.ts`), delivering the MemPalace PR A live-search-adapter eval gap while private fixtures and release/weekly held-out jobs remain open; PR #723 rendered the drawer-results section in the web memory browser by extracting `matchTypeClass`/`matchTypeLabel` verbatim into a new `packages/web/src/components/memory/drawerMatchType.ts` helper, adding a new `DrawerResultsSection.tsx` component plus tests, reusing the helper inside `MemoryDrawersView`, and rendering the section below `FactsList` in `MemoryBrowserView` only when `drawerResults` is non-empty, completing the drawer-result half of MemPalace Phase 7 Step 1 (+10 web tests across `DrawerResultsSection.test.tsx` and `MemoryBrowserView.test.tsx`).
>
> Current checkpoint: PR #470 through PR #723 plus version bumps v0.5.2-v0.8.0 are represented on `main@1d4ef0cd`. PR #708 closes the claude-mem source-key/span edge cases after #703, PR #710 closes the peer-update migration/SIGPIPE RCA after #697/#702, PR #712 adds the first control-plane drawer-aware fusion surface behind a feature flag and the reusable `searchMemoryDrawers()` helper, PR #713 commits the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json`, PR #716 hardens the drawer search backend (archived-row 404, typed DB error, strict limit predicate, per-path candidate cap) and supersedes the now-closed PR #706, and PR #717 ports the facts-side fusion hardening (sanitized drawer query, strict `drawerLimit` with 400 `INVALID_DRAWER_LIMIT`). PR #720 lands the `tierTokenCaps` half of Phase 3 Step 6 as an additive optional per-tier token ceiling on `InjectionBudget` enforced inside `context-budget.ts`, PR #719 and PR #723 together deliver the fact + drawer halves of Phase 7 Step 1 by surfacing the enrichment envelope in the memory browser via `MatchSourceBadge` for facts and the new `DrawerResultsSection` for drawer hits, PR #721 closes the Phase 1.5 backfill cost-estimate gap by adding chunk/token/spend/storage estimators to `scripts/backfill-memory-drawers.ts` across all dry-run paths, and PR #722 wires the `pnpm memory:eval --no-mock` live-search adapter through control-plane `MemorySearch` while keeping mock ranking default. The drawer-aware fusion stack (Phase 4) now has both the `/api/memory/drawers/search` and `/api/memory/facts` surfaces fully hardened, with no remaining open code follow-ups from this wave. These changes do not alter beta/dev/prod CD routing.
>
...
> Coordination note addendum: PR #663 merged as `d53e94af`, PR #664 as `5a433d68`, PR #665 as `2271bf72`, PR #666 as `ea82fec9`, PR #667 as `48bc709f`, PR #668 as `f77f8074`, PR #669 as `811bdf31`, PR #670 as `c1e1add6`, PR #671 as `e19dd7d5`, PR #672 as `b7273f5e`, PR #673 as `f106f4f7`, PR #674 as `bfe9759c`, PR #675 as `92ab8c97`, PR #677 as `1dfadaa3`, PR #678 as `bfbca627`, PR #679 as `eb4f19b2`, PR #681 as `a976cae2`, PR #682 as `a66c9bc1`, PR #684 as `a2cfdda8`, PR #685 as `150c3be7`, PR #686 as `b8ed8ce4`, PR #687 as `99e36894`, PR #688 as `135344e1`, PR #689 as `e6164cfa`, PR #691 as `465ebb2e`, PR #692 as `ef93b76d`, PR #693 as `2d5ea917`, PR #694 as `f0c632e4`, PR #695 as `47243f58`, PR #696 as `efb87d41`, PR #697 as `5ca38166`, PR #698 as `aa90762f`, PR #699 as `f7c15804`, PR #700 as `be1b4a83`, and PR #701 as `c9132f1b`, PR #702 as `6155f03a`, PR #703 as `524a32c5`, and PR #704 as `7ee21749`; PR #705 as `cdb3bb65`, PR #707 as `8ad648a5`, PR #709 as `c97b61df`, PR #708 as `c8e9d376`, PR #710 as `9ec3e1e2`, PR #711 as `dbd17a07`, PR #712 as `f8e9b1db`, PR #713 as `ec749b77`, PR #714 as `b6ab5277`, PR #716 as `f0b9b6fc`, and PR #717 as `8eb872cd`, PR #718 as `afca944d`, PR #719 as `eab86e12`, PR #720 as `94081135`, PR #721 as `5592d63c`, PR #722 as `38a49e82`, and PR #723 as `1d4ef0cd`.
>
> Current follow-up note: this pass records PRs #719 through #723, which push the MemPalace stack forward on the UI, injection-budget, backfill, and eval surfaces. The drawer-aware fusion stack (Phase 4) remains fully hardened on both `/api/memory/drawers/search` and `/api/memory/facts`; on top of that, fact-side match-type badges (PR #719) and the drawer-results section (PR #723) now surface the enrichment envelope to the memory browser UI as the first two slices of Phase 7 Step 1, `InjectionBudget` in `@agentctl/shared` gains an additive optional per-tier token ceiling enforced by `context-budget.ts` (PR #720, `tierTokenCaps` half of Phase 3 Step 6), drawer backfill tooling now estimates chunk count, embedding tokens, embedding spend, and storage bytes before write across every dry-run path (PR #721, closing the Phase 1.5 cost-estimate gap on plan line 1266), and `pnpm memory:eval --no-mock` now drives the eval fixture split through control-plane `MemorySearch` with mock ranking still the default (PR #722, Phase 0 PR A live-search adapter). Result-mode plumbing (Phase 3 Step 6 second half), "why this matched" rows beyond the basic match-type badges, per-tier context-budget rendering in the injector path, the `MEMORY_DRAWER_FUSION` default-on criteria on plan line 1045 (dev R@5 >= facts-only dev R@5 with no p95 regression), private fixtures plus release/weekly held-out automation, deeper Phase 7 surfaces (context-picker "include raw drawers" toggle, per-result navigation to drawer detail, observability dashboard), and Surface A dry-run generation remain open follow-ups.
...
- [x] Wire `pnpm memory:eval --no-mock` to live control-plane `MemorySearch` using `DATABASE_URL` plus embedding config, while preserving mock-by-default runs, dev/held-out/full split behavior, and the full-set guard.
- [ ] Extend eval honesty beyond live wiring with full private fixture coverage, release/weekly held-out gates, and no held-out tuning outside release/weekly eval jobs.
