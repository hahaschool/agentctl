...
- [x] Standalone `/mesh-peers` web page with sidebar entry, peer status table, 30s polling, and per-peer ping action over the existing list/ping APIs *(PR #425; upsert/delete UI intentionally deferred)*
...
| **P0** | ~~Mesh: Node Discovery + Peer Registry~~ | 33.4 | ✅ Delivered — PR #376 merged peer discovery, health check, auth, REST API, and 36 tests; PR #425 added the standalone `/mesh-peers` web read + ping surface over that registry. |
...
| **P1** | ~~Mesh / Runtime E2E Follow-up~~ | 16.1 / 31.3 / 33.3 | ✅ Delivered — PR #395 shipped the deeper session `Config` tab detail-state coverage and PR #396 shipped the `/conflicts` resolution-flow coverage; PR #397 landed the plan-tracking doc. See [plans/2026-04-01-mesh-runtime-e2e-follow-up-plan.md](plans/2026-04-01-mesh-runtime-e2e-follow-up-plan.md) |
