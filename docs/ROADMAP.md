...
> Last updated: 2026-04-25 (post-PR #842 durable Memory Import resume hotfix and roadmap/plan reconciliation).
>
> Current checkpoint: `main@7d13de12` is current through PR #842 after PR #839's dashboard quick-action accessibility CI coverage, PR #840's entity canonicalization/extraction benchmark groundwork, PR #841's durable Memory Import job state plus resume UI/API, and PR #842's restart-orphaned running-job resume fix. The historical PR #779-#797 Memory Operations provider-backend checkpoint remains recorded in the coordination addenda below; current unresolved memory gaps are listed in the follow-up note and Memory UI rows.
>
...
> post-#842 main CI, Security Audit, and Docker publish passed at `7d13de12`.
...
- [ ] Import completeness — durable job state plus restart-visible resume/rollback UX is covered through PR #841 and PR #842; this follow-up branch adds a persisted JSONL resume cursor plus same-job crash-before-checkpoint accounting so file churn and stale checkpoints no longer shift JSONL resume work into skipped rows. Edge creation from imported structured fields, broader `claude-mem` source-local idempotency, and embedding-similarity dedup remain follow-up work.
