...
> Last updated: 2026-04-16 (post-#607 discover timeout + hono vuln fix + peering plan). PR #598 added signed peer-update proxy forwarding for remote peers (33.11). PRs #599-#602 fixed mesh runtime issues: Tailscale CLI binary path on macOS, signed header passing (object not string), peer-update.sh path resolution for compiled JS, and Tailscale IP for selfSyncUrl in reverse registration. PR #603 replaced the hardcoded `LOCAL_APP_VERSION` in the web frontend with a runtime fetch from the control-plane `/health` endpoint. PR #604 documented the 33.12 peering UX & reliability overhaul plan. PR #605 fixed Biome import ordering and reply-chain formatting in `sync-peer-update.ts` that was blocking all CI. PR #606 updated `hono` from 4.12.12 to 4.12.14 (transitive via `@modelcontextprotocol/sdk`) to fix Dependabot alert #32 (moderate HTML injection in hono/jsx SSR). PR #607 increased `SESSION_DISCOVER_TIMEOUT_MS` from 5s to 15s for machines with many sessions over the mesh and prevented auto-refresh flicker on the discover page. Earlier mesh landing set (PRs #550-#565) and 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
> Current checkpoint: PR #470 through PR #607 plus version bumps v0.5.2-v0.5.4 are represented on `main@fa60a033`. GitHub reports 0 open code-scanning (alert #585 dismissed as false positive — route has rate limiting via config + preHandler), 0 open Dependabot, and 0 open secret-scanning alerts as of 2026-04-16. Major cleanup pass: 161 stale git worktrees removed (215 down to 54), reclaiming ~115GB of disk; 82 merged remote branches pruned.
>
...
>
> Current follow-up note: this pass records PRs #596-#607 covering PM2 token passthrough, mesh peer-update proxy signing, Tailscale CLI/path/header fixes, web version-from-CP runtime fetch, peering UX plan doc, Biome lint CI fix, hono transitive-dep vulnerability patch, and discover timeout/flicker fix. The latest landed main is `fa60a033` (v0.5.4); beta/dev CD state remains untouched.
