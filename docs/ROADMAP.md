...
> post-#848 main CI, Security Audit, and Docker publish passed at `8526fabe`.
...
- [ ] Import completeness — durable job state plus restart-visible resume/rollback UX is covered through PR #841 and PR #842; PR #845 adds a persisted JSONL resume cursor plus same-job crash-before-checkpoint accounting so file churn and stale checkpoints no longer shift JSONL resume work into skipped rows. PR #847 splits imported `claude-mem` `facts[]` entries into child facts and creates child-to-parent `summarizes` edges. This follow-up qualifies `claude-mem` observation and structured child-fact import ids by source key so overlapping row ids from different local databases do not collide; embedding-similarity dedup remains follow-up work.
