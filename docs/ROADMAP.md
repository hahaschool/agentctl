...
>
> GitHub triage on 2026-04-25 has the roadmap current through PR #825. GitHub reported no open PRs after PR #825 merged; post-merge main CI `24920301524`, Security Audit `24920301538`, and Build & Publish Docker Images `24920301534` passed at `b36171aa`. Direct alert-list API calls reported 0 open code-scanning, Dependabot, and secret-scanning alerts. Beta is intentionally left on the v0.8.2 promotion stack; the post-beta mainline changes are recorded here for the next full promotion rather than hot-applied into beta.
>
...
> post-#825 main CI, Security Audit, and Docker publish passed at `b36171aa`.
...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734/#735/#736/#737: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator, PR #735 fixes its Claude project memory target path, PR #736 adds bounded default-off PostgreSQL `fact-plus-snippet` injection with a `full-drawer` fallback, and PR #737 adds a paged control-plane API fact source for the Surface A dry run. PR #738 added store-level `memory_fact_sources` copying/deduping so duplicate-fact merges can preserve provenance, PR #748 wires accepted near-duplicate consolidation actions into that helper before invalidating duplicate facts, and PR #801 keeps legacy fact-only/no-provenance search and detail responses stable when preview lookup is unavailable. Remaining work is true `full-drawer` behavior and promotion of the dry-run bridge into the final write-gated approval flow.
...
- [ ] **PR G** — Consolidation + synthesis workers, 3 Playwright e2e specs (openai-happy, missing-embedding-alert, gemini-happy), Gate 2 procedure, CHANGELOG + QUICKSTART. `MEMORY_OPS_ENABLED_KINDS` expanded to include consolidation + synthesis. *(active branch: `codex/memory-ops-pr-g`)*
