...
- [x] Backend-independent web Playwright CI gating for `/webhooks`, `/agent-profiles`, `/audit`, `/machines`, `/agents` list, `/scheduler`, `/settings`, deployment, discover, conflicts, logs-security-findings, security-findings, session-detail, spaces-tasks-detail, memory-dashboard, session-config, memory-index, memory-reports, and memory-graph (24 specs / 89 tests in CI) using `WEB_PORT`-aware web scripts and a shared-package build preflight; runtime-sessions removed from CI due to WebSocket timing flakiness *(PRs #445, #505, #513, #516, #518, #533, #546, #608, #657, #662, #665, #700, #762)*
