...
- [ ] **PR E** — Workers: `embedding-backfill` (batch loop, cancel polling, fail-ratio guard), `drawer-backfill` (sourceRoot containment, symlink escape detection), BullMQ Worker + boot reconciliation, integration test. `MEMORY_OPS_ENABLED_KINDS=embedding-backfill,drawer-backfill`. *(active branch: `codex/memory-ops-pr-e`, based on post-#815 `main@b553868b`)*
...
| **P0** | Agent Run Quality | 16.1 | Current loop closed through PR #813; latest main CI, Security Audit, and Build & Publish passed after PR #813, the open PR queue is empty, and code-scanning, Dependabot, and secret-scanning alert lists report 0 open alerts. Historical stability work through PR #797 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. The latest delta is PR #800 deferred PreCompact capture startup, PR #801 preserved legacy no-provenance memory compatibility, PR #802 surfaced memory-eval fixture coverage while keeping failed-run artifacts visible, PR #803 hardened mobile evidence detail rendering, PR #806 landed the Settings -> Memory & Embeddings provider UI, PR #807 persisted saved-provider test results, PR #808 hardened the saved-provider status path against raw upstream error storage and stale metadata writes, PR #809/#810 synced and reconciled docs, PR #811 closed remaining Memory Ops/MemPalace plan drift, PR #812 delivered the Memory Operations PR D job-orchestration infrastructure, and PR #813 synced the roadmap and Memory Ops plan docs after that landing. |
