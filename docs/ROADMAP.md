...
>
> Current follow-up note: this pass records the post-#513 green checkpoint, the audit browser-depth follow-up, and the dependency-audit endpoint remediation needed after PR #514 exposed npm's retired legacy audit endpoint. Security Audit still emits GitHub's Node.js 20 deprecation annotation for Docker/cache/artifact actions running under the forced Node 24 runtime; the annotation is informational and independent from the dependency-audit endpoint failure.
...
> npm retired the legacy audit endpoints used by pnpm.
