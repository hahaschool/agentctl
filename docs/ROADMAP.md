...
> Last updated: 2026-04-23 (post-#735 + bounded result-mode follow-up).
>
> Current checkpoint: `origin/main@6b5d2f27` is current through PR #735. Recent memory/security landings now include PR #728 fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 workflow-owned memory-eval automation and staged private-fixture docs, PR #730 legacy empty-provenance stabilization, PR #731 roadmap sync after PR #730, PR #732 shared `sanitizeName()` hardening across control-plane and worker memory routes, PR #733 merged the `@xmldom/xmldom` + `uuid` security override fix onto `main`, PR #734 added the first script-only `MEMORY.md` dry-run generator that preserves human-curated content while proposing bounded generated sections from reviewed facts, and PR #735 corrected the Claude project memory target to `~/.claude/projects/<encoded>/memory/MEMORY.md`. This branch adds bounded injector result modes: default `fact-only`, PostgreSQL `fact-plus-snippet` with capped evidence lines, and explicit `full-drawer` fallback to `fact-only`.
>
> GitHub triage on 2026-04-23 found `0` open PRs and `0` open CodeQL, Dependabot, and secret-scanning alerts. The latest `CI`, `Build & Publish Docker Images`, and `Security Audit` runs on `main` all succeeded after PR #735. These changes do not alter dev-1/dev-2, beta, or production CD routing.
>
...
>
> Current follow-up note: the remaining MemPalace work is now narrower than the older PR #716-#723 checkpoint implied. Drawer-aware fusion hardening, fact-detail provenance reads, eval split gating/reporting, the first script-only Surface A dry-run bridge, and this branch's bounded `fact-plus-snippet` injection mode are all covered. The next honest gaps are private fixture secret provisioning/changelog discipline, richer failure-mode-targeted eval examples, true `full-drawer` result handling plus deeper additive budget accounting, merge-time provenance preservation, broader evidence UI/default-on rollout criteria, and promoting the current JSON-input dry-run bridge into the final DB/API-backed approval flow.
...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734/#735 and this bounded result-mode branch: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator, PR #735 fixes the Claude project memory path, and this branch adds default-off PostgreSQL `fact-plus-snippet` injection. Remaining work is true `full-drawer` behavior, merge-time provenance preservation, promotion of the current JSON-input dry-run bridge into the final DB/API-backed approval flow, and any residual legacy search/sync compatibility.
