...
> Last updated: 2026-04-22 (post-#730 + local eval/result-mode follow-ups -- PR #708 repaired the claude-mem session-summary backfill so new atomic facts still land under the legacy-compatible `session_summaries:<id>` source key while still deduping the short-lived #703 `:parent` key, repairs `memory_fact_sources` spans when `addFact()` retries find an existing fact, and now writes fallback provenance for every returned drawer chunk instead of only the first chunk; PR #710 replaced `scripts/peer-update.sh`'s direct `drizzle-kit migrate` call with the canonical `scripts/drizzle-migrate-apply.ts` psql-backed applier (drizzle-kit v0.31.9 silently no-ops pending SQL) and redirected post-reload probe/rollback output into `AGENTCTL_PEER_POST_RELOAD_LOG` before the final `pm2 reload` so the health-probe writer no longer SIGPIPEs on the restarting control plane, closing the 2026-04 macmini brick RCA; PR #712 extracted the drawer keyword + pgvector + RRF(k=60) pipeline into a reusable `searchMemoryDrawers()` helper in `packages/control-plane/src/memory/memory-drawer-search.ts` and wired it into `GET /api/memory/facts?mode=semantic` behind a `MEMORY_DRAWER_FUSION=true` env flag resolved once at route registration (additive `drawerResults?: MemoryDrawerSearchResult[]` envelope field, new `drawerLimit` query param defaulting to 25 and capped at 100, drawer failures degrade to fact-only), delivering the MemPalace Phase 4 fusion-flag surface; PR #713 delivered the Phase 0 facts-only baseline snapshot by extending `scripts/memory-bench.ts` with a `--baseline facts-only` mode, stamping fixture SHA256 + git SHA + ISO timestamp, and committing the deterministic baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` (aggregate R@5 0.500, R@10 0.500, MRR 1.000, NDCG@10 0.787, drawer-only hit 0.000) alongside `docs/memory-evals/README.md` documenting regeneration, so future drawer-aware eval runs now have a real comparison baseline; PR #716 hardened the drawer search backend post-#712 rebase by returning 404 from `GET /api/memory/drawers/:drawerId` for archived drawers via an `archived_at IS NULL` filter in `loadDrawerById`, throwing a typed `MemoryDrawerSearchDbError` from `memory-drawer-search.ts` that surfaces as HTTP 500 from `/api/memory/drawers/search` (opt-in per-caller via `degradeOnSqlError: false` so the fact-fusion caller still gets silent degradation), tightening `normalizeLimit` with a `/^\d+$/` predicate so `10abc`, `1.5`, `+5`, and `0x10` reject with 400, and raising the per-path candidate fetch to `max(CANDIDATE_LIMIT, limit)` so `limit > 50` actually reaches up to `MAX_LIMIT=100` (7 route tests + 6 helper tests), superseding the now-closed PR #706; PR #717 ported the facts-side piece #716 intentionally deferred by running `runDrawerFusion()`'s raw `q` through `sanitizeQuery()` from `@agentctl/shared` (empty/rejected verdict skips drawer fusion cleanly instead of crashing the `/api/memory/facts` envelope) and switching `drawerLimit` from loose `z.string().max(16).optional()` to a strict Zod refinement `^[0-9]+$` → `int().min(1).max(100)` that returns 400 `INVALID_DRAWER_LIMIT` for rejected values, while removing the dead `resolveDrawerLimit()` helper (11 new tests), so the drawer-aware fusion stack (Phase 4) now has both the `/api/memory/drawers/search` and `/api/memory/facts` surfaces fully hardened with no remaining open code follow-ups from this wave; PR #719 surfaced fact-side match-source badges, PR #720 added optional `InjectionBudget.tierTokenCaps` context-budget enforcement, PR #721 added drawer backfill dry-run token/cost/storage estimates, PR #722 wired opt-in live `pnpm memory:eval --no-mock`, PR #723 rendered raw drawer fusion results in the Memory Browser, PR #726 added browser-level Memory Browser enrichment coverage for match-source badges, drawer results, and the current delete confirmation flow, PR #728 added fact-detail provenance/source previews plus archived/unavailable markers on `GET /api/memory/facts/:id` and the Memory Browser detail panel, PR #729 added workflow-owned memory eval automation with staged private fixtures under `tmp/memory-eval/`, PR #730 stabilizes legacy fact-detail responses by keeping `sourcePreviews: []` when provenance rows are absent or preview lookup degrades, current `main` already includes human-readable `pnpm memory:eval` By Tag tables plus capped failure examples, local commit `932cfd91` gates `held-out/full` eval runs behind workflow-owned env flags while documenting the private-fixture contract, and local commit `80523670` adds bounded injection result-mode plumbing with PostgreSQL `fact-plus-snippet` support plus explicit `full-drawer` fallback.)
>
> Current checkpoint: PR #470 through PR #730 plus the local held-out/full gate and bounded result-mode follow-ups are represented on `main@80523670`. PR #708 closes the claude-mem source-key/span edge cases after #703, PR #710 closes the peer-update migration/SIGPIPE RCA after #697/#702, PR #712 adds the first control-plane drawer-aware fusion surface behind a feature flag and the reusable `searchMemoryDrawers()` helper, PR #713 commits the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json`, PR #716 hardens the drawer search backend (archived-row 404, typed DB error, strict limit predicate, per-path candidate cap) and supersedes the now-closed PR #706, PR #717 ports the facts-side fusion hardening (sanitized drawer query, strict `drawerLimit` with 400 `INVALID_DRAWER_LIMIT`), PR #719 surfaces fact-side match-source badges, PR #720 adds optional `InjectionBudget.tierTokenCaps` context-budget enforcement, PR #721 adds drawer backfill dry-run token/cost/storage estimates, PR #722 wires opt-in live `pnpm memory:eval --no-mock`, PR #723 renders raw drawer fusion results in the Memory Browser, PR #726 adds browser-level Memory Browser enrichment coverage for match-source badges, drawer results, and the current delete confirmation flow, PR #728 adds fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 adds workflow-owned eval automation and private-fixture staging docs, PR #730 stabilizes legacy fact-detail responses for empty/degraded provenance lookups, current `main` includes human-readable By Tag summaries plus capped failure examples in the eval report, local commit `932cfd91` keeps held-out/full execution behind workflow-owned env flags, and local commit `80523670` adds bounded injector result modes (`fact-only` default, PostgreSQL `fact-plus-snippet`, explicit `full-drawer` fallback). GitHub triage on 2026-04-22 found no open PRs, no failed recent CI/security runs, and `0` open CodeQL/Dependabot alerts. These changes do not alter beta/dev/prod CD routing.
>
...
>
> Current follow-up note: this pass records PRs #716 through #723, which together port and finish the drawer search/get edge-behavior hardening the now-closed PR #706 originally aimed at; #706 was superseded and closed. The drawer-aware fusion stack (Phase 4) now has both the `/api/memory/drawers/search` and `/api/memory/facts` surfaces fully hardened (archived-row 404, typed DB error surfaced as HTTP 500 with opt-in silent degradation for the fact-fusion caller, strict `^\d+$` limit predicate, per-path candidate cap reaching `MAX_LIMIT=100`, sanitized drawer fusion query via `@agentctl/shared`'s `sanitizeQuery()`, and strict `drawerLimit` returning 400 `INVALID_DRAWER_LIMIT`) with no remaining open code follow-ups from this wave. The MemPalace drawer stack now has worker contracts, CP search/get backend routes, production embedding-client wiring, a web drawer-search page, claude-mem source-key/span repair, peer-update migration/reload safety, the Phase 4 drawer-aware fusion flag behind `MEMORY_DRAWER_FUSION=true` on `/api/memory/facts` with a sanitized query and strictly validated `drawerLimit`, and the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` with regeneration docs in `docs/memory-evals/README.md`, optional tier token caps in context-budget enforcement, dry-run backfill token/cost/storage estimates, opt-in live eval wiring, and Memory Browser fact/drawer result visibility.
...
- [ ] Finish the remaining Phase 3 read/injection work after the landed fact-detail evidence path: expand beyond the local bounded injector result-mode slice (`fact-only` default, PostgreSQL `fact-plus-snippet`, explicit `full-drawer` fallback) into real full-drawer behavior, merge-time provenance preservation, and the bounded Surface A (`MEMORY.md`) dry-run bridge after PR #730 stabilized legacy empty-provenance fact-detail responses. Fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728.
