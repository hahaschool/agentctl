...
> Last updated: 2026-04-23 (post-#737 + merge-provenance follow-up).
>
> Current checkpoint: `origin/main@56b4a598` is current through PR #737. Recent memory/security landings now include PR #728 fact-detail evidence previews plus archived/unavailable provenance markers, PR #729 workflow-owned memory-eval automation and staged private-fixture docs, PR #730 legacy empty-provenance stabilization, PR #731 roadmap sync after PR #730, PR #732 shared `sanitizeName()` hardening across control-plane and worker memory routes, PR #733 merged the `@xmldom/xmldom` + `uuid` security override fix onto `main`, PR #734 added the first script-only `MEMORY.md` dry-run generator that preserves human-curated content while proposing bounded generated sections from reviewed facts, PR #735 corrected the Claude project memory target to `~/.claude/projects/<encoded>/memory/MEMORY.md`, PR #736 added bounded injector result modes with default `fact-only`, PostgreSQL `fact-plus-snippet`, and explicit `full-drawer` fallback to `fact-only`, and PR #737 advanced Surface A by allowing the dry-run generator to source reviewed facts from the control-plane `/api/memory/facts` API as well as JSON fixtures. This branch starts merge-time provenance preservation with a store-level helper that copies and de-duplicates `memory_fact_sources` spans from duplicate facts onto the survivor fact.
>
> GitHub triage on 2026-04-23 found `0` open PRs and `0` open CodeQL, Dependabot, and secret-scanning alerts before this follow-up branch started. The latest `CI`, `Build & Publish Docker Images`, and `Security Audit` runs on `main` all succeeded after PR #737. These changes do not alter dev-1/dev-2, beta, or production CD routing.
>
