...
- [ ] Finish the remaining Phase 3 read/injection work after PRs #728/#730/#734/#735/#736/#737: fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728, PR #730 keeps legacy no-provenance detail responses on a stable empty-`sourcePreviews` contract, PR #734 adds the first bounded Surface A (`MEMORY.md`) dry-run generator, PR #735 fixes its Claude project memory target path, PR #736 adds bounded default-off PostgreSQL `fact-plus-snippet` injection with a `full-drawer` fallback, and PR #737 adds a paged control-plane API fact source for the Surface A dry run. The current follow-up branch adds store-level `memory_fact_sources` copying/deduping so duplicate-fact merges can preserve provenance. Remaining work is true `full-drawer` behavior, API-level near-duplicate merge wiring, promotion of the dry-run bridge into the final write-gated approval flow, and any residual legacy search/sync compatibility.
