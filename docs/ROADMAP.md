...
> Last updated: 2026-04-15 (post-#561 mesh version observability checkpoint). Latest mesh landing set: PR #550 persisted peer ping diagnostics, PR #551 added in-place mesh peer editing, PR #552 added the rate-limited signed reverse-registration endpoint, PR #553 registered 33.9-33.11 planning, PR #554 added machine-origin provenance badges, PR #555 added nullable peer-version columns, PR #556 exposed `/health` `appVersion`/`gitSha`/`schemaVersion`, PR #557 added mesh envelope schema/protocol metadata plus apply-side compat gates and `docs/MESH_COMPAT.md`, PR #558 added `/mesh-peers` version/drift UI plus the lazy-loaded sidebar peer-version tooltip, PR #560 renumbered the peer-version migration from duplicate `0024` to `0025`, and PR #561 persisted peer build/schema metadata from on-demand and background health pings. Earlier 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
> Current checkpoint: PR #470 through PR #558 plus the v0.4.0 version bump and version-bump SIGPIPE hotfix are represented on `main@2bdd54f2`. GitHub reports 0 open code-scanning, 0 open Dependabot, and 0 open secret-scanning alerts as of the post-#555 check; refresh alerts again after post-#558 checks finish. Post-#555 main checks passed: CI `24444616456`, Security Audit `24444616428`, and Build & Publish Docker Images `24444616447`. Post-#557 main checks passed: CI `24444895702`, Security Audit `24444895704`, and Build & Publish Docker Images `24444895701`. Post-#558 main checks started as CI `24445468284`, Security Audit `24445468249`, and Build & Publish Docker Images `24445468269`; Docker publish is already green and CI/Security are still running at this checkpoint.
>
...
>
> Current follow-up note: this pass records PRs #550-#558 across mesh peer diagnostics/editing/reverse-registration/provenance/version-schema/build-metadata/schema-protocol-compat/version UI, and fixes the duplicate `0024` migration risk by renumbering `0024_sync_nodes_peer_version.sql` to `0025_sync_nodes_peer_version.sql`. The latest landed main is `2bdd54f2`; beta/dev CD state remains untouched.
...
- [ ] **Auto-call reverse registration during peer add.** When `POST /api/sync/peers` creates or updates a peer on node A, A should call the target's `/api/sync/peers/register`, surface a warning when the target is old/unreachable, and use the target identity to fill missing public-key fields safely.
...
| **P0** | Mesh Bidirectional Registration + Cross-Node Visibility | 33.8 | In progress — PR #552 delivered the rate-limited bootstrap-token plus signed-envelope `POST /api/sync/peers/register` endpoint, and PR #554 delivered machine-row provenance plus `/machines` origin badges. Remaining work: auto-call reverse registration on peer add, show one-way warnings/manual retry, and prove two-node machine replication end-to-end. See [plans/2026-04-15-mesh-bidirectional-registration-plan.md](plans/2026-04-15-mesh-bidirectional-registration-plan.md). |
...
| **P1** | Fleet Rollout & Peer Auto-Update (two-topology) | 33.11 | Planned — beta promotion today still requires manual `git pull && pnpm build && pm2 reload` on each mesh node. Activates the existing Docker fleet path and adds a PM2 mesh path with `scripts/peer-update.sh`, `pnpm agentctl peer update`, opt-in timers, settings/peer update UI, rollback history, and two-node dry-run coverage. Explicit non-goal: no default-on unattended automation. Depends on the delivered 33.9 drift signal plus the remaining 33.10 operator-warning/two-node proof. See [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md). |
...
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | In progress — PR #552 delivered the signed reverse-registration endpoint and PR #554 delivered machine provenance badges; remaining scope is auto-register-on-add, one-way warning/manual retry, and two-node replication proof | 33.8 |
...
| [mesh-fleet-rollout-peer-update-plan](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md) | Planned — defines Docker fleet activation plus PM2 mesh peer-update CLI/timers/UI/rollback, gated on delivered 33.9 ping/UI drift visibility and the remaining 33.10 operator-warning/two-node proof | 33.11 |
