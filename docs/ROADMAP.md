...
> Last updated: 2026-04-14 (post-PR #460 + memory scopes coverage follow-up). The 2026-04-13 landing set includes PR #395 (session `Config` tab e2e depth), PR #396 (`/conflicts` resolution-flow e2e depth), PR #397 (mesh/runtime e2e follow-up tracking doc), PR #398 (dependabot `drizzle-orm 0.38.4 -> 0.45.2`, closes `GHSA-gpj5-g38j-94v9`), PR #400 (DAST WebSocket fuzz scan pnpm setup), PR #401 (`chore(deps): patch vite and next CVEs`, adding the `vite ">=7.3.2"` pnpm override and bumping `next ^16.1.7 -> ^16.2.3`), PR #402 (Anthropic Claude Agent SDK lockfile refresh to `@anthropic-ai/sdk 0.81.0`), PR #404 (roadmap/security-plan reconciliation after those fixes landed), PR #405 (manual-takeover relay re-verification closing §2.4 with new rate-limit hardening on all three manual-takeover handlers), PR #406 (durable roadmap sync wording), PR #407 (memory decay UI exposing existing `/api/memory/decay/*` routes), PR #408 (emergency-stop UI exposing existing `/api/agents/*/emergency-stop` routes with a typed "STOP ALL" fleet confirmation), PR #409 (roadmap sync after #405-#408 merged), PR #410 (narrow CodeQL `#579` false-positive suppression for the already-rate-limited manual-takeover GET handler), PR #411 (AgentsPage + AgentDetailPage test repair, restoring the 123-test targeted suite), PR #413 (standalone CodeQL source-suppression formatting for the same `#579` false positive), PR #414 (web Vitest `jest-dom` setup plus stale query/mock repairs), PR #415 (`/memory/reports` Playwright generation success/failure coverage), PR #416 (Logs UI Security Findings tab backed by the existing `/api/security/findings` route), PR #417 (StatusBadge native tooltips and screen-reader descriptions for known lifecycle/status values), PR #418 (NotificationBell pending-approvals popover Playwright coverage for allow-once, allow-for-session, deny, and empty states), PR #419 (plan status drift cleanup and orphan plan indexing), PR #420 (full web Vitest residual-failure cleanup plus small UI contract fixes), PR #422 (NotificationBell e2e aria-label follow-up after #420's more descriptive pending-approval labels), PR #423 (OAuth PKCE route rate limiting for initiate/callback/refresh), PR #424 (roadmap/plan sync after #422/#423), PR #425 (standalone `/mesh-peers` web page over existing sync-peers list/ping APIs), PR #426 (SessionHeader and DashboardQuickActions accessibility/title/aria-hidden polish), PR #427 (accounts route rate limiting on credential listing and write paths), PR #428 (roadmap/main-stability/mesh P4 docs sync after #425-#427), PR #429 (account verification route rate limiting before credential decrypt and outbound provider checks), PR #430 (roadmap/main-stability docs sync after #429), PR #431 (`/mesh-peers` Playwright coverage for render, empty, ping-success, and ping-failure states), PR #436 (`/security-findings` Playwright coverage for happy-path rows/summary badges, empty state, severity filter, and error banner), PR #438 (`/webhooks` management page wiring `GET/POST/PATCH/DELETE /api/webhooks` + per-row test action into a dense web page with a sidebar entry, add/edit dialog, and delete confirmation), PR #439 (rate-limit batch 4 covering `mobile-push-devices`, `notification-preferences`, `memory-synthesis`, `memory-consolidation`, and `knowledge-maintenance` write routes, explicitly completing the rate-limit coverage audit started in PR #423), PR #440 (CI/security scan cleanup: Biome 2.4 security lint syntax plus Grype SARIF validation to skip empty invalid uploads), PR #442 (SessionHeader export-menu focus rings, ErrorDetailPanel `aria-expanded`/`aria-controls`, and GitStatusBadge loading announcement), PR #443 (Zod input validation and length bounds for checkpoint, mobile push device, and knowledge maintenance write surfaces), PR #444 (`/webhooks` Playwright coverage for table rendering, create validation, delete confirmation, and test success/failure toasts), PR #445 (backend-independent web Playwright coverage now runs in CI for the webhooks slice), PR #446 (slash-search hotkey regression coverage on Agents, Machines, and Sessions), PR #447 (mobile-push device validation negative coverage and oversized-parameter routing), PR #448 (batch-2 input validation hardening across webhooks, permission requests, approvals, handoffs, sync conflicts, and memory facts), PR #449 (Sessions page keyboard help overlay and shortcut catalog sync), PR #450 (centralized keyboard help overlay ownership in Sidebar, removing page-local `?` owners from Dashboard/Sessions and adding keydown propagation regression coverage), PR #451 (roadmap/plan sync after #445-#450), PR #452 (sync-peer URL, role/status, interval, and SSRF validation hardening), PR #453 (Deployment promote-panel beta gate clarity without changing promotion behavior), PR #454 (`/logs` Security Findings Playwright coverage), PR #455 (`/memory/browser` facts-flow Playwright coverage), PR #457 (`/memory/import` wizard completion/cancellation coverage), PR #458 (`/settings` notification preferences backend-independent Playwright coverage), PR #459 (`/discover` grouped/filter/import/new-session backend-independent Playwright coverage), and PR #460 (roadmap checkpoint sync after #458/#459). CodeQL `#579` was formally dismissed as a false positive after fresh `main` analysis still flagged the real `@fastify/rate-limit` route shape; local source-shape coverage keeps the limiter before `authorizeManualTakeover`. This follow-up records the `/memory/scopes` E2E slice on branch `agent/codex-461-memory-scopes-e2e` after GitHub reported 0 open PRs plus 0 open code-scanning, Dependabot, and secret-scanning alerts on `main@c5cfa718`.
>
> Current checkpoint: PR #460 is merged on `main@c5cfa718`; post-merge CI, Security Audit, and Docker image workflows are green, with 0 open PRs and 0 open code-scanning, Dependabot, or secret-scanning alerts. Active low-overlap browser follow-ups include this `/memory/scopes` branch and the separate `/memory/graph` `codex-462` claim.
>
> Coordination note: PR #457 merged on `main@13a9d674`, PR #458 merged as `aaa292a4`, PR #459 merged as `3efb2e17`, and PR #460 merged as `c5cfa718`; all four post-merge main workflow sets completed successfully.
...
- [x] Add backend-independent `/memory/scopes` Playwright coverage for scope-tree rendering, create payloads, and delete confirmation/cascade behavior *(current branch)*
...
- [x] Backend-independent `/memory/scopes` Playwright coverage for scope-tree rendering, create payloads, and delete confirmation/cascade behavior *(current branch)*
...
| **P0** | ~~Mesh: Node Discovery + Peer Registry~~ | 33.4 | ✅ Delivered — PR #376 merged peer discovery, health check, auth, REST API, and 36 tests; PR #425 added the standalone `/mesh-peers` web read + ping surface over that registry, PR #431 added focused Playwright coverage, and PR #452 hardened sync peer URL/SSRF validation. |
...
| [dev-environment-cd-strategy](plans/2026-03-12-dev-environment-cd-strategy.md) | Delivered — PR #445 closed the deferred web `WEB_PORT` package-script gap | 12.0-12.5 |
