...
**Plan:** [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md)

**Motivation:** Adding a mesh peer today requires SSH to each machine to edit env files, rebuild, restart PM2, then debug silently-failing reverse registration. A 2026-04-16 live fleet exercise (laptop + macmini) took ~2 hours of debugging for what should be a 30-second "click Add, both sides connected" flow. This is antithetical to the project vision of remote fleet management from iPhone/iPad.

**Scope (4 phases):**

1. **Auto-Detection & Self-Identity (P0):** Auto-detect Tailscale IP at startup via `tailscale ip -4`, add self-identity card on `/mesh-peers`, expose `selfSyncUrl` on `/health`. Eliminates manual `TAILSCALE_IP` env var.
2. **Settings UI for Mesh Config (P0):** `mesh_config` DB table (or extend `settings`) for tailscale_ip_override, sync_url_override, registration_token, auto_update_enabled. New Settings → Mesh section with token management. All config editable from web UI — no env file editing.
3. **Add Peer Flow Redesign (P0):** Synchronous add-and-register returning both local + reverse results. Pre-flight checks (probe, token compat, schema compat) in the Add Peer dialog. Actionable error messages for every failure code. Retry with backoff for failed reverse registrations.
4. **Operational Improvements (P1):** Stale peer cleanup (>7 days), mesh health dashboard (replication lag, conflict rate), bidirectional connection test, version-bump script auto-updating `LOCAL_APP_VERSION`.

**Success criteria:** (1) New operator can add a peer entirely from the web UI — no SSH, no env files, no PM2 restarts. (2) "Discover → Add → Bidirectional" takes < 30 seconds with clear success/failure feedback. (3) Every failure state has an actionable error message visible in the UI.
...
| **P0** | Mesh Peering UX & Reliability Overhaul | 33.12 | Planned — 4 phases: auto-detect Tailscale IP, Settings → Mesh UI, synchronous add-and-register flow, operational improvements. See [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md). |
...
| [mesh-peering-ux-reliability-overhaul-plan](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) | Planned — auto-detect Tailscale IP, Settings → Mesh config UI, synchronous add-and-register flow with pre-flight checks, operational improvements (stale cleanup, health dashboard, bidirectional test) | 33.12 |
