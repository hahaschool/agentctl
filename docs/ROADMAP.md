...
> Last updated: 2026-04-21 (post-#714 -- PR #708 repaired the claude-mem session-summary backfill so new atomic facts still land under the legacy-compatible `session_summaries:<id>` source key while still deduping the short-lived #703 `:parent` key, repairs `memory_fact_sources` spans when `addFact()` retries find an existing fact, and now writes fallback provenance for every returned drawer chunk instead of only the first chunk; PR #710 replaced `scripts/peer-update.sh`'s direct `drizzle-kit migrate` call with the canonical `scripts/drizzle-migrate-apply.ts` psql-backed applier (drizzle-kit v0.31.9 silently no-ops pending SQL) and redirected post-reload probe/rollback output into `AGENTCTL_PEER_POST_RELOAD_LOG` before the final `pm2 reload` so the health-probe writer no longer SIGPIPEs on the restarting control plane, closing the 2026-04 macmini brick RCA; PR #711 synced the previous roadmap/plan checkpoint for #705, #707, and #709 while flagging #706, #708, and #710 as still-open follow-ups; PR #712 extracted the drawer keyword + pgvector + RRF(k=60) pipeline into a reusable `searchMemoryDrawers()` helper in `packages/control-plane/src/memory/memory-drawer-search.ts` and wired it into `GET /api/memory/facts?mode=semantic` behind a `MEMORY_DRAWER_FUSION=true` env flag resolved once at route registration (additive `drawerResults?: MemoryDrawerSearchResult[]` envelope field, new `drawerLimit` query param defaulting to 25 and capped at 100, drawer failures degrade to fact-only), delivering the MemPalace Phase 4 fusion-flag surface; PR #713 delivered the Phase 0 facts-only baseline snapshot by extending `scripts/memory-bench.ts` with a `--baseline facts-only` mode, stamping fixture SHA256 + git SHA + ISO timestamp, and committing the deterministic baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` (aggregate R@5 0.500, R@10 0.500, MRR 1.000, NDCG@10 0.787, drawer-only hit 0.000) alongside `docs/memory-evals/README.md` documenting regeneration, so future drawer-aware eval runs now have a real comparison baseline; PR #714 synced that merge wave into the roadmap. Open follow-up: PR #716 supersedes closed PR #706 and ports the drawer search/get strict-limit and archived-row hardening onto the post-#712 `searchMemoryDrawers()` helper; it still needs to port PR #706's facts-side drawer-fusion sanitization and malformed `drawerLimit` rejection before merge.)
>
> Current checkpoint: PR #470 through PR #714 plus version bumps v0.5.2-v0.8.0 are represented on `main@b6ab5277`. PR #708 closes the claude-mem source-key/span edge cases after #703, PR #710 closes the peer-update migration/SIGPIPE RCA after #697/#702, PR #712 adds the first control-plane drawer-aware fusion surface behind a feature flag and the reusable `searchMemoryDrawers()` helper, PR #713 commits the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json`, and PR #714 records the merge-wave roadmap sync. Closed PR #706 is now superseded by review-gated PR #716; #716 still needs the facts-side fusion sanitizer/drawerLimit port before it should merge. These changes do not alter beta/dev/prod CD routing.
>
...
> Coordination note addendum: PR #663 merged as `d53e94af`, PR #664 as `5a433d68`, PR #665 as `2271bf72`, PR #666 as `ea82fec9`, PR #667 as `48bc709f`, PR #668 as `f77f8074`, PR #669 as `811bdf31`, PR #670 as `c1e1add6`, PR #671 as `e19dd7d5`, PR #672 as `b7273f5e`, PR #673 as `f106f4f7`, PR #674 as `bfe9759c`, PR #675 as `92ab8c97`, PR #677 as `1dfadaa3`, PR #678 as `bfbca627`, PR #679 as `eb4f19b2`, PR #681 as `a976cae2`, PR #682 as `a66c9bc1`, PR #684 as `a2cfdda8`, PR #685 as `150c3be7`, PR #686 as `b8ed8ce4`, PR #687 as `99e36894`, PR #688 as `135344e1`, PR #689 as `e6164cfa`, PR #691 as `465ebb2e`, PR #692 as `ef93b76d`, PR #693 as `2d5ea917`, PR #694 as `f0c632e4`, PR #695 as `47243f58`, PR #696 as `efb87d41`, PR #697 as `5ca38166`, PR #698 as `aa90762f`, PR #699 as `f7c15804`, PR #700 as `be1b4a83`, and PR #701 as `c9132f1b`, PR #702 as `6155f03a`, PR #703 as `524a32c5`, and PR #704 as `7ee21749`; PR #705 as `cdb3bb65`, PR #707 as `8ad648a5`, PR #709 as `c97b61df`, PR #708 as `c8e9d376`, PR #710 as `9ec3e1e2`, PR #711 as `dbd17a07`, PR #712 as `f8e9b1db`, and PR #713 as `ec749b77`.
>
> Current follow-up note: this pass records PR #714 and the #706 -> #716 handoff. The one remaining code follow-up is PR #716 (drawer search/get strict behavior), but it is not yet a complete #706 replacement because facts-side fusion sanitization and malformed `drawerLimit` rejection still need to be ported. The MemPalace drawer stack now has worker contracts, CP search/get backend routes, production embedding-client wiring, a web drawer-search page, claude-mem source-key/span repair, peer-update migration/reload safety, the Phase 4 drawer-aware fusion flag behind `MEMORY_DRAWER_FUSION=true` on `/api/memory/facts`, and the Phase 0 facts-only eval baseline at `docs/memory-evals/phase-0-facts-only-baseline.json` with regeneration docs in `docs/memory-evals/README.md`.
