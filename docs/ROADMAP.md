...
> Coordination note addendum 5: PR #798 merged as `5d138c0f`, PR #799 as `368da836`, PR #800 as `c1f28d8e`, PR #801 as `d64f3ba3`, PR #802 as `253a77db`, and PR #803 as `234da9e4`.
>
...
- [ ] **PR C** — Settings UI: `ApiError.details` extension, `memoryProvidersApi` web client, `ProviderDialog` (test-before-save, verified-only catalog), `MemoryEmbeddingsSection` in Settings. Minor version bump deferred to the PR C branch/release flow. *(active branch: `agent/claude-1/feat/memory-ops-pr-c`, rebase onto post-#803 main before opening PR)*
...
| **P0** | Agent Run Quality | 16.1 | Current loop closed through PR #803; latest main CI, Security Audit, Build & Publish, code scanning, Dependabot, and secret scanning are green/empty. Historical stability work through PR #797 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. The latest delta is PR #800 deferred PreCompact capture startup, PR #801 preserved legacy no-provenance memory compatibility, PR #802 surfaced memory-eval fixture coverage while keeping failed-run artifacts visible, and PR #803 hardened mobile evidence detail rendering. |
