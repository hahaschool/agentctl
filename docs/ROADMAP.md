...
- [ ] **Playwright two-node fixture.** PR #654 added the opt-in live fixture foundation and first version-drift assertion: one live peer ping updates the mesh-peers row and update-available banner with the expected peer version. Remaining: forced `schemaVersion + 2` envelope rejection with the 33.10 banner, and `agentctl peer update --dry-run` planned-step reporting without state mutation.
