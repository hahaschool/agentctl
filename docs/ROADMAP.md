...
- [ ] **PR D** — Job infrastructure: `memory/ops/config.ts`, BullMQ queue singleton, `JobsRepository` (advisory lock + fleet check + durable cancel), `JobEventsRepository`, SSE stream with pg_notify + replay, preview endpoint + capabilities route. `MEMORY_OPS_ENABLED_KINDS=` empty. *(branch: `agent/claude-1/feat/memory-ops-pr-d`)*
...
| **P0** | Agent Run Quality | 16.1 | Current loop closed through PR #810; latest main CI, Security Audit, Build & Publish, code scanning, Dependabot, and secret scanning are green/empty. Historical stability work through PR #797 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. The latest delta is PR #800 deferred PreCompact capture startup, PR #801 preserved legacy no-provenance memory compatibility, PR #802 surfaced memory-eval fixture coverage while keeping failed-run artifacts visible, PR #803 hardened mobile evidence detail rendering, PR #806 landed the Settings -> Memory & Embeddings provider UI, PR #807 persisted saved-provider test results, PR #808 hardened the saved-provider status path against raw upstream error storage and stale metadata writes, PR #809 synced docs, and PR #810 folded the concurrent hardening status into the roadmap/plan checkpoint. |
