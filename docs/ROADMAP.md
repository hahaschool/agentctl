...
- [x] Finish drawer backfill batching/backoff and embedding generation/backfill for CLI-written facts *(local `main@c6238281` follow-up after PRs #721/#722/#726: execute-mode backfills now batch/retry embedding calls, generate embeddings for newly written atomic facts, and backfill embeddings onto existing CLI-written facts during idempotent resumes).*
...
- [x] Enrich the human-readable `pnpm memory:eval` report with By Tag summaries plus capped failure examples while keeping `--json` output unchanged *(local `main@f852e5d9`)*.
- [ ] Extend eval honesty beyond live wiring with full private fixture coverage, release/weekly held-out gates, and no held-out tuning outside release/weekly eval jobs. Local `main@f747122d` already gates `held-out/full` execution behind workflow-owned env flags and documents the private-fixture delivery contract; this branch adds `.github/workflows/memory-evals.yml` to run weekly `held-out`, release-tag `full`, and manual dispatch against gitignored staged fixtures under `tmp/memory-eval/`, so the remaining work narrows to provisioning/rotating the repo secrets plus richer failure-mode-targeted eval examples.
...
- [ ] Finish the remaining Phase 3 read/injection work after the landed fact-detail evidence path: preserve legacy no-provenance rendering/search/sync behavior, then injector result modes/wrapper plumbing, merge-time provenance preservation, and the bounded Surface A (`MEMORY.md`) dry-run bridge. Fact-detail evidence/quote previews plus unavailable-source markers landed in PR #728.
...
- [ ] Extend retrieval only after eval/backfill with rank-bucket boosts, drawer-aware search, drawer-grep enrichment, source-neighbor hydration, capped additive boosts, MCP drawer get/search parity, full `memory_dedup_check` skip/merge scoring, result modes, and injector wrapper plumbing for `tierTokenCaps`.
