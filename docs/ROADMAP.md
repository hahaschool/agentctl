...
> Last updated: 2026-04-16 (post-#607 mesh fixes, version display, lint, security dep, discover UX). PR #597 synced the roadmap after PR #596. PR #598 added signed peer-update proxy forwarding to remote peers (33.11). PR #599 fixed Tailscale CLI binary path resolution for macOS app bundle. PR #600 fixed `createPeerSignedHeader` call to pass object instead of string in the update proxy. PR #601 fixed `peer-update.sh` path resolution for compiled JS. PR #602 used Tailscale IP for `selfSyncUrl` in reverse registration. PR #603 replaced the hardcoded `LOCAL_APP_VERSION` constant with a runtime fetch from the CP `/health` endpoint. PR #604 documented the §33.12 peering UX & reliability overhaul plan. PR #605 resolved Biome import ordering and formatting in `sync-peer-update`. PR #606 updated hono to 4.12.14 to fix an HTML injection vulnerability. PR #607 increased worker timeout on the discover page and prevented auto-refresh flicker. Earlier 2026-04-13/14 memory, webhook, settings, discovery, mesh, and agent browser coverage follow-ups remain represented below.
>
> Current checkpoint: PR #470 through PR #607 plus the v0.5.4 version bumps are represented on `main@fa60a033`. Refresh GitHub security alerts after this docs sync lands.
>
...
- [x] Backend-independent web Playwright CI gating for `/webhooks`, `/agent-profiles`, `/audit`, `/machines`, `/agents` list, `/scheduler`, and `/settings` using `WEB_PORT`-aware web scripts and a shared-package build preflight *(PRs #445, #505, #513, #516, #518, #533, #546)*
...
**Post-delivery fix:** PR #607 increased the worker timeout on the discover page and prevented auto-refresh flicker that caused the peer list to flash during background polling.
...
**Post-delivery fixes:** PR #596 added `SYNC_PEER_REGISTRATION_TOKEN` to the explicit `env` block in `ecosystem.beta.config.cjs` and `ecosystem.mesh.config.cjs`. PM2's explicit env overrides inherited `process.env`, so the token was invisible to the CP, causing reverse-registration to fail with `PEER_REGISTRATION_DISABLED` ("one-way" peering) on the live fleet despite the feature being correctly implemented at the application layer. PR #602 switched `selfSyncUrl` in reverse registration to use the Tailscale IP instead of the hostname, fixing peering on networks where MagicDNS is unavailable.
...
**Post-delivery fixes:** PR #596 updated `LOCAL_APP_VERSION` from v0.5.0 to v0.5.1 in `packages/web/src/lib/mesh-version.ts` to match the current release, fixing a false "mixed mesh version" drift banner that appeared because the hardcoded local version lagged behind the actual deployed version. PR #603 then replaced the hardcoded `LOCAL_APP_VERSION` constant entirely with a runtime fetch from the CP `/health` endpoint, eliminating version-display drift permanently *(PR #603)*.
...
- [x] **Manual PM2 peer-update script + signed self-update route.** PR #563 added `scripts/peer-update.sh`, a signed peer-auth-protected `POST /api/sync/peers/:peerId/update` route, route-local Fastify rate limiting before authorization, mutex protection for overlapping updates, structured update responses, and focused route/UI coverage. PR #598 added signed peer-update proxy forwarding so the CP can sign and forward update requests to remote peers. PR #599 fixed Tailscale CLI binary path resolution for macOS app bundle (`/Applications/Tailscale.app/Contents/MacOS/Tailscale`). PR #600 fixed `createPeerSignedHeader` in the proxy to pass an object instead of a string. PR #601 fixed `peer-update.sh` path resolution for compiled JS output.
...
### 33.12 Mesh Peering UX & Reliability Overhaul (P0) — Planned

**Plan:** [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) (v3, documented in PR #604)
...
0. **Bug Fixes (P0):** Fix `version-bump.sh` missing `git add` for `mesh-version.ts`; add `reverse_registration_*` columns to Drizzle schema (drift from migration 0026); deprecate old `GET /api/sync/discover` endpoint.
1. **Tailscale IP Auto-Detection (P0):** `tailscale ip -4` auto-detect with CGNAT validation (reject loopback even from env var). Expose `selfSyncUrl` on `/health` (no token status — `/health` is public/rate-limit-exempt).
...
3. **Structured Error Contract + Frontend Mapping (P0):** Preserve `errorCode`/`httpStatus` from remote reverse-registration JSON. New `reverse_registration_error_code` column. Frontend error-code-to-actionable-message mapping. Pre-flight non-mutating token check via registration endpoint.
4. **Retry + Operational (P1):** Backend in-memory retry with backoff (30s/60s/120s, dedup with manual retry). Self-identity card. Stale peer cleanup.
...
| **P1** | Fleet Rollout & Peer Auto-Update (two-topology) | 33.11 | In progress — PM2 topology: PR #563 (manual update), PR #571 (`agentctl peer update` CLI), PR #572 (opt-in schedulers), PR #577 (settings UI), PR #580 (rollback), PRs #598-#601 (peer-update proxy forwarding, Tailscale CLI macOS path, signed header fix, compiled JS path fix). Docker topology: PR #575 (build provenance), PR #579 (migration gate), PR #593 (machines.yml fleet structure). Cross-topology: PR #570 (version-compat API), PR #582 (update-available banner). Remaining: end-to-end canary dry-run, Playwright two-node fixture. See [plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md). |
| **P0** | Mesh Peering UX & Reliability Overhaul | 33.12 | Planned (v3) — Phase 0: version-bump.sh fix + Drizzle schema drift + deprecate old discover. Phase 1: Tailscale IP auto-detect with CGNAT validation. Phase 2: local-only `mesh_local_config` table + `MeshConfigProvider` + Settings → Mesh UI. Phase 3: structured error codes + frontend mapping + preflight token check. Phase 4: backend retry + identity card + stale cleanup. See [plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md). |
...
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | In progress — PR #552 delivered the signed reverse-registration endpoint, PR #554 delivered machine provenance badges, PR #564 delivered auto-register-on-add plus `/mesh-peers` one-way retry, PR #596 fixed PM2 env passthrough, and PR #602 fixed `selfSyncUrl` to use Tailscale IP; remaining scope is Playwright two-node fixture (docker compose) | 33.8 |
...
| [mesh-fleet-rollout-peer-update-plan](plans/2026-04-15-mesh-fleet-rollout-peer-update-plan.md) | In progress — PR #563 delivered the first PM2 manual update slice, PRs #598-#601 fixed peer-update proxy forwarding, Tailscale CLI macOS path, signed header argument shape, and compiled JS path resolution; remaining scope is end-to-end canary dry-run and Playwright two-node fixture | 33.11 |
| [mesh-peering-ux-reliability-overhaul-plan](plans/2026-04-16-mesh-peering-ux-reliability-overhaul-plan.md) | Planned (v3) — 5 phases: bug fixes (version-bump.sh, Drizzle drift, discover deprecation), Tailscale IP auto-detect with CGNAT/loopback validation, local-only `mesh_local_config` table (NOT synced) + `MeshConfigProvider` replacing static startup opts + Settings → Mesh UI with write-only token, structured reverse-registration error contract + frontend mapping + non-mutating preflight token check, backend retry with backoff + identity card + stale cleanup | 33.12 |
