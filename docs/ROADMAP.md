...
> bringing the latest CI lane to 32 spec files / 118 executed tests. PR #822
...
> post-#835 main CI, Security Audit, and Docker publish passed at `4bc82924`.
...
- [ ] Import completeness — durable job state plus restart-visible resume/rollback UX is covered through PR #841 and this resume-orphan hotfix; edge creation from imported structured fields, broader `claude-mem` source-local idempotency, crash-atomic progress accounting, non-positional JSONL resume cursors, and embedding-similarity dedup remain follow-up work.
