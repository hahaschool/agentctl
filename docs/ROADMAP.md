...
> Last updated: 2026-04-25 (post-#792 PR/CI/security sweep and roadmap/plan sync).
>
> Current checkpoint: `main@e2fd1b29` is current through PR #792. PRs #779–#792 merged since the previous roadmap sync: PR #779 (`eca52aac`) added create/delete UI to the task graphs page, PR #780 (`800c6f3d`) synced the roadmap through PR #777, PR #781 (`940a6a17`) added focused MemoryDrawersView unit coverage, PR #782 (`c1eb2158`) added the Space detail ContextRefsPanel with add/delete UI, PR #783 (`4578728f`) landed Memory Operations UI PR A (migration `0033`, shared provider/ops/audit types, embedding-client extension, and runtime credential filtering), PR #784 (`742d354c`) hardened the PreCompact hook timeout/stdout behavior, PR #785 (`36c808d`) added agent-detail loop start/stop controls, PR #786 (`78202bdf`) added editable consolidation resolutions, PR #787 (`603dcd5d`) fixed migration-check destructive-comment quoting, PR #788 (`c44cade8`) wired JSONL history import through backend/frontend with CodeQL-safe tests, PR #789 (`63ba8f1`) added audit session replay and suspicious-pattern detection, PR #790 (`2e69ae0b`) added live dashboard metrics sparklines, PR #791 (`0b156707`) fixed loop UI start requests to send a default `LoopConfig`, and PR #792 (`e2fd1b29`) stabilized metrics sparkline SVG gradient IDs.
>
> GitHub triage on 2026-04-25 has the roadmap current through PR #792. GitHub currently reports 0 open code-scanning alerts, 0 open Dependabot alerts, and 0 open secret-scanning alerts. Beta is intentionally left on the v0.8.2 promotion stack; the post-beta mainline changes are recorded here for the next full promotion rather than hot-applied into beta.
>
...
> LLM-generated narratives; JSONL history import now has a basic backend/frontend
> path but still needs stronger dedup/preview semantics and large-file bounds;
> and rich consolidation/graph modes from the original design remain follow-up
> work.
...
- [ ] Import completeness — rollback/resume UX, edge creation from imported structured fields, embedding-similarity dedup, duplicate re-import prevention, preview/import count alignment, and bounded streaming for very large JSONL files remain follow-up work.
...
| **P1** | Unified Memory System UI | 4.8 | Partial — route family, facts browser, dashboard, claude-mem import, basic JSONL history import, basic graph/table, deterministic reports, synthesis lint, maintenance, and core API routes exist. Not complete: JSONL import rollback/resume, duplicate re-import prevention, preview/import count alignment, large-file streaming bounds, edge generation from imported observations, rich graph Timeline/Clusters/focus/time-lapse, LLM narrative reports, and full consolidation merge/resolution workflows. |
