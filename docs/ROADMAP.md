...
- [ ] **Authenticated bidirectional handshake.** When `POST /api/sync/peers` creates a new peer on node A, A signs a `register-peer` envelope with its Ed25519 key (already used by `peer-auth.ts`) and calls `POST /api/sync/peers/register` on the target. The envelope carries A's `/health` identity plus proposed `{ machineId, hostname, tailscaleIp, publicKey, syncUrl }`; the target validates the signature and may corroborate A's Tailscale IP from the request source or `tailscale status --json` before upserting the reverse row with `sync_status = 'unknown'` until its own ping confirms reachability. The target responds with its own identity so A can fill in `publicKey` if missing.
