...
- [ ] Extend memory edges with temporal validity/source references and `memory_traverse` only after reviewing mesh `sync_capture_change()` payload compatibility, schema-version gates, and entity canonicalization/backfill requirements; PR #840 adds the first pure helper and extraction benchmark, PR #852 adds the local-only canonical entity schema/store, PR #855 adds dry-run proposals, and PR #858 adds the first read-only `GET /api/memory/timeline` slice over fact validity windows/direct edges without schema or mesh payload changes. Human-approved apply/runtime wiring, dedicated edge temporal fields, worker/MCP/UI timeline wiring, and edge-level recursive/as_of behavior remain open.
...
| **P0** | Agent Run Quality | 16.1 | Current loop is closed through PR #859. PR #857 added private fixture scaffold tooling, PR #858 added the first read-only memory timeline API slice, and PR #859 added dry-run Memory Ops rollout preflight tooling; post-merge main CI `24930333778`, Security Audit `24930333781`, and Build & Publish Docker Images `24930333777` passed at `26197b09`. There are no open PRs during this sync, and code-scanning, Dependabot, and secret-scanning alert lists report 0 open alerts. Historical stability work through PR #856 remains summarized in this roadmap's checkpoint ledger and the main-stability plan registry row. |
