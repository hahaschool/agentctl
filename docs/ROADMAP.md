...
**7,320+ unit tests** across 113+ files + **220+ Playwright e2e checks**, with the backend-independent webhooks, webhook-deliveries, agent-profiles, agent-settings, audit, machines, approvals, notification-bell approvals, agents list, scheduler, settings, settings-push-devices, session-config, and memory-index slices now gated in CI (21 specs / 72 tests). The `/sessions/[id]` and `/memory/dashboard` route-smoke specs add thin browser coverage for detail/dashboard composition without widening the CI allowlist yet, and the live two-node mesh fixture now covers version drift, peer-update dry-run, and schema-ahead rejection behind opt-in gates outside the default lane. The scheduler jobs page is reachable from the web sidebar and covered by focused mocked-query unit tests. All packages build and lint cleanly (TypeScript 0 errors, Biome 0 errors).
...
- [ ] **Playwright coverage.** PR #654 added the opt-in live two-node fixture foundation and first 33.11 version-drift assertion. Remaining 33.8 browser coverage: add peer on A ⇒ sees self row on B, upsert machine on A ⇒ appears on B, break handshake ⇒ UI shows one-way warning.
...
| [mesh-bidirectional-registration-plan](plans/2026-04-15-mesh-bidirectional-registration-plan.md) | In progress — PR #552 signed reverse-registration endpoint, PR #554 machine provenance badges, PR #564 auto-register-on-add plus one-way retry, PR #576 mesh health summary, PR #589 two-node machine replication proof, PR #596 PM2 registration-token passthrough, and PR #654 opt-in live two-node fixture foundation; remaining 33.8 scope is add-peer/reverse-registration, machine visibility, and one-way warning/retry browser assertions | 33.8 |
