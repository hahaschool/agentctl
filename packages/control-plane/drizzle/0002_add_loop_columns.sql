...
ALTER TABLE "agents" ADD COLUMN IF NOT EXISTS "loop_config" jsonb DEFAULT NULL;

-- Add loop_iteration to agent_runs (which iteration of a loop this run represents)
ALTER TABLE "agent_runs" ADD COLUMN IF NOT EXISTS "loop_iteration" integer DEFAULT NULL;

-- Add parent_run_id to agent_runs (links sub-runs to their parent loop run)
ALTER TABLE "agent_runs" ADD COLUMN IF NOT EXISTS "parent_run_id" text DEFAULT NULL;
