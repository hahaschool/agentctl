...
async function fetchDiscovery<T>(url: string): Promise<{ ok: true; data: T } | { ok: false; error: string }> {
  try {
...
export const machineCapabilitiesRoutes: FastifyPluginAsync<MachineCapabilitiesRoutesOptions> =
  async (app, opts) => {
    ...
    app.post<{
      ...
      async (request, reply) => {
        ...
        try {
          await dbRegistry.heartbeat(machineId, updatedCapabilities);
        } catch {
