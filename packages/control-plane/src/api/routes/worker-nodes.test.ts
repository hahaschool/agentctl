...
describe('worker node routes', () => {
  ...
  const mockWorkerNodeStore = {
    listNodes: vi.fn().mockResolvedValue(makeNodes()),
  } as unknown as WorkerNodeStore;

  const mockTaskRunStore = {
    listRuns: vi.fn().mockResolvedValue(makeRuns()),
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (mockWorkerNodeStore.listNodes as ReturnType<typeof vi.fn>).mockResolvedValue(makeNodes());
    (mockTaskRunStore.listRuns as ReturnType<typeof vi.fn>).mockResolvedValue(makeRuns());
