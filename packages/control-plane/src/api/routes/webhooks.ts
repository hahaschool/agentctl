...
export const webhookRoutes: FastifyPluginAsync<WebhookRoutesOptions> = async (app, opts) => {
  ...
  app.post<{ Body: CreateWebhookBody }>('/', async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>('/:id', async (request, reply) => {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        throw new ControlPlaneError('WEBHOOK_NOT_FOUND', `Webhook subscription '${id}' not found`, {
          webhookId: id,
        });
      }
  ...
  app.patch<{ Params: { id: string }; Body: UpdateWebhookBody }>('/:id', async (request, reply) => {
    const { id } = request.params;
    ...
    try {
      const existing = await db.execute(sql`SELECT * FROM webhook_subscriptions WHERE id = ${id}`);
  ...
  app.delete<{ Params: { id: string } }>('/:id', async (request, reply) => {
    ...
    try {
      const existing = await db.execute(sql`SELECT id FROM webhook_subscriptions WHERE id = ${id}`);
