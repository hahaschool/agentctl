...
// Rate limit the PKCE flow: /initiate and /refresh spawn outbound HTTP requests
// to Anthropic and allocate in-memory flow state; /callback is the public-facing
// redirect target. Keep the window tight so a flood cannot exhaust memory,
// saturate outbound egress, or probe pending OAuth states.
...
export const oauthRoutes: FastifyPluginAsync<OAuthRoutesOptions> = async (app, opts) => {
  ...
  // -----------------------------------------------------------------------
  // Rate limiting for the OAuth PKCE flow.
  // Every route in this plugin triggers either an outbound HTTP call to
  // Anthropic or an in-memory flow allocation, so the limiter is mandatory
  // to prevent memory DoS + outbound traffic amplification.
  // -----------------------------------------------------------------------
  const oauthRateLimitMax = readRateLimitEnv('OAUTH_RATE_LIMIT_MAX', OAUTH_RATE_LIMIT.max);
  const oauthRateLimitWindowMs = readRateLimitEnv(
    'OAUTH_RATE_LIMIT_WINDOW_MS',
    OAUTH_RATE_LIMIT.timeWindow,
  );
  const oauthRateLimitError = () => ({
    statusCode: 429,
    error: 'RATE_LIMITED',
    message: 'Too many OAuth requests',
  });
  const oauthFastifyRateLimit = {
    max: oauthRateLimitMax,
    timeWindow: oauthRateLimitWindowMs,
    errorResponseBuilder: oauthRateLimitError,
  } as const;
  ...
  app.post<{
    ...
    '/initiate',
    {
      config: { rateLimit: oauthFastifyRateLimit },
      preHandler: [app.rateLimit(oauthFastifyRateLimit)],
    },
    async (request, reply) => {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!accessToken) {
          return reply
            .type('text/html')
            .send(callbackHtml({ error: 'No access token in response' }));
        }
