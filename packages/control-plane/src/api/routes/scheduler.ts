...
export const schedulerRoutes: FastifyPluginAsync<SchedulerRoutesOptions> = async (app, opts) => {
  ...
  }>('/jobs/heartbeat', async (request, reply) => {
  ...
  app.delete<{ Params: { key: string } }>('/jobs/:key', async (request, reply) => {
  ...
  app.delete<{ Querystring: { confirm?: string } }>('/jobs', async (request, reply) => {
