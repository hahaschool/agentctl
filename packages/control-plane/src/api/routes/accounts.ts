...
// Rate limit the write-path routes and the credential-decryption listing:
// these endpoints persist or decrypt long-lived provider credentials. A flood
// could exhaust the credential store, mass-decrypt stored tokens, or probe
// encryption boundaries. Keep the window tight so abuse is bounded.
const ACCOUNTS_RATE_LIMIT = {
  max: 20,
  timeWindow: 60_000,
} as const;
...
export const accountRoutes: FastifyPluginAsync<AccountRoutesOptions> = async (app, opts) => {
  ...
  // -----------------------------------------------------------------------
  // Rate limiting for account write paths + credential-decrypting list.
  // POST/PUT/DELETE all touch the encrypted credential store, and GET /
  // decrypts every stored credential to build the masked listing — all of
  // which need abuse protection at the Fastify layer.
  // -----------------------------------------------------------------------
  const accountsRateLimitMax = readRateLimitEnv('ACCOUNTS_RATE_LIMIT_MAX', ACCOUNTS_RATE_LIMIT.max);
  const accountsRateLimitWindowMs = readRateLimitEnv(
    'ACCOUNTS_RATE_LIMIT_WINDOW_MS',
    ACCOUNTS_RATE_LIMIT.timeWindow,
  );
  const accountsRateLimitError = () => ({
    statusCode: 429,
    error: 'RATE_LIMITED',
    message: 'Too many account requests',
  });
  const accountsFastifyRateLimit = {
    max: accountsRateLimitMax,
    timeWindow: accountsRateLimitWindowMs,
    errorResponseBuilder: accountsRateLimitError,
  } as const;
  ...
  // Preserve rate-limit (429) and other client-error responses by re-delegating
  // through the rate-limit plugin's error body / statusCode so abuse protection
  // isn't flattened to an opaque 500.
  app.setErrorHandler((error: Error & { statusCode?: number }, request, reply) => {
    if (error.statusCode === 429) {
      return reply.code(429).send(accountsRateLimitError());
    }
    ...
    async (_request, reply) => {
      ...
      const masked = rows.map((r) => {
        ...
        try {
          ...
          app.log.warn(
            { accountId: r.id, err },
            'Failed to decrypt credential for account listing',
          );
        }
        return {
          id: r.id,
