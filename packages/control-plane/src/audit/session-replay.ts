...
export class SessionReplayBuilder {
  /** Build a timeline from raw audit log entries. */
  static buildTimeline(entries: AuditEntry[], sessionId: string, agentId: string): SessionTimeline {
    const sessionEntries = entries
      .filter((e) => e.sessionId === sessionId && e.agentId === agentId)
      .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());

    const events: ReplayEvent[] = sessionEntries.map((entry) =>
      SessionReplayBuilder.entryToEvent(entry),
    );
  ...
  static filterEvents(timeline: SessionTimeline, filter: ReplayFilter): ReplayEvent[] {
  ...
  static generateSummary(timeline: SessionTimeline): SessionSummary {
  ...
  static findSuspiciousPatterns(timeline: SessionTimeline): SuspiciousPattern[] {
    const patterns: SuspiciousPattern[] = [];

    const rapidFire = SessionReplayBuilder.detectRapidFire(timeline.events);
    if (rapidFire) {
      patterns.push(rapidFire);
    }

    const highDenial = SessionReplayBuilder.detectHighDenialRate(timeline.events);
    if (highDenial) {
      patterns.push(highDenial);
    }

    const unusualSequence = SessionReplayBuilder.detectUnusualToolSequence(timeline.events);
    if (unusualSequence) {
      patterns.push(unusualSequence);
    }

    const costSpikes = SessionReplayBuilder.detectCostSpikes(timeline.events);
  ...
  private static entryToEvent(entry: AuditEntry): ReplayEvent {
    const eventType = SessionReplayBuilder.mapKindToEventType(entry.kind);
  ...
  private static mapKindToEventType(kind: string): ReplayEvent['eventType'] {
    switch (kind) {
      case 'pre_tool_use':
      case 'tool_call':
        return 'tool_call';
   ...
   */
  private static detectRapidFire(events: ReplayEvent[]): SuspiciousPattern | null {
   ...
   */
  private static detectHighDenialRate(events: ReplayEvent[]): SuspiciousPattern | null {
    const toolCalls = events.filter((e) => e.eventType === 'tool_call');
   ...
   */
  private static detectUnusualToolSequence(events: ReplayEvent[]): SuspiciousPattern | null {
    const toolCalls = events.filter((e) => e.eventType === 'tool_call');
   ...
   */
  private static detectCostSpikes(events: ReplayEvent[]): SuspiciousPattern[] {
    const patterns: SuspiciousPattern[] = [];
