...
describe('StatusBadge', () => {
  ...
  it('applies animate-status-dot to the dot for pulse statuses (running)', () => {
    const { container } = render(<StatusBadge status="running" />);
    const dot = container.querySelector('span > span') as HTMLElement;
    expect(dot.className).toContain('animate-status-dot');
  });

  it('applies animate-status-dot to the dot for "online"', () => {
    const { container } = render(<StatusBadge status="online" />);
    const dot = container.querySelector('span > span') as HTMLElement;
    expect(dot.className).toContain('animate-status-dot');
  });

  it('applies animate-status-dot to the dot for "active"', () => {
    const { container } = render(<StatusBadge status="active" />);
    const dot = container.querySelector('span > span') as HTMLElement;
    expect(dot.className).toContain('animate-status-dot');
  });

  it('does NOT apply animate-status-dot to the dot for "starting"', () => {
    const { container } = render(<StatusBadge status="starting" />);
    const dot = container.querySelector('span > span') as HTMLElement;
    expect(dot.className).not.toContain('animate-status-dot');
  });

  it('does NOT apply animate-status-dot for non-pulse statuses ("stopped")', () => {
    const { container } = render(<StatusBadge status="stopped" />);
    const dot = container.querySelector('span > span') as HTMLElement;
    expect(dot.className).not.toContain('animate-status-dot');
  });

  it('does NOT apply animate-status-dot for "error" status', () => {
    const { container } = render(<StatusBadge status="error" />);
    const dot = container.querySelector('span > span') as HTMLElement;
