...
describe('native import probes', () => {
  ...
  it('materializes and resumes a synthetic Claude session for Codex imports', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'agentctl-claude-home-'));
    ...
    mockProbeNativeImportPrerequisites.mockResolvedValueOnce({
      reason: 'not_implemented',
      metadata: {
        targetCli: { command: 'claude', available: true, version: '2.1.71 (Claude Code)' },
        sourceStorage: {
          rootPath: '/tmp/.codex',
          exists: true,
          sessionLocated: true,
          sessionPath: '/tmp/.codex/sessions/rollout.jsonl',
        },
        prerequisitesMet: true,
        sourceSessionSummary: {
          recentMessages: [
            { role: 'user', text: 'Switch this task over to Claude Code.' },
            { role: 'assistant', text: 'I will continue from the imported Codex transcript.' },
          ],
        },
      },
    });
  ...
  it('returns a typed prerequisite failure for Codex to Claude imports', async () => {
    mockProbeNativeImportPrerequisites.mockResolvedValueOnce({
  ...
  it('returns a typed resume failure when Claude cannot open the materialized session', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'agentctl-claude-home-'));
    tempDirs.push(homeDir);
    process.env.HOME = homeDir;
    ...
    expect(result.ok).toBe(false);
    expect(result.reason).toBe('resume_failed');
    expect(result.metadata.error).toBe('Claude resume failed');
  });
});
