...
function validatePath(raw: unknown): string {
  if (!raw || typeof raw !== 'string') {
    throw new WorkerError(
      'INVALID_PATH',
      'A non-empty "path" parameter is required',
    );
  }
  ...
  const home = homedir();
  if (!resolved.startsWith(home) && !resolved.startsWith('/tmp') && !resolved.startsWith('/var') && !resolved.startsWith('/opt') && !resolved.startsWith('/usr')) {
    throw new WorkerError(
      'INVALID_PATH',
  ...
  app.put<{ Body: { path?: string; content?: string } }>(
    ...
    async (request) => {
      ...
      if (typeof body.content !== 'string') {
        throw new WorkerError(
          'INVALID_PATH',
          'Request body must include a "content" string field',
        );
      }
